...
Another way of protecting Snapshot copies is with ONTAP link:../snaplock/snapshot-lock-concept.html[snapshot locking]. Snapshot locking is a SnapLock capability where
Snapshot copies are rendered indelible manually or automatically with a retention period on the volume snapshot policy. Snapshot locking is also referred to as tamper-proof snapshot locking. The
purpose of snapshot locking is to prevent rogue or untrusted administrators from deleting Snapshot copies on the primary and secondary ONTAP systems. Rapid recovery of locked Snapshot copies on primary systems can be achieved in order to restore volumes corrupted by ransomware.
