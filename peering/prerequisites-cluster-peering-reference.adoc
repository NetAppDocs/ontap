...
[.lead]
Before you set up cluster peering, you should confirm that the connectivity, port, IP address, subnet, firewall, and cluster requirements are met.
...
The supported number of cluster peers depends on the ONTAP version:

 * ONTAP 9.14.1 and later: Up to 400 cluster peers are supported.
 * Earlier ONTAP releases: Up to 255 cluster peers are supported.
 
 [NOTE]
 These values represent supported and validated limits. There is no strictly enforced maximum cluster peer limit.  

// 2026 May 13, ONTAPDOC-3894
// 2026-Apr-10, GitHub issue #2021
