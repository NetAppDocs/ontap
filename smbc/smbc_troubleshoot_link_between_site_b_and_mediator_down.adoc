...
. Enter the SnapMirror break command (`snapmirror break -destination_path <svm>:<volume>`) on the consistency group constituent volumes to convert volumes from DP to RW, to enable I/O from Site B.
+
