...
SM-BC supports xref:supported-deployment-config-concept.adoc[fan-out configurations] with the `MirrorAllSnapshots` policy and, beginning in ONTAP 9.11.1, the the `MirrorAndVault` policy. Fan-out configurations are not supported with `XDPDefault` policy. 

If you experience a failover on the SM-BC destination in a fan-out configuration, you will have to manually xref:resume-protection-fanout-configuration.adoc[resume protection in the fan-out configuration].
