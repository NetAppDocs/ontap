...
[.lead]
The process of adding and removing volumes in an an active SM-BC relationship depends on the version of ONTAP you are using. 
...
Beginning in ONTAP 9.13.1, you can non-disruptively add volumes to a consistency group with an active SM-BC relationship from the source or destination. Removing volumes is a disruptive operation and you must break the SnapMirror relationship before proceeding removing volumes. 
