...
A planned failover is initiated by the administrator of the secondary cluster. The operation requires switching the active and standby roles so that the secondary cluster takes over from the primary. The secondary cluster can then begin processing input and output requests locally without disrupting client operations.
