...
summary: "Federal Information Processing Standards 140-2 compliant for all SSL connections."
---
= Configure ONTAP network security using FIPS
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
You must reconfigure existing SSH public key accounts to use a supported key type before enabling FIPS, or administrator authentication will fail.

For more information, see link:../authentication/enable-ssh-public-key-accounts-task.html[Enable SSH public key accounts].

ONTAP 9.18.1 introduces support for the ML-KEM, ML-DSA, and SLH-DSA post-quantum computing cryptographic algorithms for SSL, providing an additional layer of security against potential future quantum computer attacks. These algorithms are only available when <<Disable FIPS,FIPS is disabled>>. Post-quantum cryptographic algorithms are negotiated when FIPS is disabled and the peer supports them.
...
. Beginning with ONTAP 9.9.1, rebooting is not required. If you are running ONTAP 9.8 or earlier, manually reboot each node in the cluster individually. 
...
You can enable SSLv3 with the following command. If you are running ONTAP 9.9.1 or later, you do not see the warning message.
....
...
. If you are running ONTAP 9.8 or earlier, manually reboot each node in the cluster individually.
...
* link:../networking/configure-tls-offload.html[Configure TLS hardware offload]

// 2026-May-29, ONTAPDOC-3912
// 2026-Jan-15, ONTAPDOC-3496
// 2026, Jan 08, GH-Public PR - 1935
// 2025 Oct 06, ONTAPDOC-3240
