...
ONTAP 9.18.1 introduces support for postquantum computing cryptographic algorithms for the SSLv3 protocol, providing an additional layer of security against potential future quantum computer attacks. These algorithms are only accessible with the SSLv3 protocol, which can only be enabled when <<Disable FIPS,FIPS is disabled>>.
...
If you need to use the SSLv3 protocol, you must disable FIPS. SSLv3 can only be enabled when FIPS is disabled.

Beginning with ONTAP 9.18.1, SSLv3 in ONTAP supports postquantum computing cryptographic algorithms, offering an additional layer of security against potential future quantum computer attacks. These algorithms are only accessible with the SSLv3 protocol.
