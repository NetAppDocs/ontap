...
ONTAP 9.18.1 introduces support for postquantum computing cryptographic algorithms for the SSLv3 protocol, providing an additional layer of security against potential future quantum computer attacks. These algorithms are only available when <<Disable FIPS,FIPS is disabled>>.
...
Beginning with ONTAP 9.18.1, SSL in ONTAP supports postquantum computing cryptographic algorithms, offering an additional layer of security against potential future quantum computer attacks. These algorithms are only available when FIPS is disabled. After disabling FIPS, the postquantum computing algorithms are used automatically if the client supports them.
