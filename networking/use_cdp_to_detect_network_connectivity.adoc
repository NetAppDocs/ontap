...
summary: "In a data center, you can use neighbor discovery protocols to view network connectivity between a pair of physical or virtual systems and their network interfaces. ONTAP supports two neighbor discovery protocols, Cisco Discovery Protocol and Link Layer Discovery Protocol."
---
...
CDP must also be enabled on any Ethernet switches and routers before information about neighbor devices can be displayed.
...
[NOTE]
Sometimes when LIFs are changed on the node, the CDP information is not updated at the receiving device side (for example, an Ethernet switch). If you encounter such a problem, you should configure the network interface of the node to the down status and then to the up status.
