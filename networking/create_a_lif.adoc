...
+
CAUTION: In a MetroCluster configuration, NetApp strongly recommends that you create a subnet object before you create a LIF. A subnet object allows ONTAP to determine failover targets on the destination cluster because it has an associated broadcast domain. For instructions, refer to link:../networking/create_a_subnet.html[Create a subnet].
+
....
