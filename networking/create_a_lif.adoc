...
+
CAUTION: In a MetroCluster configuration, NetApp strongly recommends that you create a subnet object before you create a LIF. The presence of a subnet object allows ONTAP to determine failover targets on the destination cluster because a subnet object has an associated broadcast domain. link:../networking/create_a_subnet.html[Create a subnet]
+
....
