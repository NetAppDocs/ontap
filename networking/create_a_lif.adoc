...
+
CAUTION: NetApp recommends creating subnet objects for all LIFs on data SVMs. This is especially important for MetroCluster configurations, as the subnet object enables ONTAP to determine failover targets on the destination cluster because each subnet object has an associated broadcast domain. For instructions, refer to link:../networking/create_a_subnet.html[Create a subnet]. 
+
....
