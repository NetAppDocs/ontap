...
* ONTAP utilizes Network Time Protocol (NTP) to synchronize time across the cluster. After changing LIF IP addresses, you need to update the cluster NTP settings to match the new IPs or else synchronization may fail. Refer to link:../software_setup/task_synchronize_the_system_time_across_the_cluster.html[Synchronize the system time across an ONTAP cluster] for more information on NTP configuration.
