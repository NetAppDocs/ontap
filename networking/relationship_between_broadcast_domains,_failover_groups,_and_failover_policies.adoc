...
The `sfo-partner-only` failover policy is not a default, but can be used to when you want the LIF to fail over to a port on the home node or SFO partner only.
