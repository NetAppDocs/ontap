...
. Optionally enter a post-quantum pre-shared key (PPK) identity with the `-ppk-identity` parameter to enhance security against future quantum computer attacks. When you enter a PPK identity, you will be prompted to enter the PPK secret. PPKs are supported for IPsec beginning with ONTAP 9.17.1. PPKs are only supported for pre-shared key (PSK) authentication. PPKs are not supported for certificate authentication.
