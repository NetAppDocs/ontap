...
summary: "There are several tasks you need to perform to configure and activate the IPsec hardware offload feature for selected traffic on your ONTAP cluster."
---
...
[.lead]
There are several tasks you need to perform to configure and activate the IPsec hardware offload feature for selected traffic on your ONTAP cluster.

[NOTE]
Make sure to review link:../networking/ipsec_prepare.html[Prepare to use IP security] before configuring IPsec. For example, you might need to decide whether to use the IPsec hardware offload feature available beginning with ONTAP 9.16.1.
...
If you are using the IPsec hardware offload feature, you can enabled it using the parameter `is-offload-enabled` on the modify command in step 2.
