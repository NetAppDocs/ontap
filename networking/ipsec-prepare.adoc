...
.ONTAP 9.17.1
Support for IPsec hardware offload is extended to link aggregation groups. Postquantum preshared keys (PPKs) are supported for IPsec.
...
Prior to 9.17.1, The IPsec hardware offload feature does not support link aggregation. And so it cannot be used with an interface or link aggregation group as administered through the `network port ifgrp` commands at the ONTAP CLI.
