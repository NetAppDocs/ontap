...
summary: "Beginning with ONTAP 9.8, you have the option to use IP security to protect your network traffic. IPsec is one of several data-in-motion or in-flight encryption options available with ONTAP. You should prepare to configure IPsec before using it in a production environment."
---
= Learn about IP security on the ONTAP network
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
If you are using ONTAP 9.16.1 or later, you have the option of offloading certain computationally intensive operations, such as encryption and integrity checks, to a network interface controller (NIC) card installed at the storage node. The network throughput performance impact for IPsec encryption operations offloaded to the NIC card is approximately 5% or less compared to plaintext operations with no IPsec encryption enabled. This can significantly improve the performance and throughput of the network traffic protected by IPsec.
...
The X50131A, X50131B and X50135A cards are supported on the following platforms:
...
// 2026-Aug-5, GitHub issue #1916
// 9-26-25 ONTAPDOC-3251
