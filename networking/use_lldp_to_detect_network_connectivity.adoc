...
LLDP must also be enabled on any Ethernet switches and routers before information about neighbor devices can be displayed.
...
* Displayed information might differ between the cluster and the Ethernet switch.
+
The Chassis ID and Port ID data displayed by the Ethernet switch might be different for CNA and non-CNA ports.
