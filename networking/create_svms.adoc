...
+
Beginning with ONTAP 9.19.1, SVM administrators can manage and monitor their SVMs using the new SVM dashboard. Use the management network interface IP address and the vsadmin password to access the SVM dashboard.
+
When this option is selected, you can specify the vsadmin password and select the *Add a network interface for storage VM management* check box. 
+
Specify the following for the management network interface for the SVM:
+
* Node
* IP address
* Subnet mask
