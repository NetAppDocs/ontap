...
* The `scan-queue-timeout` value should be lesser than the `scanner-pool request-timeout` value. 
If it is set to a higher value, access to files might be delayed and might eventually time out. 
To avoid this, configure the `scan-queue-timeout` to 5 seconds less than the `scanner-pool request-timeout` value. Refer to the scan engine vendor’s documentation for instructions on how to change the `scan-queue-timeout` settings. The `scanner-pool timeout` can be changed by using the following command in advanced mode and by providing the appropriate value for the `request-timeout` parameter:
