...
The default System Manager session inactivity timeout is 30 minutes. A timeout is important to prevent stale sessions and session piggybacking.

NOTE: If SAML is configured, the inactivity timeout is controlled by settings on the IdP.

[role="tabbed-block"]
====
.System Manager 9.7 and later
--
...
.System Manager 9.7 and earlier
--

. Select the image:icon_gear.gif[Actions icon].
. Select *General* on the left column.
. Change the minutes value between 2 and 180 or enter "0" to disable the timeout.
. Select *Save*.
--
