...
* When FIPS mode is enabled, TLSv1 and SSLv3 are disabled in accordance to key elements of the FIPS 140-2 standard. 

* Only TLSv1.1 and TLSv1.2 remain enabled.

NOTE: ONTAP prevents you from enabling TLSv1 and SSLv3 when FIPS 140-2 compliance is enabled. If you enable FIPS 140-2 and then subsequently disable it, TLSv1 and SSLv3 remain disabled, but TLSv1.2 or both TLSv1.1 and TLSv1.2 remain enabled, depending on the previous configuration.
...
. Set the `is-fips-enabled` parameter to `true` using the `security config modify` command.
. Confirm online status with the `security config show command`.
