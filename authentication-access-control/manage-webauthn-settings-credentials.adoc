...
Beginning with ONTAP 9.19.1, administrators must use the parameter -rp-domains in the WebAuthn global settings command. This allows administrators to explicitly restrict the set of permitted Relying Party (RP) domains accepted during WebAuthn (FIDO2) multi-factor authentication in System Manager. 

Users will only be able to access MFA from the allowed RP domains.

NOTE: If you are upgrading to ONTAP 9.19.1 from ONTAP 9.16.1 through 9.18.1, see link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-597688[CONTAP-597688] for more information. 
...
.System Manager
--
. Select *Cluster > Settings > Multifactor authentication (MFA)*.
. Add, delete, or modify the allowed RP domains. 
--

.CLI
--
. Modify the allowed rp domains. Set -rp-domains to the full cluster FQDN:
+
[source,console]
----
security webauthn modify -rp-domains <mycluster.us-east.example.com>
----
Learn more about `security webauthn modify` in the link:https://docs.netapp.com/us-en/ontap-cli/security-webauthn-modify.html#parameters[ONTAP command reference^]. 
--
====
...
--
====
// 2026 July 22, CLI link update
// 2026 July 13, CONTAP-686913
// 2026 April 1, GH-1967
