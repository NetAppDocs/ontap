...
sidebar: sidebar
keywords: pnfs, performance, tuning, best practices, considerations, flexgroup, metadata, nconnect, session trunking, kerberos
summary: "Follow these best practices and considerations when using pNFS in ONTAP for optimal performance and reliability."
---
...
[.lead]
When using pNFS in NetApp ONTAP, there are considerations and best practices that should be kept top of mind for best results. For questions or suggestions, contact NetApp at doccomments@netapp.com and include TECHNICAL REPORT 4063 BEST PRACTICES in the subject line.

== Volume type recommendation

pNFS in NetApp ONTAP works with both FlexVol volumes and FlexGroup volumes, but for the best overall results, use FlexGroup volumes.
...
* Massive capacity possibilities (up to 60PB) and high file counts (up to 200 billion files)
...
However, since NFSv4.1 is a stateful protocol, a network interface migration can be disruptive to clients that are actively using the NFS mount. It is a best practice to conduct network interface migrations in a maintenance window and notify clients of potential network disruptions.

.Related information

* link:../networking/migrate_a_lif.html[Migrate a LIF]
...
* link:../nfs-admin/specify-nfsv4-id-domain-task.html[Specify the NFSv4 ID domain]

== nconnect

As mentioned previously, nconnect in ONTAP can help improve performance in some workloads. With pNFS, it is important to understand that while nconnect can improve performance by greatly increasing the total number of TCP connections to the storage system, it can also create issues when many clients are leveraging the mount option by overwhelming the TCP connections on the storage. NetApp's hardware universe covers the TCP connection limits per node.

When a node's TCP connection limits are exceeded, no new TCP connections are allowed until existing connections are freed. This can create complications in environments that may experience mount storms.

The following table shows how pNFS with nconnect may overwhelm TCP connection limits:
...
While the two features can feasibly be used together, using NFSv4.1 referrals with pNFS may result in the undesired effect of stacking multiple metadata servers on the same node and reducing the ability to spread metadata servers across multiple cluster nodes. If metadata servers are not spread evenly across a cluster when using pNFS, then a single node's CPU can get overwhelmed with metadata requests and create a performance bottleneck.
...
[cols="20,40,40"]
|===
| Kerberos flavor | MB/s | Completion time (s)
...
* link:../nfs-config/kerberos-nfs-strong-security-concept.html[Kerberos with NFS for strong security]
...
The following section covers ONTAP CLI commands that pertain specifically to pNFS. This includes configuration, troubleshooting, and statistics.
...
=== Show pNFS devices (advanced priv)
...
=== Show pNFS device mappings (advanced priv)
...
=== Capture pNFS-specific performance counters (advanced priv)

----
::*> statistics start -object nfsv4_1 -vserver SVM -sample-id [optional-name]
----

=== View pNFS-specific performance counters (advanced priv)

----
::*> statistics show -object nfsv4_1 -vserver SVM
----

=== View list of pNFS-specific counters (advanced priv)
...
=== Viewing active network connections for NFS
...
=== Viewing NFS version information for connected clients
...
For instance, to see clients with activity in the past 10 minutes for the PNFS SVM:
...
10.x.x.a nfs4.2 PNFS_root default 9m 10s 0 149 false 10.x.x.a nfs4.2 FG__0001 default 9m 10s 135847 0 false 10.x.x.b nfs4.2 PNFS_root default 8m 12s 0 157 false 10.x.x.b nfs4.2 FG__0001 default 8m 12s 52111 0 false
----
