...
sidebar: sidebar
keywords: pnfs, architecture, metadata server, data server, parallel nfs, layout
...
Clients that wish to access pNFS need network connectivity to data and metadata paths available on the NFS server. If the NFS server contains network interfaces that are not reachable by the clients, then the server may advertise data paths to the client that are inaccessible, which can cause outages.
...
image::pNFFS_fig_007.png[Establishing the metadata server in pNFS in NetApp ONTAP]
...
image::pNFFS_fig_008.png[Packet capture for pNFS mount]
...
image::pNFFS_fig_009.png[Remote read path using NFSv4.1 without pNFS]

image::pNFFS_fig_010.png[Localized read path using pNFS]
...
== Detailed steps – pNFS devices

The following shows the details of how a pNFS device populates in ONTAP once a client makes a request to read or write a file in a volume.
...
FlexGroup volumes in ONTAP present storage via FlexVol volume constituents that span multiple nodes in a cluster, which allows a workload to leverage multiple hardware resources while maintaining a single mountpoint. Since multiple nodes - with multiple network interfaces – are interacting with the workload, it's a natural result to see remote traffic traverse the backend cluster network in ONTAP.

image::pNFFS_fig_011.png[Single file access in a FlexGroup volume without pNFS]

When utilizing pNFS, ONTAP keeps track of the file and volume layouts of the FlexGroup volume and maps them to the local data interfaces in the cluster. For instance, if a constituent volume contains a file being accessed resides on node 1, then ONTAP will notify the client to redirect the data traffic to the data interface on node 1.

image::pNFFS_fig_012.png[Single file access in a FlexGroup volume with pNFS]

pNFS also provides a way to present parallel network paths to files from a single client that NFSv4.1 without pNFS does not offer. For instance, if a client wants to access 4 files at the same time from the same mount using NFSv4.1 without pNFS, the same network path would be utilized for all files and the ONTAP cluster would instead send remote requests to those files. The mount path can become a bottleneck for the operations, as they all follow a single path and arrive at a single node and is also servicing metadata operations along with the data operations.

image::pNFFS_fig_013.png[Multiple simultaneous file access in a FlexGroup volume without pNFS]

When pNFS is used to access the same 4 files simultaneously from a single client, the client and server negotiate local paths to each node with the files and uses multiple TCP connections for the data operations, while the mount path acts as the location for all metadata operations. This provides latency benefits by using local paths to the files, but also can add throughput benefits by way of multiple network interfaces being used, provided the client(s) can send enough data to saturate the network.

image::pNFFS_fig_014.png[Multiple simultaneous file access in a FlexGroup volume with pNFS]
