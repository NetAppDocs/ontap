...
sidebar: sidebar
keywords: pnfs, use cases, nconnect, session trunking, referrals, advanced capacity balancing
...
When using nconnect with pNFS, connections will parallelize via the nconnect option over each pNFS device advertised by the NFS server. For instance, if nconnect is set to 4 and there are 4 eligible interfaces for pNFS, then the total number of connections created will be 16 per mount point.

image::pNFFS_fig_015.png[pNFS with nconnect=4]
...
image::pNFFS_fig_016.png[NFSv4.1 session trunking in ONTAP]
...
image::pNFFS_fig_017.png[NFSv4.1 session trunking with pNFS in ONTAP]
...
image::pNFFS_fig_018.png[NFS session trunk discovery during mount – packet capture]
...
Advanced Capacity Balancing in ONTAP writes portions of file data across constituent volumes of a FlexGroup volume (not supported with single FlexVol volumes). As a file grows, ONTAP decides to begin writing data to a new multipart inode on a different constituent volume which may be on the same node or a different node. Writes, reads, and metadata operations to these multi-inode files are transparent and non-disruptive to clients. Advanced Capacity Balancing improves space management among the FlexGroup constituent volumes which provides for more consistent performance.

Recall that pNFS can redirect data IO to a localized network path depending on the file layout information stored in the NFS server. When a single large file is created in parts across multiple constituent volumes that can potentially span multiple nodes in the cluster, pNFS in ONTAP can still provide localized traffic to each file part because ONTAP maintains the file layout information for all of the file parts as well. When a file is read, then the data path locality will change as needed.

image::pNFFS_fig_019.png[Advanced Capacity Balancing with pNFS]
