...
summary: "You can use pNFS with various ONTAP features including nconnect, session trunking, and advanced capacity balancing to improve performance and flexibility."
---
...
[.lead]
You can use pNFS with various ONTAP features to improve performance and provide additional flexibility for NFS workloads.
...
In ONTAP, NFSv4.1 session trunking is used across multiple physical interfaces on a single node in a cluster to provide extra throughput and redundancy across independent connections. 

image::pNFS_fig_006.png[NFSv4.1 session trunking across multiple interfaces on single node]

When used with pNFS, these trunks can provide more overall aggregated performance across the entire cluster.

image::pNFS_fig_021.png[NFSv4.1 with pNFS performance across cluster]

Session trunking is established in multiple ways:

* *Discover automatically via mount options:* Session trunking in most modern NFS clients is established using mount options (check your OS vendor's documentation) that signal to the NFS server to send information back to the client about session trunks. This information appears through an NFS packet as an `fs_location4` call.
+
...
----
+
NOTE: If you attempt to use `max_connect` on RHEL distros, it will be treated as nconnect instead and session trunking will not work as expected.
...
NOTE: Each node gets its own session trunk. Trunks do not traverse nodes.

NOTE: When using pNFS, use only session trunking _or_ nconnect. Using both will result in undesirable behavior, such as only the metadata server connection getting the benefits of nconnect with the data servers using a single connection.
...
When `trunkdiscovery` is used, an added GETATTR call (FS_Locations) is leveraged for the listed session trunk interfaces on the NFS server node where the mount interface is located. Once those are returned, subsequent mounts are made to the returned addresses. This is seen in a packet capture during mount.
...
You can use NFSv4.1 referrals with pNFS, but the functionality is unnecessary. Enabling referrals with pNFS will not show noticeable results.
...
link:../flexgroup/enable-adv-capacity-flexgroup-task.html[Advanced capacity balancing] in ONTAP writes portions of file data across constituent volumes of a FlexGroup volume (not supported with single FlexVol volumes). As a file grows, ONTAP decides to begin writing data to a new multipart inode on a different constituent volume which might be on the same node or a different node. Writes, reads, and metadata operations to these multi-inode files are transparent and non-disruptive to clients. Advanced capacity balancing improves space management among the FlexGroup constituent volumes which provides for more consistent performance.
