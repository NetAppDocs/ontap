...
Session trunking can be established multiple ways:
...
+
The mount option in use depends on the client OS version. For instance, Ubuntu Linux flavors generally use `max_connect=n` to signal a session trunk is to be used. In RHEL Linux distros, the `trunkdiscovery` mount option is used.
+
.Ubuntu example
----
...
* *Establish manually:* You can establish session trunking manually by mounting each individual IP address to the same export path and mount point. For example, if you have two IP addresses on the same node (10.10.10.10 and 10.10.10.11) for an export path of `/pNFS`, you would run the mount command twice:
+
----
...
NOTE: When using pNFS, use only either session trunking _or_ nconnect. Using both will result in undesirable behavior, such as only the metadata server connection getting the benefits of nconnect with the data servers using a single connection.
