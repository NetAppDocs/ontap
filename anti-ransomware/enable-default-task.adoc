...
[.lead]
Beginning with ONTAP 9.10.1, you can configure storage VMs (SVMs) such that new volumes are enabled by default for Autonomous Ransomware Protection (ARP).

For ARP 9.10.1 to 9.15.1, you can configure new volumes to enable ARP in learning mode, in which the system analyzes the workload to characterize normal behavior. For ARP/AI 9.16.1, ARP is enabled automatically in active mode for new volumes and no learning period is required.
