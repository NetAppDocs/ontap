...
Beginning with 9.18.1, you can manually enable ARP by default on all new volumes from the cluster level (select *Cluster > Security* and image:icon_kabob.gif[Menu options icon] in the *Anti-ransomware* section then select *Enable on all existing volumes*). Prior to ONTAP 9.18.1, if your system is running an earlier version of ONTAP and you'd like to link:enable-default-task.html[enable ARP by default in all new volumes], you can change this setting at the SVM level.
