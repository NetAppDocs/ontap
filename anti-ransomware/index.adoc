...
Beginning with ONTAP 9.18.1, ARP is enabled by default on all new volumes for AFF A and C series platforms. Default enablement is in effect after a 12-hour grace period following an upgrade or immediately for a new ONTAP 9.18.1 installation, provided that an ARP license is installed. Existing volumes must be link:enable-task.html[manually enabled] for ARP.
