...
.Automatic default enablement on new AFF volumes

Beginning with ONTAP 9.18.1, ARP is enabled by default on all new volumes for AFF A series and AFF C series, ASA, and ASA r2 systems. Default enablement is in effect after a 12-hour grace period following an upgrade or immediately for a new ONTAP 9.18.1 installation, provided that an ARP license is installed. Existing volumes must be link:enable-task.html[manually enabled] for ARP.

This default ARP enablement does not apply to these link:use-cases-restrictions-concept.html#unsupported-configurations[unsupported volumes or configurations].

During the grace period, you can link:default-enable-change.html[opt out of default enablement for new volumes using System Manager or the ONTAP CLI]. If you do not opt out, ARP is automatically enabled for all new volumes created after the end of the grace period.

.Manual default enablement on new volumes
Beginning with 9.18.1, you can manually enable ARP by default on all new volumes from the cluster level (select *Cluster > Security* and select image:icon_pencil.gif[Edit icon] in *Anti-ransomware* section). Prior to ONTAP 9.18.1, if your system is running an earlier version of ONTAP and you'd like to link:enable-default-task.html[enable ARP by default in all new volumes], you can change this setting at the SVM level.
...
| Manual enablement per volume or SVM-level default setting; default enablement on all supported volumes in 9.18.1 and later
