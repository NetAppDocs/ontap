...
a| Evaluation
a| A two- to four-week evaluation period is performed to determine baseline encryption behavior. Detection and alerts can occur while baseline thresholds are being established. You can determine if the evaluation period is complete by running the `security anti-ransomware volume show` command and checking `Block device detection status`. link:anti-ransomware/respond-san-entropy-eval-period.html[Learn about the ONTAP ARP/AI evaluation period for block-device workloads].
a| NAS volumes that contain ONTAP-detected hypervisor virtual disks with ONTAP 9.17.1P5 and later
...
link:anti-ransomeware/respond-san-entropy-eval-period.html[Learn more about SAN volumes and the entropy evaluation period].
