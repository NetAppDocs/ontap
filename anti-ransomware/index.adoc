...
* If you are reverting from ONTAP 9.11.1 and later to ONTAP 9.10.1, and you have enabled ARP with the Anti-ransomware license, you will see a warning message and might need to reconfigure ARP.
+
...
ARP is designed to protect against denial-of-service attacks where the attacker withholds data until a ransom is paid. ARP offers real-time ransomware detection based on the following: 
...
** *File IOPS*: (Used in NAS only) A surge in abnormal volume activity with data encryption (beginning with ONTAP 9.11.1)
...
ARP can be run in one of several modes depending on the complexity and evaluation needs of your environment. A learning or evaluation mode is used to establish a baseline of normal workload behavior. During learning or evaluation, ARP analyzes system metrics to develop an alert profile before enabling active protection. After ARP transitions to an active detection mode, it begins monitoring abnormal activity in real time, automatically taking protective actions and generating alerts if abnormal behavior is detected.

For ARP, the learning mode and active mode behaviors differ by ONTAP version, volume type, and protocol.

=== NAS environments and mode types

Learning and active modes are used for NAS environments. With ARP/AI beginning in ONTAP 9.16.1, there is no learning period when ARP is used with FlexVol volumes. 
...
|===
| Mode | State | Description | Versions and volume types
| Learning 
a| `dry-run` 
a| Learning mode is set automatically when you enable ARP for ONTAP 9.10.1 to 9.15.1. In learning mode, the ONTAP system develops an alert profile based on the analytic areas: entropy, file extension types, and file IOPS. It's recommended that you leave ARP in learning mode for 30 days. Beginning with ONTAP 9.13.1, ARP automatically determines the optimal learning interval and automates the switch, which may occur before 30 days.
...
a| * FlexVolumes with ONTAP 9.10.1 to 9.15.1 
* FlexGroup volumes with ONTAP 9.16.1

| Active
| `enabled` 
a| After running ARP in learning mode for enough time to assess workload characteristics, you can switch to active mode and start protecting your data. Beginning with ONTAP 9.13.1, ARP automatically determines the optimal learning interval and automates the switch, which may occur before 30 days.
For ARP running with ONTAP 9.10.1 to 9.15.1, ARP switches to active mode after the optimal learning period is completed. After ARP has switched to active mode, ONTAP creates ARP snapshots to protect the data if a threat is detected.
...
SAN environments use evaluation periods (similar to learning modes in NAS environments) before transitioning to active detection. The following table summarizes evaluation and active modes.

[cols="1,1,5,1", options="header"]
|===
| Mode | Status | Description | Versions and volume types
| Learning
a| `evalution-period`
...
a| * FlexVolumes with ONTAP 9.17.1 and later

| Active
a| `Active_suitable_workload`
a| A two- to four-week evaluation period is performed to determine baseline encryption behavior. You can determine if the evaluation period is complete by running the `security anti-ransomware volume show` command and checking `Block device detection status `. Learn more about `security anti-ransomware volume show` in the link:https://docs.netapp.com/us-en/ontap-cli/security-anti-ransomware-volume-show.html[ONTAP command reference^].
a| * FlexVolumes with ONTAP 9.17.1 and later
|===

== Threat assessment and ARP snapshots

ARP assesses threat probability based on incoming data measured against learned analytics. When ARP detects a abnormality, a measurement is assigned. A snapshot might be assigned at the time of detection or at regular intervals.
...
The following table summarizes the differences between ONTAP 9.16.1 and earlier and ONTAP 9.17.1 for ARP snapshots.
...
Snapshots creation interval based on trigger
...
| 6 snapshots (configurable limit)
| 6 snapshots (configurable limit)

| Retention period
a| * Determined based on trigger conditions (not fixed)
* Snapshots created before the attack are retained until admin marks the attack as true or a false positive (clear-suspect).
a| Snapshots are normally retained for 12 hours. 

If an attack is detected:

* (NAS) Snapshots created before the attack are retained until admin marks the attack as true or a false positive (clear-suspect).
