...
.ARP snapshot differences by version

* *Modify retention settings*: Beginning with ONTAP 9.11.1, you can link:modify-automatic-snapshot-options-task.html[modify retention settings for snapshots].

* *Intervals and anomaly response*: Beginning with ONTAP 9.17.1, ARP snapshots are generated at regular intervals for both NAS and SAN volumes as well as in response to detected anomalies, which has always been the standard response. ONTAP prepends a name to the ARP snapshot to make it easily identifiable.

* *SnapMirror synchronous and active sync*: Beginning with ONTAP 9.19.1 RC, ARP snapshots created for volumes that participate in SnapMirror synchronous (SMS) or SnapMirror active sync (SMAS-SAN) relationships follow these additional rules:

** ARP snapshots are created and retained on the primary volume.
** ARP snapshots are not replicated to the secondary as part of SMS or SMAS. You need to link:enable-task.html[enable ARP again] on the recovery volume after failover.

NOTE: Volume-level restore (`volume snapshot restore`) on SMS or SMAS-SAN volumes might require that you temporarily quiesce or break the SMS or SMAS relationship. In many cases, you can avoid breaking SMS or SMAS by using FlexClone or file-level restore operations from ARP or other snapshots on the primary volume.
