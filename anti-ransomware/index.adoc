...
NAS environments use learning and active modes. For NAS environments that support <<arp-ai,ARP/AI>>, there is no learning period and active monitoring begins immediately.
...
After the optimal learning period has completed and ARP is in active mode, ONTAP creates ARP snapshots to protect the data if a threat is detected.
...
* SAN volumes with ONTAP 9.17.1 and later (after an link:respond-san-entropy-eval-period.html[evaluation period])
