...
== Expanded ARP/AI attack evaluation and detection

Beginning with ONTAP 9.17.1, ARP/AI supports attack detection inside block-device volumes (SAN LUNs) or virtual disks in an NFS datastore. ARP/AI initially detected only VMware virtual disks. 
Beginning with ONTAP 9.17.1 P5, ARP/AI detection for block-device volumes is expanded to hypervisor virtual disks like Hyper-V, KVM, and OpenStack. The block-device volume evaluation period applies in the following scenarios: 

* SAN volumes 
* NAS volumes when the volume contains VMware datastore that ONTAP automatically detects
** The evaluation period is applicable to the attack detected based on the entropy changes within the guest filesystem of the virtual disk (for example, the ransomware operating on files within the guest filesystem).
** The evaluation period is not applicable to the attack detected based on the entropy and file-extension changes made directly to the virtual disks from ESXi host (for example, the ransomware operating directly on the vmdk files from the NFS datastore mount point).
+
Detection and alerts can occur during the evaluation period using a conservative threshold while baseline thresholds are being established automatically over the course of several days.
