...
[.lead]
When Autonomous Ransomware Protection (ARP) detects abnormal activity in a protected volume, it issues a warning. You should evaluate the notification to determine whether the activity is acceptable (false positive) or whether an attack seems malicious. After you categorize the attack, you can clear the warning and suspected files.

When ONTAP detects an abnormality, it also creates link:index.html#threat-assessment-and-arp-snapshots[a snapshot] of the volume to create the best recovery point. Individual ARP snapshots are retained for two days. If there are multiple ARP snapshots, they are retained for five days by default. These ARP-protected snapshots cannot be deleted manually during the defined retention period. Beginning with ONTAP 9.11.1, you can modify the xref:manage-parameters-task.html#modify-alerts[retention settings] for ARP snapshots.
...
a| False Positive a| . Select *Update* and *Clear Suspect File Types* to record your decision and resume normal ARP monitoring.
+
...
+
This action clears the warning notices and ARP resumes normal monitoring of the volume. ARP snapshots cannot be deleted until the end of the retention period.
...
This action clears the attack report. ARP snapshots cannot be deleted until the end of the retention period. There are no suspect file types to clear if you restored an entire volume.
|===
--
...
+
This operation clears the warning notices and ARP resumes normal monitoring of the volume. ARP snapshots cannot be deleted until the end of the retention period. There are no suspect file types to clear if you restored an entire volume.
...
`[-start-time <date_time> -end-time <date_time>]` Starting and ending times for the range of files to be cleared, in the form "MM/DD/YYYY HH:MM:SS".
+
This operation clears the attack report. ARP snapshots cannot be deleted until the end of the retention period. There are no suspect file types to clear if you restored an entire volume.
