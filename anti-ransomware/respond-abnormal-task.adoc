...
When an ARP warning is issued, respond by designating the file activity in one of two ways:
...
ARP must be actively protecting and not in learning mode.
...
For ONTAP 9.15.1 and earlier, the ARP snapshots are automatically deleted after you clear suspected file types. For ARP/AI in ONTAP 9.16.1 and later, ARP snapshots are automatically deleted after an abbreviated retention period triggered by the categorization operation.
