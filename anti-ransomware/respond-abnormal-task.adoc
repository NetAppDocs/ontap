...
Follow these steps when you need to classify abnormal activity and clear ARP notices:
...
* <<Classify as false positive and resume monitoring,Flow 1: Classify as false positive and resume monitoring>>
* <<Classify as potential ransomware attack, recover data, and finalize,Flow 2: Classify as potential ransomware attack, recover data, and finalize>>
...
* For ARP/AI in ONTAP 9.16.1 and later, ARP snapshots are automatically deleted after an abbreviated retention period triggered by the categorization operation.
* For ONTAP 9.15.1 and earlier, if you intend to recover data using an ARP snapshot, perform the recovery before running `clear-suspect`. This restriction does not apply if you are restoring from a non-ARP snapshot. For ONTAP 9.15.1 and earlier, ARP snapshots are immediately deleted when you clear a suspected event.
...
Use this flow when the identified file type or entropy spike is _unexpected_ in the workload.
...
. Recover data by using link:recover-data-task.html[a ARP snapshot or non-ARP snapshot].
