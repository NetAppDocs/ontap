...
Use this page when you need to classify abnormal activity and clear ARP notices. If you need to restore data, use link:recover-data-task.html[Restore data from ONTAP ARP snapshots after a ransomware attack].
...
== Snapshot behavior and approvals when you clear a suspected event
...
* For ONTAP 9.15.1 and earlier, ARP snapshots are immediately deleted when you clear a suspected event.
* For ONTAP 9.15.1 and earlier, if you intend to recover data using an ARP snapshot, perform the recovery before running `clear-suspect`. This restriction does not apply if you are restoring from a non-ARP snapshot.
...
== Flow 1: Classify as false positive and resume monitoring
...
== Flow 2: Classify as potential ransomware attack, recover data, and finalize

Use this flow when the identified file type or entropy spike is unexpected in the workload.
...
. Recover data by using link:recover-data-task.html#restore-after-system-attack[Flow 1 or Flow 2 in the recovery topic].
