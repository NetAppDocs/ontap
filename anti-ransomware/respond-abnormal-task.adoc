...
** For NAS file type warnings, mark selected files as *Potential ransomware attack* and link:recover-data-task.html[restore protected data].
** For entropy spikes (NAS and SAN) that indicate an attack, select *Mark as potential ransomware attack* and link:recover-data-task.html[restore protected data].
. [[arp-snapshot-decision-sm]]After data restoration is complete, record your decision and resume normal ARP monitoring. Recording your decision clears the attack report.
+
NOTE: For ARP/AI in ONTAP 9.16.1 and later, ARP snapshots are automatically deleted after an abbreviated retention period triggered by the categorization operation. For ONTAP 9.15.1 and earlier, if you intend to recover data using an ARP snapshot, restore the data from that ARP snapshot before selecting *Update and Clear Suspected File Types*. This restriction does not apply if you are restoring from a non-ARP snapshot. ARP snapshots are immediately deleted when you clear the suspected event. 
+
** For NAS file type warnings, select *Update and Clear Suspected File Types*.
** For entropy spikes (NAS and SAN), select *Mark as potential ransomware attack* and select *Save and dismiss*.
+
...
.Generate an attack report (NAS volumes only)
...
[NOTE]
====
This command is *not supported* on volumes that contain LUNs or NVMe namespaces (SAN workloads).
====

.Take action
. Take one of the following actions based on your evaluation of the file extensions (NAS only) or entropy spikes (NAS and SAN):
...
** Potential ransomware attack
+
. Respond to the attack and link:../anti-ransomware/recover-data-task.html[recover data from the ARP-created backup snapshot]. 
...
+
This `clear-suspect` operation clears the attack report. There are no suspected file type notices to clear if you restored an entire volume. For ARP/AI in ONTAP 9.16.1 and later, ARP snapshots are automatically deleted after an abbreviated retention period triggered by the categorization operation. For ONTAP 9.15.1 and earlier, ARP snapshots are immediately deleted when you clear a suspected event. For ONTAP 9.15.1 and earlier, if you intend to recover data using the ARP snapshot, perform the recovery before running `clear-suspect`. This restriction does not apply if you are restoring from a non-ARP snapshot.
