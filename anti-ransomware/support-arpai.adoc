...
keywords: ARP/AI, autonomous ransomware protection AI, ONTAP 9.16.1, supported configurations, SAN ransomware, NAS ransomware, machine learning ransomware, hypervisor ransomware, SnapMirror ARP, AFF ransomware protection, ASA ransomware, FlexVol ARP, FlexGroup ARP, current ARP model ONTAP, ARP/AI requirements ONTAP, SAN ransomware support ONTAP
summary: "ARP/AI (ONTAP 9.16.1 and later) is the current ARP model and supports NAS and SAN workloads across FlexVol and FlexGroup volumes on AFF, ASA, FAS, and ASA r2 systems."
---

= Supported configurations for ONTAP ARP/AI
:icons: font
:imagesdir: ../media/

[.lead]
Autonomous Ransomware Protection with AI (ARP/AI) is available for NAS and SAN workloads beginning with ONTAP 9.16.1. Before deploying ARP/AI, review the supported configurations and use cases, and confirm your environment meets the requirements.
...
* ONTAP 9.18.1 and later: FlexGroup volumes with full ARP/AI parity with FlexVol volumes.
...
NOTE: Because every write operation is committed to both volumes in a synchronous mirror relationship, enabling ARP/AI on the primary volume only is sufficient to detect anomalies. Internal metadata generated by ARP analytics on the primary volume is replicated to the secondary. Because of this, if ARP/AI is enabled on the secondary volume after a failover, the required metadata will already exist.

If a SnapMirror _asynchronous_ source volume is ARP-enabled, the SnapMirror destination volume automatically acquires the ARP configuration state (such as `dry-run` or `enabled`), ARP training data, and ARP-created snapshots of the source volume. No explicit enablement is required.

Although the asynchronous destination volume consists of read-only (RO) snapshots, no ARP processing is done on its data. However, when the SnapMirror asynchronous destination volume is converted to read-write (RW), ARP/AI is automatically enabled on the RW-converted destination volume.
...
| 60
a| * NAS: 5% of maximum IOPS for ONTAP 9.16.1 and later
