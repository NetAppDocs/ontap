...
. If you're running ONTAP 9.13.1 to 9.15.1, adaptive learning is enabled so that the change to active state is done automatically (beginning with ONTAP 9.16.1, ARP/AI does not require adaptive learning). If you do not want this behavior to be automatically enabled, change the setting at the SVM level on all associated volumes:
+
