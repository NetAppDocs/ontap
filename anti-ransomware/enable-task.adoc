...
** For 9.15.1 and earlier and FlexGroup volumes, set the volume state to `dry-run` (learning mode):
+
`security anti-ransomware volume dry-run -volume <vol_name> -vserver <svm_name>`

** For 9.16.1 and later with ARP/AI and FlexVol volumes, set the volume state to `active` (active mode):
+
`security anti-ransomware volume active -volume <vol_name> -vserver <svm_name>`

. If you upgraded to ARP 9.13.1 or later and the default state is `dry run`, adaptive learning is enabled so that the change to active state is done automatically. If you do not want this behavior to be automatically enabled, change the setting at the SVM level on all associated volumes:
+
...
** For 9.15.1 and earlier and FlexGroup volumes, set the state to `dry-run` (learning mode):
+
...
** For 9.16.1 and later with ARP/AI and FlexVol volumes, set the state to `active` (active mode):
+
