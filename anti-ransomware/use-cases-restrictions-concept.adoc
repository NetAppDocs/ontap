...
^3^ Beginning with ONTAP 9.19.1 RC, ARP/AI supports primary volumes in SnapMirror synchronous relationships for NAS and SAN on FAS, AFF A-series, AFF C-series, and AFX systems, and primary volumes in SnapMirror active sync SAN relationships on AFF A-series, AFF C-series, and ASA r2 systems. For more information, see <<snapmirror>>.
...
Beginning with ONTAP 9.19.1 RC, ARP/AI supports primary volumes that participate in SnapMirror synchronous and SnapMirror active sync SAN relationships, with the following behavior:
...
* ARP configuration state and ARP snapshots are not replicated to the secondary volume as part of SnapMirror synchronous or SnapMirror active sync in ONTAP 9.19.1 RC.
...
=== After upgrading to ONTAP 9.18.1
...
=== After upgrading to ONTAP 9.19.1 RC

If you enable ARP on volumes that are part of SnapMirror synchronous or SnapMirror active sync relationships after the upgrade, the relationship might briefly move to an out-of-sync state. This condition is temporary, and the relationship should automatically resynchronize without intervention.
