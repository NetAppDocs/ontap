...
[.lead]
Autonomous Ransomware Protection (ARP) is available for NAS workloads beginning with ONTAP 9.10.1 and SAN workloads beginning in ONTAP 9.17.1. Before deploying ARP, you should be aware of the recommended uses and supported configurations as well as performance implications. You should also ensure that your volume's workload is suited to ARP and that it meets required system configurations. 
...
* Beginning with ONTAP 9.10.1 through 9.15.1, NAS workloads on volumes with more than 4,000 unique file extensions.
+
The original ARP model can analyze up to 4,000 file extensions per volume. If the number of unique file extensions on a volume exceeds this limit, ARP cannot efficiently detect ransomware activity and the workload is considered unsuitable for ARP. This limit does not apply to ARP/AI beginning with ONTAP 9.16.1.
...
ARP supports NAS and SAN workloads across multiple ONTAP versions and environments. Review support capabilities to confirm that your environment and configuration are supported before deploying ARP.

=== Baseline support

Baseline ARP support starts in ONTAP 9.10.1 and includes NAS workloads (NFS and SMB) on FlexVol volumes and MetroCluster configurations.

=== Protocol and hypervisor support by ONTAP version

In addition to baseline NAS support, later ONTAP releases add support for the following protocols and environments:

* ONTAP 9.17.1 and later: SAN block-device workloads (volumes containing LUNs or NVMe namespaces) and NAS volumes containing virtual disks from VMware
...
| Volumes protected with SnapMirror synchronous ^3^| ✓ |  |  |  |  |  |  |  | |
| Volumes protected with SnapMirror active sync ^3^| ✓ |  |  |  |  |  |  |  | |  
...
^1^ ONTAP 9.16.1 and 9.17.1 do not provide ARP/AI support for FlexGroup volumes; beginning in ONTAP 9.18.1, FlexGroup volumes use the ARP/AI model. For behavior when upgrading to these versions, see <<arp-upgrade-considerations,ARP version-specific upgrade considerations>>.

^2^ Refer to <<arp-upgrade-considerations,ARP version-specific upgrade considerations>> for information on ARP default enablement behavior after upgrading to ONTAP 9.18.1.

^3^ Beginning with ONTAP 9.19.1, ARP/AI supports primary volumes in SnapMirror synchronous relationships for NAS and SAN on FAS, AFF A-series, AFF C-series, and AFX systems, and primary volumes in SnapMirror active sync SAN relationships on AFF A-series, AFF C-series, and ASA r2 systems. For more information, see <<snapmirror>>.

== Interoperability and VM considerations

ARP operates alongside SnapMirror data protection relationships and virtual machine environments, with some version-specific behavior and limitations you should be aware of before deploying.
...
* ARP/AI analytics run on the active, read-write volume in SnapMirror synchronous and SnapMirror active sync SAN relationships. ARP creates snapshots on the primary volume and performs machine-learning-based ransomware detection. Upon failover, the former secondary becomes the active primary, and ARP analytics continue automatically.
* ARP configuration state is automatically replicated to the secondary volume as part of SnapMirror synchronous or SnapMirror active sync initialization in ONTAP 9.19.1.
* ARP snapshots are not replicated to the secondary volume in SnapMirror synchronous or SnapMirror active sync relationships.

NOTE: Beginning with ONTAP 9.19.1, ARP configuration state is automatically replicated from the primary to the secondary volume during SnapMirror synchronous or SnapMirror active sync initialization. Upon failover, ARP detection activates automatically on the new primary without requiring manual re-enablement. Because every write operation is committed to both volumes in a synchronous mirror relationship, enabling ARP on the primary volume is sufficient to detect anomalies on both sides of the relationship.

If a SnapMirror _asynchronous_ source volume is ARP-enabled, the SnapMirror destination volume automatically acquires the ARP configuration state (such as `dry-run` or `enabled`), ARP training data, and ARP-created snapshot of the source volume. No explicit enablement is required.

Although the asynchronous destination volume consists of read-only (RO) snapshots, no ARP processing is done on its data. However, when the SnapMirror asynchronous destination volume is converted to read-write (RW), ARP is automatically enabled on the RW-converted destination volume. The destination volume does not require any additional learning procedures besides what is already recorded on the source volume.
...
* SnapMirror active sync for NAS workloads
...
When upgrading ONTAP clusters that already use ARP, you should be aware of ARP-specific behavior changes that result from an upgrade, in addition to link:../upgrade/special-considerations.html[ONTAP special considerations you must resolve].

=== ARP default enablement after upgrading to ONTAP 9.18.1

Beginning with ONTAP 9.18.1, ARP/AI default enablement behavior is available for AFF A-series and AFF C-series, ASA, and ASA r2 systems. After an upgrade, ARP/AI is automatically enabled on all new volumes after a 12-hour grace period. You must link:index.html#manual-default-enablement[enable ARP manually] on existing volumes.

=== SnapMirror relationships after upgrading to ONTAP 9.19.1

// TODO: Verify with engineering whether this brief out-of-sync behavior still occurs in 9.19.1 GA when enabling ARP on SMS/SMAS volumes after upgrade
If you enable ARP on volumes that are part of SnapMirror synchronous or SnapMirror active sync relationships after the upgrade, the relationship briefly moves to an out-of-sync state. This condition is temporary, and the relationship should automatically resynchronize without intervention.

=== FlexGroup volumes after upgrade

ONTAP 9.16.1 and 9.17.1 do not provide ARP/AI support for FlexGroup volumes. After an upgrade to these versions, FlexGroup volumes enabled for ARP continue to operate with the same ARP model used prior to ARP/AI. Beginning in ONTAP 9.18.1, FlexGroup volumes use the ARP/AI model.
...
// 2026-June-22, CONTAP-688240
// 2026-6-5,contap-719516
// 2026-June-1, ONTAPDOC-3918
// 2026-Apr-3, CONTAP-671820
