...
ARP is available for NAS NFS and SMB FlexVol volumes in on-premises ONTAP systems beginning with ONTAP 9.10.1. Beginning in 9.17.1, ARP is available for SAN FlexVol volumes for both iSCSI, FC, and NVMe with SAN storage.
...
For SAN, entropy statistics are focused at the volume level and detections occur when an entropy anomaly or file anomaly is detected.
