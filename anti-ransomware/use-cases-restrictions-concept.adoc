...
//* ARP/AI is _not_ supported with SnapMirror active sync for NAS in ONTAP 9.19.1 RC. SM-as NAS not supported until GA)
* ARP/AI analytics run only on the active, read-write primary volume in SnapMirror synchronous and SnapMirror active sync SAN relationships, where ARP creates primary-only snapshots and performs machine-learning-based ransomware detection.
* ARP configuration state and ARP snapshots are not replicated to the secondary volume as part of SnapMirror synchronous or SnapMirror active sync in ONTAP 9.19.1.

NOTE: Because every write operation is committed to both the volumes in a synchronous mirror relationship, enabling ARP on the primary volume only is sufficient to detect the anomalies. Internal metadata generated by ARP analytics on the primary volume is replicated to the secondary volume as well. Because of this, if ARP is enabled on the secondary volume after a failover, the required metadata will already exist on the secondary volume.
...
//* SnapMirror active sync (NAS) in ONTAP 9.19.1 (add back in GA, SM-as NAS not yet supported)
