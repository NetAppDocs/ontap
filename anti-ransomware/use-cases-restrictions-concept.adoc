...
| Volumes protected with Asynchronous SnapMirror | X | X | X | | 
| SVMs protected with Asynchronous SnapMirror | X | X | X | | 
| SVMs enabled for data migration | X | X | X | | 
| FlexGroup volumes | X | X | | | 
| Multi-admin verification | X | X | | | 
|===
...
Beginning with ONTAP 9.14.1, ARP offers alerts for the creation of an ARP Snapshot and for the observation of a new file extension. Alerts for these events are disabled by default. Alerts can be set at the volume or SVM level. To monitor these notifications with MAV, create rules for `security anti-ransomware volume event-log modify` for volumes or `security anti-ransomware vserver event-log modify` for SVMs. 
