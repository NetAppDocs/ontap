...
== Enable multi-admin verification with APR-protected volumes

Beginning with ONTAP 9.13.1, you can enable multi-admin verification (MAV) for additional security with APR. MAV ensures that only two or more authenticated administrators can turn off or pause APR on a protected volume or workflow. Learn how to link:../multi-admin-verify/enable-disable-task.html#system-manager-procedure[enable MAV for APR-protected volumes and workflows].
