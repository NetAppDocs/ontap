...
* ARP/AI analytics run on the active, read-write volume in SnapMirror synchronous and SnapMirror active sync SAN relationships. ARP creates snapshots on the primary volume and performs machine-learning-based ransomware detection.
* ARP snapshots are not replicated to the secondary volume in SnapMirror synchronous or SnapMirror active sync relationships. You need to link:enable-task.html[enable ARP again] on the recovery volume after failover.
