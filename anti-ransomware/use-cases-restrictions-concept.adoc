...
^3^ Beginning with ONTAP 9.19.1 RC, ARP is supported on primary volumes in SnapMirror synchronous (SMS) relationships for NAS and SAN as well as primary volumes in SnapMirror active sync (SMAS) relationships for SAN. For more information, see <<snapmirror>>.
...
Beginning with ONTAP 9.19.1 RC, ARP can protect primary volumes that participate in SnapMirror synchronous (SMS) and SnapMirror active sync (SMAS) relationships, with the following behavior:

* ARP is supported on _primary_ volumes in SnapMirror synchronous relationships for NAS and SAN.
* ARP is supported on _primary_ volumes in SnapMirror active sync relationships for SAN.
* ARP is _not_ supported with SnapMirror active sync for NAS (SMAS-NAS) in ONTAP 9.19.1 RC.
* ARP analytics and ARP snapshot creation run only on the primary volume in SMS and SMAS-SAN relationships.
