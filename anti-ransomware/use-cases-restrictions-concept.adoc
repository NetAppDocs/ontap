...
Beginning with ONTAP 9.14.1, ARP offers alerts for the creation of an ARP Snapshot and for the observation of a new file extension. These alerts are not enabled by default, and can be set at the volume or SVM level. To control these alerts with MAV, create rules for `security anti-ransomware volume event-log modify` for volumes or `security anti-ransomware vserver event-log modify` for SVMs. 
