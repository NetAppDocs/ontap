...
^3^ Beginning with ONTAP 9.19.1 RC, ARP/AI supports primary volumes in SnapMirror synchronous (SMS) relationships for NAS and SAN on FAS, AFF A-series, AFF C-series, and AFX systems, and primary volumes in SnapMirror active sync (SMAS) SAN relationships on AFF A-series, AFF C-series, and ASA r2 systems. For more information, see <<snapmirror>>.
...
Beginning with ONTAP 9.19.1 RC, ARP/AI supports primary volumes that participate in SnapMirror synchronous (SMS) and SnapMirror active sync (SMAS) SAN relationships, with the following behavior:

* ARP/AI is supported on _primary_ volumes in SnapMirror synchronous (SMS) relationships for NAS and SAN on FAS, AFF A-series, AFF C-series, and AFX systems.
* ARP/AI is supported on _primary_ volumes in SnapMirror active sync (SMAS) SAN relationships on AFF A-series, AFF C-series, and ASA r2 systems.
* ARP/AI is _not_ supported with SnapMirror active sync (SMAS) for NAS in ONTAP 9.19.1 RC.
* ARP/AI analytics run only on the active, read-write primary volume in SMS and SMAS-SAN relationships, where ARP creates primary-only snapshots and performs machine-learning-based ransomware detection.
* ARP configuration state and ARP snapshots are not replicated to the secondary volume as part of SMS or SMAS in ONTAP 9.19.1 RC.
