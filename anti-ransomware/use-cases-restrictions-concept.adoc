...
+
For environments without ARP/AI, users could create files with extensions that aren't detected in the learning period. Because of this, there a is greater possibility of false positives in this workload.
* Images and video
+
...
ARP is available for NAS NFS and SMB FlexVol volumes in on-premises ONTAP systems beginning with ONTAP 9.10.1. Beginning in 9.17.1, ARP is also available for SAN FlexVol volumes with LUNs or VDMKs.
...
ARP can detect file extension changes on an NFS volume outside of the VM if a new extension enters the volume encrypted or a file extension changes.
