...
[.lead]
To keep up-to-date protection against the latest ransomware threats, ARP/AI (available beginning with ONTAP 9.16.1) offers automatic updates that occur outside of regular ONTAP release cadences. If you have link:../update/enable-automatic-updates-task.html [enabled automatic system and firmware updates] then you will also be able to start receiving automatic updates to ARP/AI. You can also elect to install ARP/AI update security bundles manually as described in this procedure.

The default method of updating ARP/AI is link:../update/index.html[automatic updates]. Beginning with ONTAP 9.16.1, security updates for ARP/AI are available in System Manager software downloads in addition to system and firmware bundles. If your ONTAP cluster is already enrolled in automatic firmware and system updates, you don't need to do anything else: ARP/AI security updates will also be automatically downloaded and installed.

If you want to manually update ARP/AI, you can choose to be notified when new updates arrive. Manual updates are available from the NetApp Support Site. These can be downloaded and installed using System Manager.
