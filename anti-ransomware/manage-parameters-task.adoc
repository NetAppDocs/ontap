...
. Navigate to **Volumes** then select the individual volume for which you want to modify settings. 
...
. Navigate to **Storage VM** then select the SVM you want to enable settings for. 
. Under the **Security** heading, locate the **Anti-ransomware** card. Select image:icon_kabob.gif[make a choice] then **Edit Ransomware Event Severity**.
. To receive notifications for **New file extension detected** and **Ransomware snapshot created**, select the dropdown menu under the **Severity** heading. Modify the setting from **Don't generate event** to **Notice**. 
. Select **Save**. 
--
...
+
`security anti-ransomware volume event-log modify -is-enabled-on-new-file-extension-seen true`
* To set alerts for the creation of an ARP Snapshot:
+
`security anti-ransomware volume event-log modify -is-enabled-on-snapshot-creation true`
...
* To set alerts for a new file-extension:
+
`security anti-ransomware vserver _svm_name_ event-log modify -is-enabled-on-new-file-extension-seen true`
* To set alerts for the creation of an ARP Snapshot:
+
`security anti-ransomware vserver event-log modify -is-enabled-on-snapshot-creation true`
