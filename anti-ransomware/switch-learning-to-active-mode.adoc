...
[.lead]
For NAS environments, manually or automatically switch an ARP-enabled volume from learning mode to active mode. You'll need to switch modes if you are using ARP with ONTAP 9.15.1 and earlier or if your ARP is running on FlexGroup volumes with ONTAP 9.17.1 and earlier.
