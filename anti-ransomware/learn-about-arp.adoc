...
summary: For ONTAP 9.10.1 through 9.15.1, the original ARP model uses a learning period to establish behavioral baselines before switching to active ransomware detection in NAS environments.
---

= Learn about ARP (ONTAP 9.10.1 to 9.15.1)
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
For ONTAP 9.10.1 through 9.15.1, Autonomous Ransomware Protection (ARP) monitors NAS environments using a learning period to establish behavioral baselines before switching to active ransomware detection. ARP analyzes data entropy, file extension changes, and file IOPS patterns to identify potential ransomware attacks.

NOTE: The original ARP model received no new functional improvements after ONTAP 9.14.1. If you are running ONTAP 9.16.1 or later, see link:learn-about-arpai.html[Learn about ARP/AI].
...
ARP creates locked snapshots when early signs of an attack are detected. You'll need to first confirm whether the attack is real or a false positive. If you confirm the attack, the volume can be restored using the ARP snapshot.
