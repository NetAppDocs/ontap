...
keywords: ransomware, arp/ai, autonomous protection, ai, machine learning, security, detect, detection, threat, attack, anti-ransomware, entropy, snapshot, NAS, SAN, automatic updates, evaluation period, current ARP model, no learning period ransomware ONTAP, immediate active ransomware protection, pre-trained ransomware model ONTAP
summary: Beginning with ONTAP 9.16.1, ARP/AI is the current default ransomware protection model. It uses a pre-trained machine-learning model to provide immediate active protection for NAS and SAN environments with no learning period required.
---
...
[.lead]
ARP/AI is the current ONTAP ransomware protection model for ONTAP 9.16.1 and later. It uses a pre-trained machine-learning model to detect constantly evolving forms of ransomware in NAS and SAN environments, providing immediate active protection with no learning period and supporting automatic security updates independent of ONTAP releases.

ARP/AI is distinct from the earlier ARP model used in ONTAP 9.10.1 through 9.15.1, which required a learning period before active protection. For a side-by-side feature comparison, see link:index.html#arp-ai-vs-legacy-arp-feature-comparison[the ARP/AI and legacy ARP feature comparison].
...
* SAN FlexVol volumes with ONTAP 9.17.1 and later (active immediately, even during the link:entropy-eval-period-arpai.html[evaluation period])

NOTE: For upgrade-specific behavior, such as transitions from earlier ARP functionality, see link:support-arpai.html#arp-upgrade-considerations[Upgrade considerations for existing clusters].
...
Beginning with ONTAP 9.18.1, ARP/AI is enabled by default on all new volumes for AFF A-series, AFF C-series, ASA, and ASA r2 systems. This automatic default enablement does not apply to link:support-arpai.html#unsupported-configurations[unsupported volumes or configurations]. You can link:enable-arpai.html[manually enable ARP/AI on existing volumes].

You can link:default-enable-change-arpai.html[turn default enablement on or off] at any time after deployment.

NOTE: For upgrade-specific enablement behavior, including grace period and SnapMirror version requirements, see link:support-arpai.html#arp-upgrade-considerations[Upgrade considerations for existing clusters].
