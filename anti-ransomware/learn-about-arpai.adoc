...
ARP/AI is distinct from the earlier ARP model used in ONTAP 9.10.1 through 9.15.1, which required a learning period before active protection. For a side-by-side feature comparison, see link:index.html#autonomous-ransomware-protection-with-ai-arp-ai-and-legacy-arp-feature-comparison[the ARP/AI and legacy ARP feature comparison].
...
ARP/AI creates a snapshot when early signs of an attack are detected. An analysis is then conducted to confirm or dismiss the potential attack. Beginning with ONTAP 9.17.1, ARP/AI also generates snapshots at regular 4-hour intervals; the presence of these snapshots should not be regarded as an anomaly. If an attack is confirmed, the attack probability is set to `Moderate` and an attack notification is generated.
