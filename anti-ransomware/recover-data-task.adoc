...
[NOTE]
----
This only applies when you are restoring using the `volume snap restore` command as outlined below.  If you are restoring data using Flex Clone, Single File Snap Restore or other methods still works as expected with automatic anti-ransomware snapshot backups.
----
+
[cols=2, options="header"]
|===
