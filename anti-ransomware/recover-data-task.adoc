...
To restore from a Snapshot other than the `Anti_ransomware_backup` Snapshot after a system attack was identified, you must first release the ARP Snapshot. If no system attack was reported, you must first restore from the `Anti_ransomware_backup` Snapshot then complete a subsequent restoration of the volume from the Snapshot of your choosing. 
...
.If ARP reported a system attack 
...
.If no system attack was reported
. Display the Snapshot copies in volumes:
+
Select *Storage > Volumes*, then select the volume and *Snapshot Copies*.
. Select image:icon_kabob.gif[Menu Option] them choose the `Anti_ransomware_backup` Snapshot. 
. Select *Restore*.
...
. To restore from the ARP, skip to step two. To restore data from earlier Snapshot copies, you must release the lock on the ARP Snapshot.
+
...
. If a system attack was not identified, you must first restore the volume using the ARP Snapshot. 
include::../_include/restore-snapshots-cli.adoc[]
. Repeat steps 2 and 3 using the desired Snapshot. 
--
====
