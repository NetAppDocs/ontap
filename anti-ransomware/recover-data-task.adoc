...
== Restore after a system attack was identified
...
.System Manager
--
. If you want to restore data from earlier Snapshot copies, instead of from the ARP copies, you must perform the following steps to release the anti-ransomware Snapshot lock.
+
If you want to restore from the ARP copies, it is not necessary to release the lock and you can skip these steps.
...
. If you want to restore data from earlier Snapshot copies, instead of from the ARP copies, you must perform the following steps to release the anti-ransomware Snapshot lock.
+
f you want to restore from the ARP copies, it is not necessary to release the lock and you can skip this step.
+
...
.Related information
https://docs.netapp.com/us-en/ontap/anti-ransomware/respond-abnormal-task.html[Respond to abnormal activity reported by Autonomous Ransomware Protection]

== Restore after ARP copies were generated, but no system attack was identified
 
In the event that no system attack was identified and you want to restore data from earlier Snapshot copies instead of from the ARP copies, you must restore from the ARP copies first. Then repeat this process to restore from the earlier Snapshot copies.  Restoring from the ARP copies first releases the anti-ransomware Snapshot lock.  

If you want to restore from the ARP copies, you do not need to take any steps to release the anti-ransomware Snapshot lock.
