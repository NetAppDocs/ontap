...
* <<restore-without-attack,Restore when a system attack is not identified>>
...
.CLI
--
. If you are restoring to a non-ARP snapshot using `volume snapshot restore`, mark the attack as potential ransomware (`-false-positive false`) and clear suspect files to release the lock:
+
[source,cli]
----
