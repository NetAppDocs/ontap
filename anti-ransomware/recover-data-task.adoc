...
. If you want to restore data from earlier Snapshot copies, instead of from the ARP copies, you must perform the following steps to release the anti-ransomware Snapshot lock.
+
If you want to restore from the ARP copies, it is not necessary to release the lock and you can skip this step.
+
...
In the event that no system attack was identified and you want to restore data from earlier Snapshot copies instead of from the ARP copies, you must restore from the ARP copies first. Restoring from the ARP copies first releases the anti-ransomware Snapshot lock.  Then you can repeat this process to restore from the earlier Snapshot copies.  
