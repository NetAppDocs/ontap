...
[.lead]
Autonomous Ransomware Protection (ARP) creates snapshots when it detects a potential ransomware threat. You can use one of these ARP snapshots or another snapshot of your volume to restore data. 

.About this task
The ARP snapshot creates snapshots with one of the following prepended names:
* `Anti_ransomware_backup`: Used in ONTAP 9.16.1 and earlier (Example: `Anti_ransomware_backup.2022-12-20_1248`).
* Anti_ransomware_periodic_backup": Used in ONTAP 9.17.1 and later (Example: `Anti_ransomware_periodic_backup.2025-06-01_1248`).
...
+
Select *Storage > Volumes*, then select the volume and *Snapshot Copies*.
. Select image:icon_kabob.gif[Menu options icon] them choose the `Anti_ransomware` snapshot. 
. Select *Restore*.
