...
If the ARP-protected volume is part of a SnapMirror synchronous (SMS) or SnapMirror active sync (SMAS) relationship in ONTAP 9.19.1 RC, additional recovery considerations apply:

* ARP snapshots are created and retained only on the primary volume. They are not replicated to the secondary volume as part of SMS or SMAS.
...
* Volume-level restore (`volume snapshot restore`) on SMS or SMAS-SAN volumes might require that you temporarily quiesce or break the SMS or SMAS relationship.
* In many cases, you can avoid breaking SMS or SMAS by using FlexClone or file-level restore operations from ARP or other snapshots on the primary volume.
====
...
+
If the volume is part of an SMS or SMAS-SAN relationship in ONTAP 9.19.1 RC and you plan to perform a volume-level restore, follow SMS or SMAS documentation to quiesce or break the protection relationship before starting the restore, then reestablish protection after the restore is complete.
...
. If the volume is part of an SMS or SMAS-SAN relationship in ONTAP 9.19.1 RC and you intend to perform a volume-level restore with `volume snapshot restore`, quiesce or break the SnapMirror relationship according to SMS or SMAS documentation before running the restore operation.
