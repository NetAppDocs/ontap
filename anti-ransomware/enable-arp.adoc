...
ARP for NAS workloads (ONTAP 9.10.1 to 9.15.1) starts in link:index.html#learn-about-arp-modes[learning mode] (`dry-run` state). The system analyzes the workload to characterize normal behavior before switching to active mode for threat detection. Beginning in active mode before the learning period completes can lead to excessive false positive reports.
