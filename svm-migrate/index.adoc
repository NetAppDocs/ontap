...
// [NOTE]
// ====
//If you are migrating from one cluster type to another (from a FAS cluster to an AFF cluster, for example), the maximum number of volumes supported is based on the HA pair that is hosting the source SVM.
// ====
