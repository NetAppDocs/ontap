...
summary: 'Beginning with ONTAP 9.10.1, cluster administrators can non-disruptively relocate an SVM from a source cluster to a destination cluster to manage capacity and load balancing, or to enable equipment upgrades or data center consolidations. Beginning with ONTAP 9.12.1, this feature is supported on FAS and AFF systems and on hybrid aggregates.'
---
= Learn about ONTAP SVM data mobility
:icons: font
:imagesdir: ../media/


[.lead]
Beginning with ONTAP 9.10.1, cluster administrators can non-disruptively relocate an SVM from a source cluster to a destination cluster to manage capacity and load balancing, or to enable equipment upgrades or data center consolidations. 

Non-disruptive SVM relocation is supported on AFF systems in ONTAP 9.10.1 and 9.11.1. Beginning with ONTAP 9.12.1, this capability is supported on both FAS and AFF systems and on hybrid aggregates.
...
Beginning with ONTAP 9.18.1, SVM migration is supported from ASA storage systems to ASA r2 storage systems. If you have an ASA r2 storage system (ASA A1K, ASA A90, ASA A70, ASA A50, ASA A30, ASA A20, or ASA C30) and you would like to migrate an SVM to your ASA r2 system from an ASA system, follow link:https://docs.netapp.com/us-en/asa-r2/manage-data/migrate-svm.html[these steps^].  
...
Follow link:https://docs.netapp.com/us-en/asa-r2/manage-data/migrate-svm.html[these steps^] for SVM migration from ASA to ASA r2 systems.
...
[NOTE]
 ...
 When migrating from an AFF cluster to a FAS cluster with hybrid aggregates, auto volume placement attempts to perform a like-to-like aggregate match. For example, if the source cluster has 60 volumes, the volume placement tries to find an AFF aggregate on the destination to place the volumes. When there is not sufficient space on the AFF aggregates, the volumes are placed on aggregates with non-flash disks.
...
[NOTE]
The maximum number of volumes that you can migrate per SVM in a mixed or hybrid cluster is based on the  cluster member that supports the lower number of volumes.

[cols="1,1,1,1,1,1"]
|===

h| Source h| Destination h| ONTAP 9.14.1 and later h| ONTAP 9.13.1 h| ONTAP 9.12.1 h| ONTAP 9.11.1 and earlier
...
* link:../peering/create-cluster-relationship-93-later-task.html[The source and destination clusters are peered to each other].
* The source and destination clusters have the SnapMirror synchronous link:../system-admin/install-license-task.html[license installed]. This license is included with link:../system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one[ONTAP One].
* All nodes in the source cluster are running ONTAP 9.10.1 or later. For specific ONTAP array controller support, see link:https://hwu.netapp.com/[Hardware Universe^].
* All nodes in the source cluster are running the same ONTAP version.
* All nodes in the destination cluster are running the same ONTAP version.
* The destination cluster ONTAP version is at the same or no more than two major newer versions as the source cluster.
* The source and destination clusters support the same IP subnet for data LIF access.
* Both the source and destination clusters must have at least one interface that has access to all the migrating SVM's networks, otherwise the migration precheck will fail.
* The source SVM contains fewer than the xref:Maximum supported volumes per SVM[maximum number of supported data volumes for the release]. 
* Sufficient space for volume placement is available on the destination.
* The Onboard Key Manager or external key management is configured at the cluster level on the destination if the source SVM has encrypted volumes.
** In this case, key managers configured at the SVM level on the source do not migrate to the destination. The destination uses the cluster-level key manager.
...
+
NOTE: Migrating an SVM between the local and partner cluster in a MetroCluster configuration is not supported.
+
...
Check for operations that can conflict with an SVM migration:
...
* No SVM migration is running on the destination cluster. Only one SVM migration is allowed at any given time.
 ...
 * link:../nas-audit/enable-disable-auditing-svms-task.html[Audit log must be enabled on the destination cluster].
...
| Cloud instances
| Not supported
a| Migrating SVMs to or from on-prem instances to the cloud is not supported.
...
* Migrating an SVM between the local and partner cluster in the same MetroCluster configuration.

See the <<Prerequisites,prerequisites>> to migrate an SVM in a MetroCluster configuration. 

| NetApp Aggregate Encryption (NAE)
| ONTAP 9.11.1
| NAE volumes must be placed on NAE supporting destination. If no NAE destination is available, the migration operation fails.
...
| ONTAP 9.10.1
| NVE volumes are migrated as NVE volumes on the destination.
...
| ONTAP Select
| Not supported
| Migrating SVMs to or from ONTAP Select instances is not supported.
...
| SMB protocol
| ONTAP 9.12.1
However, SMB1 protocol is not supported. 
a| 
SMB migrations are disruptive and require a client refresh after the migration.
...
* Any ongoing transfers are canceled during cutover and new transfers fail during cutover. They cannot be restarted until the migration completes.
...
| Vscan
| Not supported
| Migration of Vscan-enabled SVMs is not supported.
...
* Migrated vault destination SnapMirror volumes must perform a resync to reactivate protection.
The vault resync is necessary because the migration creates a new baseline between the migration SVMs that is more recent than the vault's previous baseline. Performing a resync reactivates the vault, which will also deletes any data newer than the current vault baseline, specifically the snapshot that was generated during the migration.

// 2025-Jan-9, ONTAPDOC-3591
// 2026-Jan-9, ONTAPDOC-3584
// 2025-Dec-8, ONTAPDOC-3514
// 2025-Dec-5, ONTAPDOC-3514
// 2025-Dec-4, ONTAPDOC-3508
// 2025 Nov 21, GH 1875
// 2025-Nov-24, ONTAPDOC-3462
