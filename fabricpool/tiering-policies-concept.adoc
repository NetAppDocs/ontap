...
---
= ONTAP FabricPool volume tiering policies
:icons: font
:imagesdir: ../media/
...
The following table lists volume tiering policies and the default retrieval behavior for each policy.

|===

h| Tiering policy h| Default retrieval behavior
...
default
a|
When cold blocks in a volume are read, they use the default behavior of their volume tiering policy. This policy is the default value for any volume regardless of the hosted aggregate type.
...
never
a|
When cold blocks in a volume with a cloud retrieval policy set to "`Never"` are read, they remain cold and stay on the cloud tier. They are not written back to the local tier.
...
* For tiering policy "`none,`" all tiered data is written back to the local tier.
* For tiering policy "`snapshot-only,`" tiered, non-snapshot, data is written back to the local tier. 

|===

.Example of modifying the cloud retrieval policy

The following example changes the volume's tiering policy to `none` and the retrieval policy to `promote`:
...
// 2026-July-03, Rewrite of cloud retrieval section
