...
When you need to make changes to your network that includes FQDN updates, you should change the ONTAP S3 FQDN that is used for FabricPool tiering to avoid network disruptions.

.Steps

. Turn off certificate verification for the FabricPool tier:
+
`storage aggregate object-store config modify -object-store-name <object store> -is-certificate-validation-enabled false`
. Change the FQDN for the ONTAP S3 server:
+
`vserver object-store-server modify -vserver <vserver> -object-store-server <new fqdn>`
. (Optional) If certificate validation is needed and the hostname was changed, link:../s3-config/create-install-ca-certificate-svm-task.html[renew the certificate].
. Update the FabricPool tier with the new FQDN:
+
`storage aggregate object-store config modify -object-store-name <object_store_name> -server <new fqdn>`
. Verify that all object stores, and their mirrors if any, are available in the FabricPool tier:
+
`storage aggregate object-store show -fields object-store-availability,mirror-type,is-mirror-degraded`
+
For more information, see link:https://docs.netapp.com/us-en/ontap-cli/storage-aggregate-object-store-show.html#description[storage aggregate object-store show^].
. Repeat these steps on any additional object stores.
. If you renewed certificates on the ONTAP S3 server and renewed certificates on the FabricPool tier, reenable certificate verification:
+
...
// 2026-July-28, GitHub issue #2015
