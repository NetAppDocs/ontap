...
Beginning with ONTAP 9.8, when a volume has link:determine-data-inactive-reporting-task.html[inactive data reporting] enabled, FabricPool will use the volume's heat map to immediately queue cold data to begin tiering as soon as it is written to the destination aggregate. 
...
Beginning with ONTAP 9.6, if a volume move's destination local tier uses the same bucket as the source local tier, data on the source volume that is stored in the bucket does not move back to the local tier. Tiered data stays at rest and only hot data needs to be moved from one aggregate to another. This optimized volume move results in significant network efficiencies.

If a volume move is not optimized, the additional traffic (up via reads/GETs, and down via writes/PUTs) places additional network and compute demands on both the ONTAP cluster and the object store and can result in additional costs when tiering to public object stores.
...
* Source and destination aggregates using different encryption keys 
* FlexClone volumes 
...
=== Volume metadata
Unrelated to a volume move being optimized or not, ONTAP stores information about where all data are located, including local and tiered data. When a volume is moved from one aggregate to another, this information needs to be moved to the destination aggregate as well. This metadata always stays on the local tier, but depending on the amount of tiered data this can cause the volume move process to take more time to take then expected to finish.

For example, a 300TB optimized volume move means that 300TB of cold data doesn't need to move from one aggregate to another (or trigger 300TB of reads and 300TB of writes to the object store). But it does mean that the metadata associated with 300TB of data (~15TB of data in this example) will need to be written to the destination aggregate.
