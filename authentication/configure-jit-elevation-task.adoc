...
* JIT privilege elevation is only supported for users using password, nsswitch, or domain authentication to log in. Multifactor authentication (MFA) is not supported for JIT privilege elevation.
