...
Cluster administrators can set up JIT access for ONTAP users, and configure the default JIT session validity period and maximum JIT validity period duration globally and for specific SVMs.
...
* The default `max-jit-validity-period` is 90 days. This setting determines the maximum period during which a user can access JIT elevation after the configured start date. You can configure the JIT validity period for individual users, but it cannot exceed the maximum JIT validity period.
