...
`-vserver`
a|
The Vserver to which the Duo authentication settings apply. 
a|
...
`-enabled`
a|
Enable Duo multi-factor authentication. Set to `true` by default. When enabled, Duo multi-factor authentication is enforced during SSH login according to the configured parameters. When Duo is disabled (set to `false`), Duo authentication is ignored.
