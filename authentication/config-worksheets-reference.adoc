...
`-enabled`
a|
Enable Duo multi-factor authentication. Set to `true` by default. When enabled, Duo multi-factor authentication is enforced during SSH login according to the configured parameters. When Duo disabled, Duo authentication is ignored.
