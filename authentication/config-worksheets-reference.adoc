...
You provide these values with the `security login domain-tunnel create` command when you have already configured a CIFS server for a data SVM and you want to configure the SVM as a gateway or _tunnel_ for Active Directory domain controller access to the cluster.
...
`-vserver`
a|
The name of the SVM for which the CIFS server has been configured.
a|
...
[NOTE]
====
Starting with ONTAP 9.2, the `-ldap-servers` field replaces the `-servers` field. This new field can take either a host name or an IP address as the value for the LDAP server.
====
...
[NOTE]
====
Starting with ONTAP 9.2, the `-nis-servers` field replaces the `-servers` field. This new field can take either a host name or an IP address as the value for the NIS server.
====
