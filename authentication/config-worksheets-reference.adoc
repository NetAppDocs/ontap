...
You provide these values with the `security login duo create` command when you enable Cisco Duo multi-factor authentication with SSH logins for a Vserver.
...
`-vserver`
a|
The Vserver to which the Duo Security settings apply. 
a|
...
`-fail-mode`
a|
On service or configuration errors that prevent Duo authentication, fail "safe" (allow access) or "secure" (deny access). The default is "safe", which means that Duo Security authentication is bypassed if it fails due to errors such as the Duo Security API server being inaccessible. 
...
`-max-prompts`
a|
If a user fails to authenticate with a second factor, Duo Unix will prompt the user to authenticate again. This option sets the maximum number of prompts that Duo Unix will display before denying access. Must be 1, 2, or 3. The default value is 1.

For example, when `max-prompts = 1`, the user will have to successfully authenticate on the first prompt, whereas if `max-prompts = 2`, if the user enters incorrect information at the initial prompt, he/she will be prompted to authenticate again.
...
// 
// The maximum number of times that an unenrolled user is permitted to bypass Duo Security and use only the primary authentication method to login. For example, if max-unenrolled-logins=3, then ONTAP will allow the user to bypass Duo Security authentication and login via SSH up to 3 times using only the primary authentication method. SSH login will be disallowed at the 4th attempt if the user is still not yet enrolled in Duo Security.
//
// By default, max-unenrolled-logins=-1, which means that Duo Security authentication is bypassed for unenrolled users, and that there is no limit to the number of times an unenrolled user can log in using only the primary authentication method.

// If set to 0, this means that the user cannot login without enrolling with Duo Security.
a|

a|
`-enabled`
a|
Enable Duo Security. Set to `true` by default. When enabled, Duo Security authentication is enforced during SSH login according to the configured parameters. When Duo Security is disabled, Duo authentication is ignored.
