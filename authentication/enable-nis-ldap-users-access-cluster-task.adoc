...
+
[NOTE]
The field `-ldap-servers` replaces the field `-servers`. This field can take either a hostname or an IP address for the LDAP server.
...
----
+
The name service check command is available.
