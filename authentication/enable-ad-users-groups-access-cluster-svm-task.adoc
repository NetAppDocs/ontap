...
In ONTAP 9.10.1 through 9.13.1, the default setting for this command is `false`.
In ONTAP 9.14.1 and later, AES session keys are used during Netlogon secure channel establishment by default (the default setting is `true`).
