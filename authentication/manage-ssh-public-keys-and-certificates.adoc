...
[.lead]
For increased SSH authentication security with administrator accounts, you can use the `security login publickey` set of commands to manage the SSH public key and its association with X.509 certificates. Learn more about `security login publickey` in the link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=security+login+publickey[ONTAP command reference^].
