...
You can define an authorization server to ONTAP using any of the administrative interfaces, including the ONTAP CLI, System Manager, or ONTAP REST API. For example, with the ONTAP CLI you use the command `security oauth2 client create`.
...
The OAuth 2.0 access tokens issued by the authorization servers are verified by ONTAP and used to make role-based access decisions for ONTAP REST API client requests.

=== Acquiring an access token

You need to acquire an access token from an authorization server defined to the ONTAP cluster where you use the ONTAP REST API. To acquire a token, you must contact the authorization server directly.
