...
| xref:../flexgroup/supported-unsupported-config-concept.html[snapshot tagging in FlexGroup volumes] | You can add, modify, and delete tags and labels (comments) in to help identify Snapshot copies and to help avoid accidentally deleting Snapshot copies in FlexGroup volumes.
