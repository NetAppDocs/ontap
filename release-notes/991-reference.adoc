...
| link:../nfs-admin/ldaps-concept.html[LDAP support on any port] | LDAPS can be configured on any port, although port 636 remains the default.
...
|===
| Update  | Description 
a| link:../networking/network_features_by_release.html[Cluster resiliency]
a| 
...
| xref:../san-migration/task_checking_supported_configurations_for_fli_using_san_lun_migrate_app.html[Foreign LUN import (FLI)] | The SAN LUN Migrate App on the NetApp Support Site can be used to qualify a foreign array that is not listed in the FLI interoperability matrix. 
...
| xref:../task_asa_software_configuration.html#asa-limitations-and-restrictions[Support for 12-node clusters on ASAs] | 12-node clusters are supported for AFF ASA configurations. ASA clusters can include a mix of various ASA system types.
| xref:../task_asa_software_configuration.html#asa-limitations-and-restrictions[NVMe-oF protocol on ASAs] | The NVMe-oF protocol support is also available with an AFF ASA system.
...
a| You can configure Cisco DUO as a SAML identity provider (IdP), enabling users to authenticate using Cisco DUO when they log in to System Manager.
+
[NOTE]
Support begins in ONTAP 9.9.1P13
|===
...
| link:https://docs.netapp.com/us-en/ontap-cli-991/volume-modify.html[Set number of files to maximum for volume^] | Set number of files to maximum for volume | Automate file maximums with the volume parameter `-files-set-maximum`, eliminating the need to monitor file limits. 
...
| [SMB access support in qtress] | SMB access is supported to qtrees in FlexVol and FlexGroup volumes with SMB enabled.
|===
...
[NOTE]
Support begins in ONTAP 9.9.1P13
| xref:../task_admin_monitor_risks.html[System Manager displays risks reported by Active IQ] | SMB access is supported to qtrees in FlexVol and FlexGroup volumes with SMB enabled.
...
| xref:../concept_admin_viewing_managing_network.html[Enhanced network port management] | The network interfaces page has enhanced capabilities to display and manage which interfaces are on their home ports.
...
a| 
* xref:../concept_admin_viewing_managing_network.html[A new **Tiers** column names the local tiers (aggregates) in which each volume resides.]
* xref:../concept_capacity_measurements_in_sm.htmll[System Manager shows the physical used capacity along with the logical used capacity at the cluster level as well as the local tier (aggregate) level.]
