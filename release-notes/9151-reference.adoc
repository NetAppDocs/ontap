...
a|
xref:../consistency-groups/limits-reference.html[Increased limit for volumes in a consistency group using SnapMirror asynchronous] 
a|
...
| Update | Description 
a|
xref:../install-ip/task-configure-end-to-end-encryption.html#enable-end-to-end-encryption[MetroCluster IP support for end-to-end encryption] 
a|
...
| Update | Description 
a|
xref:../nfs-rdma/[Support for NFSv3 with RDMA]
a|
...
a|
xref:../nas-audit/plan-fpolicy-external-engine-config-concept.html[Protobuf is Google’s language-neutral mechanism for serializing structured data. It is smaller, faster, and simpler compared to XML, which helps improve FPolicy performance] 
a|
...
a|
You can create the FPolicy persistent store and automate its volume creation and configuration at the same time using the `fpolicy persistent-store create` command. 
...
| Update | Description 
a|
xref:../san-admin/san-config-mcc-concept.html[MetroCluster IP support for NVMe] 
a|
The NVMe/TCP protocol is supported on four-node MetroCluster IP configurations.  
|===
...
| Update | Description 
a|
xref:../authentication/dynamic-authorization-overview.html[ONTAP now supports Dynamic Authorization for SSH connections] 
a|
...
a|
ONTAP now supports TLS 1.3 for S3 storage, FlexCache, and Cluster Peering encryption 
a| 
...
a|xref:../nfs-admin/tls-nfs-strong-security-concept.html[TLS is now supported for NFS connections] 
a|
...
a|xref:../multi-admin-verify/index.html#rule-protected-commands[Eligible set of rule-protected commands extended for multi-admin verification] 
a|
Administrators can create multi-admin verification rules to protect cluster configuration, LUN deletion, system configuration, security configuration for IPsec and SAML, volume snapshot creation, vServer configuration, and other.
a|
Delivery of AutoSupport messages using SMTP with TLS
a|While the recommended transport of AutoSupport messages to NetApp is HTTPS, unencrypted SMTP has also been available. With ONTAP 9.15.1, customers now have the option of using TLS with SMTP. The SMTPS protocol establishes a secure transport channel by encrypting the email traffic as well as the optional email server credentials. Explicit TLS is used and so TLS is activated after the TCP connection is created. If copies of the messages are sent to local email addresses, the same configuration is used.
|===
...
| Update | Description 
a|
xref:../task_cp_dashboard_tour.html[Performanace enhancements for the  System Manager dashboard] 
a|
