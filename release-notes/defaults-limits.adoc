...
| NAS audit
| The limits for `file-session-io-grouping-count` and `file-session-io-grouping-duration` parameters increase so that you can optionally select fewer, more consolidated NAS audit event notifications. This benefits SVMs with high rates of IO, reducing the storage impact on the destination volume.
