...
| ARP/AI is automatically enabled by default on all new volumes after a 12-hour grace period on supported platforms.
| ONTAP 9.18.1
...
//ONTAPDOC-4219
| Time-to-live (TTL) for cached positive credentials for ONTAP SVMs
| Beginning with ONTAP 9.17.1, the time-to-live (TTL) for cached positive credentials for ONTAP SVMs has decreased from 24 hours (86,400,000 milliseconds) to 1 hour (3,600,000 milliseconds). 
| ONTAP 9.17.1
...
| SnapMirror cloud
| SnapMirror cloud increases the number of S3 buckets supported from 6 to 100.
| ONTAP 9.19.1

| S3 connection flow control limit
| The flow control limit for ONTAP S3 connections have decreased from 2 MB to 256 KB for cloud instances that have <= 32GB of memory.
| ONTAP 9.19.1 
//CONTAP-655662

| S3 idle connection timeout
| S3 idle connection timeout has been reduced from 20 minutes to 5 minutes.
| ONTAP 9.19.1 
//CONTAP-630301
...
| Cluster peering
| Increased supported cluster peer limit from 255 to 400, improving scalability for environments with large numbers of cluster peer relationships.
| ONTAP 9.14.1
...
* The maximum number of constituents in a FlexGroup cannot exceed 24.
* SVM-DR volume limits are 500 per node, 1000 per HA pair (including FlexVol volumes and FlexGroup constituents).
[NOTE]
Limits are platform-specific. You should verify the correct limits for your configuration by going to link:https://hwu.netapp.com[NetApp Hardware Universe^] and selecting your platform and ONTAP version. 

// 21-Sept-2026, ONTAPDOC-4348
...
| The maximum number of asynchronous volume-level concurrent transfers has increased from 100 to 200. SnapMirror Cloud transfers have increased from 32 to 100 on high-end systems and from 6 to 20 SnapMirror transfers on low-end systems.
| ONTAP 9.8
