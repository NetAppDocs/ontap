...
+
ONTAP 9.16.1 adds more commands to the MAV framework for additional protection from malicious insiders. These enhancements include many Consistency Group (CG) commands, VScan and Autonomous Ransomware Protection (ARP) management, and NVMe configuration.
...
* xref:../ipsec-prepare.html[IPSec HW offload support for new network cards]
+
ONTAP 9.16.1 offers higher "over-the-wire" encryption performance when utilizing the IPSec hardware offload functionality on offload cards introduced for all the new AFF-A and AFF-C platforms.
...
+
NetApp FlexGroup volumes can optionally stripe file data across multiple back-end constituent volumes for increases in performance, predictability of performance, reducing performance bottlenecks, and adding consistency in balancing capacity across the backend constituent volumes.
* Improved ONTAP Upgrade Resiliency and Simplicity
+
Upgrading ONTAP software is now more seamless and resilient with the removal of reboot cycles and streamlined interaction during the ANDU process.
