...
* Learn about https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html[new and enhanced ONTAP software features for NetApp AFX storage systems^].
* Learn about https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html[new and enhanced ONTAP software features for NetApp ASA r2 storage systems^].
...
ONTAP 9.19.1 delivers new and enhanced features in the areas of security, scalability, and storage management to support data infrastructure modernization. These enhancements help you scale, operate more efficiently, and secure data for the future. For a complete list of new features and enhancements, see xref:whats-new-9191.adoc[What's new in ONTAP 9.19.1].

* xref:../performance-admin/guarantee-throughput-qos-task.html[Configurable QoS burst policies] 
+
Configurable QoS burst policies allow controlled, time‑limited performance bursts above QoS limits, improving workload responsiveness during peak events without sacrificing fairness or requiring over‑provisioning.
* xref:../nfs-admin/enable-nfs-over-tls-task.html[NFS over TLS 1.3] 
+ 
Secures NFS data in flight with standards-based TLS 1.3 encryption, helping to strengthen protection and simplify encrypted NFS adoption at enterprise scale.  
* xref:../s3-config/regenerate-access-keys.html[Multiple S3 access keys per user]
+
Adds support for multiple active S3 access keys per user, enabling seamless credential rotation without application downtime and helping customers strengthen security and compliance while maintaining uninterrupted access to object workloads. 
* xref:../snapmirror-active-sync/protect-nas-task.html[SnapMirror active sync for NAS]
+
With NAS support, SnapMirror active sync is now a unified business continuity solution providing zero RPO with transparent failover. 
* xref:../san-config/configure-fc-nvme-hosts-ha-pairs-reference.html[FC direct-attached]
+
Introduces secure FC HBA card support, enabling trusted host connectivity and establishing a foundation for hardware‑backed, in‑flight FC security while preserving performance and supporting enterprise compliance requirements.  
* xref:../san-admin/multipath-automatic-lif-failover-support.html[Unified active-active LIF failover ]
+
AFF systems provide active‑active multipathing with automatic LIF failover, keeping all SAN paths active‑optimized so host I/O continues immediately during failover.  
* link:https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html[iSCSI support in FLI^]
+
Adds iSCSI support for Foreign LUN Import (FLI), enabling to migrate block data from third‑party storage systems to ONTAP using existing iSCSI connectivity with minimal disruption.  
* xref:../s3-config/create-modify-bucket-policy-task.html[S3 conditional writes and deletes]
+
Enables support for conditional writes and deletes with the S3 API, allowing applications to enforce data consistency checks during object updates and removals, reducing the risk of accidental overwrites or deletions in concurrent and distributed object workloads. 
* xref:../concepts/path-failover-concept.html#nas-path-failover[Combined LIF failover ]
+
...
* xref:../networking/improve-tcp-wan-performance.html[Enhanced WAN performance]
+
Enhanced WAN performance with TCP RACK improves throughput and resiliency over congested networks, accelerating remote file access and inter-site data services. 
* xref:../networking/configure-tls-offload.html[Hardware‑accelerated kTLS for faster backend cluster encryption]
+
Enhances backend cluster encryption by introducing hardware‑accelerated kTLS, offloading encryption processing to supported SmartNICs to deliver higher performance and lower CPU overhead while maintaining always‑on, secure communication across cluster and HA traffic. 
* xref:../anti-ransomware/use-cases-restrictions-concept.html#snapmirror[AI‑powered ransomware detection for synchronously replicated workloads ]
+
Extends Autonomous Ransomware Protection (ARP/AI) to SnapMirror synchronous synchronous for (SAN and NAS) and SnapMirror active sync SAN environments, preserving detection and recovery signals in zero‑RPO architectures. 
* xref:../concept-svm-admin-dashboard-sm.html[Monitor and manage the SVM using the new ONTAP System Manager dashboard]
+
Provides a tenant friendly GUI for SVM Management, eliminating need for complex CLI interface.     

== ONTAP 9.18.1 highlights

ONTAP 9.18.1 delivers new and enhanced features in the areas of security, scalability, and storage management to support data infrastructure modernization. These enhancements help you scale, operate more efficiently, and secure data for the future. For a complete list of new features and enhancements, see xref:whats-new-9181.adoc[What's new in ONTAP 9.18.1].

* xref:../networking/configure-backend-encryption.html[Backend Cluster HA Encryption]
+
Ability to use mTLS for encrypting the backend cluster network.
* xref:../networking/configure_network_security_using_federal_information_processing_standards_fips.html[PQC support for data in transit]
+
Ability to use post-quantum algorithms for data-in-transit.
* xref:../networking/ipsec-prepare.html[IPv6 IPSec support with HW offload]
+
Support for IPSec with hardware offload with IPv6 address.
* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[ARP/AI support FlexGroup volumes]
+
NetApp FlexGroup volumes support ARP/AI, providing advanced, machine-learning-based ransomware detection and protection.
* xref:../san-admin/nvme-copy-offload.html[Copy Offload for NVMe]
+
Accelerates large data transfers by offloading copy operations directly to storage, reducing host CPU and network load.
* xref:../s3-snapshots/list-restore-s3-snapshot.html[Storage driven S3 bucket snapshot restores]
+
...
ONTAP 9.17.1 delivers innovations in AI-powered Autonomous Ransomware Protection for SAN and adds NVMe front-end host support with SnapMirror active sync and consistency group hierarchical management. These enhancements reinforce NetApp’s leadership in data security and cyber resilience, improving ONTAP performance, scalability, efficiency, and business continuity. For a complete list of new features and enhancements, see xref:whats-new-9171.adoc[What's new in ONTAP 9.17.1].

* xref:../anti-ransomware/index.html[Support for SAN with Autonomous Ransomware Protection]
+
ARP/AI supports SAN volumes with encryption-based anomaly detection, introduces new commands for detailed entropy statistics, and unifies ransomware protection messaging in System Manager that had previously focused on NAS. Configurable detection thresholds and more deterministic snapshot retention provide greater flexibility for diverse workloads. 
...
+
SnapMirror active sync adds support for NVMe access for VMware workloads with NVMe/TCP and NVMe/FC host access for two-node ONTAP clusters.
...
+
ONTAP 9.16.1 adds more commands to the MAV framework for additional protection from malicious insiders. These enhancements include many Consistency Group (CG), VScan and Autonomous Ransomware Protection (ARP/AI) management, and NVMe configuration commands.
* xref:../anti-ransomware/index.html[Autonomous Ransomware Protection with AI enhancements (ARP/AI)]
+
Autonomous Ransomware Protection has been upgraded with new AI capabilities, allowing it to detect and respond to ransomware attacks with 99% precision and recall. Because the AI is trained on a comprehensive dataset, there is no longer a learning period for ARP/AI running on FlexVol volumes and ARP/AI starts in active mode right away. ARP/AI also introduces an automatic update capability independent of an ONTAP upgrade to ensure constant protection and resilience against the latest threats.
...
+
Beginning with ONTAP 9.15.1, you also have the option of backing up the symlink itself instead of the data it points to. This can provide several benefits, including improved performance of your backup applications. You can enable the feature using the ONTAP CLI or ONTAP REST API.
...
** xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[Multi-admin verification support]
+
...
ONTAP CLI, ONTAP REST API, and System Manager. For optimal performance, constituent members within a FlexGroup should have their used capacity evenly distributed.
