...
|===
| Update | Description 
| xref:../encryption-at-rest/configure-netapp-volume-encryption-concept.html[NVE supported on SVM root volumes] | SVM root volumes can now be encrypted using unique keys with NetApp Volume Encryption.
| xref:../snaplock/snapshot-lock-concept.html[Ability to set Snapshot copy locking on long-term retention Snapshot copies ] and xref:../snaplock/initialize-complianceclock-task.html[and to reinitialize the Compliance Clock] | On clusters with a SnapLock license, tamperproof Snapshot copy locking for Snapshot copies with long-term retention can now be set for Snapshot copies created on non-SnapLock SnapMirror destination volumes and the Compliance Clock can be initialized when no SnapLock volumes are present.
...
| xref:../task_dp_configure_storage_vm_dr.html[Asynchronous data protection support for consistency groups within SVM disaster recovery relationship] | SVMs configured for SVM disaster recovery now replicate consistency group information to the secondary site if the SVM contains a consistency group.
...
| Update | Description 
| xref:../s3-config/index.html[S3 object storage support on mirrored and unmirrored aggregates] | You can enable an S3 object storage server on an SVM in a mirrored or unmirrored aggregate in MetroCluster IP and FC configurations.
| xref:../s3-config/ontap-s3-supported-actions-reference.html[Object locking based on users roles and lock retention period] | Objects in S3 buckets can now be locked from being overwritten or deleted. The ability to lock objects is based on specific users or time.
...
| Enhancements to OAuth 2.0 support | ONTAP 9.14.1 has been enhanced to extend the core token-based authentication and OAuth 2.0 support initially provided with ONTAP 9.14.0. Authorization can now be configured using Active Directory or LDAP with group-to-role mapping. Sender-constrained access tokens are also supported and secured based on Mutual TLS (mTLS). And in addition to Auth0 and Keycloak, Microsoft Windows Active Directory Federation Service (ADFS) is now supported as an Identity Provider (IdP).
| OAuth 2.0 Authorization Framework
a| The Open Authorization (OAuth 2.0) framework is added and provides token-based authentication for ONTAP REST API clients. This enables more secure management and administration of the ONTAP clusters using automation workflows powered by REST API scripts or Ansible. The standard OAuth 2.0 features are supported, including issuer, audience, local validation, remote introspection, remote user claim, and proxy support. Client authorization can be configured using selfcontained OAuth 2.0 scopes or by mapping the local ONTAP users. Supported Identity Providers (IdP) include Auth0 and Keycloak using multiple concurrent servers.
+
[NOTE]
OAuth 2.0 Authorization Framework support begins with ONTAP 9.14.0.
...
| Assignment of tags xref:../assign-tags-cluster-task.html[to clusters] and xref:../assign-tags-volumes-task.html[volumes] | You can use System Manager to use tags to categorize clusters and volumes in different ways, for example, by purpose, owner, or environment. This is useful when there are many objects of the same type. Users can quickly identify a specific object based on the tags that have been assigned to it.
| xref:../consistency-groups/index.html[Enhanced support for consistency group monitoring] | System Manager now displays historical data about consistency group usage.
