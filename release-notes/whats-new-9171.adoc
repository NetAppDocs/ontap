...
a|
Any newly created volume on an ONTAP cluster allocated for NAS protocol has File System Analytics (FSA) enabled by default. FSA is activated automatically as soon as a volume is created, providing immediate analytics capabilities without additional configuration.
