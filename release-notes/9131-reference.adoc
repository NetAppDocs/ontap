...
| SnapMirror Business Continuity (SM-BC) enhancements
a| 
* link:../smbc/smbc_admin_add_and_remove_volumes_in_consistency_groups.html[Non-disruptively add volumes to a consistency group with an active SM-BC relationship.]
* link:../smbc/supported-configurations-reference.html#ndmp-restore[Utilize NDMP restore with SM-BC].
| link:link:../consistency-groups/protect-task.html#configure-asynchronous-snapmirror-protection[Asynchronous SnapMirror support with a single consistency groups]
...
|===
| Update | Description 
| xref:../concepts/rdma-concept[Expanded hardware support for RDMA cluster interconnect] | ONTAP supports AFF A900, ASA A900, and FAS9500 systems for cluster interconnect RDMA with an X91153A cluster NIC, helping to reduce latency, decrease failover times, and accelerate communication between nodes.
...
a|
* xref:../use-cases-restrictions-concept.html#multi-admin-verification-with-volumes-protected-with-arp[Multi-admin verify functionality with Autonomous Ransomware Protection]
