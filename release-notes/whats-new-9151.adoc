...
a|
xref:../consistency-groups/configure-task.html[Support for admin privilege level for ONTAP REST API and ONTAP CLI operations with consistency groups] 
a|
ONTAP CLI and ONTAP REST API operations for consistency groups are now supported at the administrative privilege level. 
a|
