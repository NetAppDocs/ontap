...
a|
Expanding from the previous two-node limit, SnapMirror active sync supports up to four clusters, enabling data replication for larger workloads  
...
| Update | Description
a|
ONTAP S3 support for NAS S3 tags and user metadata
a|
You can apply https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-tagging.html[tags and metadata^] that comply with AWS S3 specifications to objects or files stored in the ONTAP S3 service and presented in duality mode using NFS and SMB. Apply tags and metadata on S3 objects using S3 clients to define lifecycles, back charging, data categories, and custom workflows on data stored as object or files in ONTAP. When integrated with AWS data services such as Bedrock or Athena, tagging and metadata become central to the data processing provided by these services. 
...
a|
ONTAP supports migrating an SVM from a non-MetroCluster HA pair to a MetroCluster configuration or from a MetroCluster configuration to a non-MetroCluster HA pair. You can also migrate an SVM between two MetroCluster IP configurations or from a MetroCluster FC configuration to a MetroCluster IP configuration.
