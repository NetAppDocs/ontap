...
a|
ARP/AI protects primary volumes in SnapMirror synchronous (NAS and SAN) relationships on FAS, AFF, and AFX systems, and in SnapMirror active sync SAN relationships on AFF and ASA r2 systems. Detection runs on the active read-write primary, where ARP/AI snapshots are created (snapshots are not replicated).
