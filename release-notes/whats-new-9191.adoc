...
| Update | Description 
a|
Autonomous Ransomware Protection workload characteristics removed from System Manager UI
a|
System Manager no longer displays Autonomous Ransomware Protection workload characteristics, including baseline and surge statistics. This information has been removed as surge activity was frequently misinterpreted as an actual ransomware attack or as something that required manual adjustment.

Autonomous Ransomware Protection behavior and detection are unchanged, and only the System Manager presentation of these details is affected. Surge statistics remain available using ONTAP CLI and ONTAP REST API options for advanced analysis
...
a|
ARP/AI protects primary volumes in SnapMirror synchronous (NAS and SAN) relationships on FAS, AFF, and https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html[AFX^] systems, and in SnapMirror active sync SAN relationships on AFF and https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html[ASA r2^] systems. Detection runs on the active read-write primary, where ARP/AI snapshots are created (snapshots are not replicated).

Clusters upgraded to ONTAP 9.19.1 that previously had default ARP/AI enablement suppressed due to synchronous replication relationships will have it automatically restored. A pre-update warning notifies administrators and provides the option to disable it.
