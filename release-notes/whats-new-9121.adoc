...
| xref:../snaplock/snapshot-lock-concept.html[Tamperproof Snapshot copies] | You can lock a snapshot on a non-SnapLock volume to provide protection from ransomware attacks. Locking Snapshot copies helps ensure that they are not deleted accidentally or maliciously.
|===
