...
The same steps are used when configuring FC adapters for the FC protocol and the FC-NVMe protocol. However, only certain FC adapters support FC-NVMe. See https://hwu.netapp.com[NetApp Hardware Universe] for a list of adapters that support the FC-NVMe protocol.
