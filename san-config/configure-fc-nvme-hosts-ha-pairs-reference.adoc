...
Beginning with ONTAP 9.19.1, direct-attachment FC and FC-NVMe configurations are supported for certain AFF, ASA r2 and FAS systems using the required QLogic or Emulex FC adapter card. In a direct-attachment configuration, one or more hosts are directly connected to your HA pairs without the presence of a switch. This configuration can be especially valuable in remote sites and hybrid environments where simplicity is key.

[NOTE]
====
* MetroCluster configurations do not support direct attachment. 
* Direct-attachment configurations do not support link:../svm-migrate/migrate-svm-task.html[SVM migration].
====

For a complete list of supported platforms and adapter cards, see the link:https://hwu.netapp.com/[NetApp Hardware Universe^].

image:scrn_en_drw_fc-302020-direct-sing-on.png[Direct attach configuration]


.Related information
* link:../san-admin/configure-fc-adapters-task.html[Configure ONTAP FC ports for direct-attach to FC and FC-NVMe hosts].
...
// 2026 Jun 09, ONTAPDOC-3573
// 2026 Jun 08, ONTAPDOC-3935
