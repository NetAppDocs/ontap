...
. Verify that your backup application provider's interoperability matrix indicates that the backup application is compatible with ONTAP.

. Verify that the planned configuration is supported by using the https://mysupport.netapp.com/matrix[NetApp Interoperability Matrix Tool^]:
+
