...
.System Manager
--
Create LUNs to provide storage for an ESXi host using the FC or iSCSI SAN protocol with System Manager for ONTAP 9.7 or later. LUNs appear as disks to the ESXi host.
...
.CLI
--
This procedure creates new LUNs on an existing storage VM using the ONTAP CLI. Your FC or iSCSI protocol should already be set up.
