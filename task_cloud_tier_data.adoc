...
If you are tiering to ONTAP S3, there must be entries for the ONTAP S3 server in the local cluster's DNS server, whether that's an external or a local DNS server, including the S3 server's FQDN name and the IP addresses on its network interfaces.
