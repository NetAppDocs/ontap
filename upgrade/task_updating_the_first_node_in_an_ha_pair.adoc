---
permalink: upgrade/task_updating_the_first_node_in_an_ha_pair.html
sidebar: sidebar
keywords: 
summary: 'You can update the first node in an HA pair by initiating a takeover by the node’s partner. The partner serves the node’s data while the first node is upgraded or downgraded.'
---
