...
---
= ONTAP version compatibility for mixed version clusters
:icons: font
:imagesdir: ../media/
...
. link:concept_upgrade_methods.html[Upgrade] the nodes that do not support the minimum ONTAP version required by your new controllers to the highest ONTAP version they support.  
+
...
+
If there is a difference between the ONTAP version reported in the output for the `version *` (cluster) and `version` (individual nodes) commands, update all nodes to the same ONTAP and patch version by running a link:concept_upgrade_methods.html[cluster image update].
