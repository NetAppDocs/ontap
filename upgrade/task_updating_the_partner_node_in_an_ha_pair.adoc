---
permalink: upgrade/task_updating_the_partner_node_in_an_ha_pair.html
sidebar: sidebar
keywords: 
summary: 'After updating the first node in an HA pair, you update its partner by initiating a takeover on it. The first node serves the partner’s data while the partner node is upgraded or downgraded.'
---
