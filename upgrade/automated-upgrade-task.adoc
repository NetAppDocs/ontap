...
= Perform an automated nondisruptive ONTAP upgrade
:toclevels: 1
:hardbreaks:
:nofooter:
:icons: font
...
[.lead]
When you perform an automated nondisruptive upgrade (ANDU), ONTAP automatically installs the target ONTAP image on each node, validates that the cluster can be upgraded successfully, and then executes either a xref:concept_upgrade_methods.html[batch or rolling upgrade] in the background based on the number of nodes in the cluster. 
...
[NOTE]
If you are upgrading to ONTAP 9.15.1 or later through the NetApp Console, follow the link:https://docs.netapp.com/us-en/console-software-updates/get-started/software-updates.html[upgrade procedure in the NetApp Console documentation^].

[IMPORTANT]
Modifying the setting of the `storage failover modify-auto-giveback` command option before the start of an ANDU has no impact on the upgrade process. The ANDU process ignores any preset value to this option during the takeover/giveback required for the update. For example, setting `-autogiveback` to false prior to beginning ANDU does not interrupt the automatic upgrade before giveback. Learn more about `storage failover modify-auto-giveback` in the link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback[ONTAP command reference^].
...
* Optionally, link:../update/pre-stage-system-firmware.html[pre-stage system firmware for a single-reboot upgrade]. 
+
Beginning with ONTAP 9.17.1, if newer BIOS or LOADER firmware is already present on a node's boot device before you perform an ANDU, the upgrade process preserves the newer firmware rather than overwriting it with the version bundled in the ONTAP image. This allows both the ONTAP software upgrade and the firmware update to complete in a single reboot cycle.
...
.. Select *Validate* to run the pre-upgrade validation checks.
+
...
+
A list of complete automated upgrade pre-checks is displayed along with any errors or warnings that should be addressed before you begin the upgrade process. You must resolve all errors before proceeding with the upgrade. It is best practice to also resolve warnings.
+
Beginning with ONTAP 9.19.1, errors and warning are displayed separately to help you easily identify issues that must be resolved before you upgrade.  
...
If you pre-staged your firmware, use the `system node firmware bootmedia show` command to verify that the intended firmware version is active on each node.
...
// 2026 May 07, ONTAPDOC-3598
// 2025-Sept-10, BLUEXPDOC-87
