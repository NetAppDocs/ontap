...
---
= Manual ONTAP NDU on a four- or eight-node MetroCluster configuration
:icons: font
:imagesdir: ../media/

[.lead]
A manual nondisuptive upgrade (NDU) of a four-node or eight-node MetroCluster configuration involves preparing for the update, updating the DR pairs in each of the one or two DR groups simultaneously, and performing post-upgrade tasks.

.About this task

* You must use the ONTAP command line interface (CLI) to perform this task.
...
. If MetroCluster Tiebreaker software is enabled, disable it.
...
----
+
The following example shows that the system image2 (target ONTAP image) is the default and current version on node_A_1:
+
----
 cluster_A::*> system image show
                  Is      Is               Install
...
----
+
The following example shows that the system image2 (target ONTAP image) is the default and current version on node_B_1:
+
----
 cluster_B::*> system image show
                  Is      Is               Install
...
----
+
The following example shows that the system image2 (target ONTAP image) is the default and current version on node_A_2:
+
----
cluster_A::*> system image show
                 Is      Is                 Install
...
----
+
The following example shows that the system image2 (target ONTAP image) is the default and current version on node_B_2:
+
----
cluster_B::*> system image show
                 Is      Is                 Install
