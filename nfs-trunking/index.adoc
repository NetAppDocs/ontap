...
[lead]
Beginning with ONTAP 9.12.1P3, NFSv4.1 clients can take advantage of session trunking functionality to open multiple connections to the NFS server, thereby increasing the speed of data transfer and providing resiliency through multipathing.
...
In ONTAP 9.12.1P3, trunking is restricted to LIFs on a single node; trunking cannot span LIFs across multiple nodes.
