...
+
[NOTE]
Although you can modify an existing SVM to enable trunking, it's recommended that you use a new, dedicated SVM for NFS trunking. Enabling NFS trunking on an existing NFS server can impact I/O on the existing client I/O and reduce concurrency due to required remounting of NFS clients.  
