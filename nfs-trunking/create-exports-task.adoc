...
ONTAP export requirements:

* The default export policy of the SVM root volume must include a rule to allow all clients open access through NFS. 
+
Learn more about link:../nfs-config/open-export-policy-svm-root-volume-task.html[NFS client access on the ONTAP SVM.]
* You must add the same rule to the default policy if you created a custom export policy.
+
Learn more about link:../nfs-admin/default-export-policy-svms-concept.html[default export policies for ONTAP NFS SVMs.]
