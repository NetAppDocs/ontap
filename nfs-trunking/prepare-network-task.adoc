...
You can have multiple failover groups, but the failover group for trunking should include only those LIFS in the trunking group. Note that when the trunked LIFs are configured in a failover group on the same node, a controller failover on that node can cause the trunked LIFs to go offline. If the LIFs are not configured in a failover group on the same node and failover to another node, then trunking will no longer function.
