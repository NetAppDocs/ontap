...
summary: "Performance of workloads begins to be affected by latency when the amount of work on a node exceeds the available resources. You can manage an overloaded node by increasing the available resources, such as upgrading disks or CPU, or by reducing load, such as moving volumes or LUNs to different nodes as needed."
---
= Learn about load balancing in ONTAP
:icons: font
:imagesdir: ../media/
