...
*Recommended*: Configure the native FPolicy in NAS storage VMs to control the file extensions that are allowed or not allowed to be written on the volumes in the cluster. This helps prevent attacks from ransomware with known file extensions.  *IMPORTANT*:  Blocking extensions might lead to unexpected results.  Before you configure the native FPolicy, see link:./ontap/insights-fpolicy-best-practice-task.html[Best practice: Configure native FPolicy].
