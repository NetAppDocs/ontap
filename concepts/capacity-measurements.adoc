...
| `volume show` | `physical used` | Does not include aggregate-level storage efficiency savings, so does not accurately reflect the volume's physical used space for link:enable-temperature-sensitive-efficiency-concept.html[TSSE enabled platforms].
