...
| `volume show` | `physical used` | Does not include aggregate-level storage efficiency savings, so does not accurately reflect the volume's physical used space for link:..volumes/enable-temperature-sensitive-efficiency-concept.html[TSSE enabled platforms].
...
* Learn more about the link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_is_the_Change_in_Physical_Used_Space_Reporting_for_9.14.1_and_later[change in physical used space reporting for 9.14.1 and later^].
* Learn more about link:../volumes/commands-display-space-usage-reference.html[commands for displaying space usage information].
