...
** It is recommended but not required that CA certificates for TLS access are installed on storage VMs that host S3 servers. You can use a self-signed CA certificate or a certificate signed by an external CA vendor.
