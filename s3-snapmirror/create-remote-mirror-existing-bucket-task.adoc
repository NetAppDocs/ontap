...
When you cli*ck *Save*, the exist*ing bucket is mirrored to a new bucket in the destination storage VM.
...
----
. Install CA certificates on the source and destination clusters:
.. On the _source_ cluster, install the S3 server certificate from the _destination_ cluster:
`security certificate install -type server-ca -vserver _src_svm_ -cert-name _dest_server_certificate_`
.. On the _destination cluster_, install the S3 server certificate from the _source_ cluster:
