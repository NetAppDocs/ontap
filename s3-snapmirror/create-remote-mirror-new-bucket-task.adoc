...
##Added for 9.14.1##
Beginning with ONTAP 9.14.1, you can enable object locking on destination buckets, provided, the source and destination clusters run ONTAP 9.14.1 or later, and object-locking is enabled on the source bucket. The retention mode of the source bucket is applicable for the destination bucket. You can define the lock retention period on the destination bucket, and the same retention period is applied to the non-locked objects replicated from the source bucket. 
