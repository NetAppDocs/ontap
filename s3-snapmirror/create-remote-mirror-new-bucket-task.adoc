...
##Added for 9.14.1##
Beginning with ONTAP 9.14.1, you can enable object locking on destination buckets, provided, the source and destination clusters run ONTAP 9.14.1 or later, and object-locking is enabled on the source bucket. The retention mode of the source bucket is applied on the destination bucket. However, you can define a different lock retention period on the destination bucket, and the same retention period is applied to the non-locked objects replicated from either the source bucket or the S3 interfaces. 
