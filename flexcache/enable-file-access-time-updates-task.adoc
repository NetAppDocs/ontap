...
. Once `-atime-update` is enabled, you can specify if the files on a FlexCache volume can be scrubbed automatically:
+
`volume flexcache config modify -vserver <SVM name> -volume <volume_name> -is-atime-scrub-enabled <true|false>`

For additional information about the `-is-atime-scrub-enabled` parameter, see the 
