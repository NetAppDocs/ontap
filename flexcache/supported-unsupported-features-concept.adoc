...
* Supported beginning with ONTAP 9.5. The primary SVM of an SVM DR relationship can have the origin volume; however, if the SVM DR relationship is broken, the FlexCache relationship must be re-created with a new origin volume.

* Beginning with ONTAP 9.18.1, you can create cache volumes for origin volumes with SVMs that belong to an SVM-DR relationship. The cache SVMs must be peered with both the primary and secondary origin SVMs that are part of an SVM-DR relationship before creating cache volumes.
