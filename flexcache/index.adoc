...
Beginning with ONTAP 9.19.1, performance improves when you have hundreds to thousands of clients simultaneously listing the same small NAS directory, 2MB or less (approximately 25,000 files). This enhancement reduces the need to use workarounds, like local FlexCache volumes, to support high client counts performing directory listings. For lower client counts, performance will not be negatively impacted. ONTAP 9.19.1 allows more CPU resources to process listing requests of small NAS directories. This performance enhancement is automatic and does not require configuration.
...
// 18-MAR-2026 ONTAPDOC-3787 and NADIAA for my change only
