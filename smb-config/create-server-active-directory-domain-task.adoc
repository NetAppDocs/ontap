...
You require the following minimum privileges on the organizational unit (OU) to create the SMB server:
...
* Read msDS-SupportedEncryptedTypes
* Write msDS-SupportedEncryptedTypes
...
* You can choose to add the SMB server to a different OU by using the `-ou` option.
* Beginning with ONTAP 9.13.1, you can configure a default site for the SMB server. ONTAP can use this site when a LIF does not have site membership and as a fallback if site discovery fails for a trusted domain when locating trusted-domain domain controllers.
...
. Create the SMB server in an AD domain: `+vserver cifs create -vserver vserver_name -cifs-server smb_server_name -domain FQDN [-ou organizational_unit][-default-site site_name][-netbios-aliases NetBIOS_name, ...][-keytab-uri {(ftp|http)://hostname|IP_address}][-comment text]+`
+
...
Beginning with ONTAP 9.13.1, the SMB server default site is also used as a fallback if ONTAP cannot discover site information for a trusted domain.
