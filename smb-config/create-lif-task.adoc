 ...
 ** The `-data-protocol` parameter is not required when creating a LIF using a service policy (beginning with ONTAP 9.6).
 +
 When using ONTAP 9.5 and earlier, the `-data-protocol` parameter must be specified when the LIF is created, and cannot be modified later without destroying and re-creating the data LIF.
