...
* If an object version (in a versioned bucket) or an object (in an non-versioned bucket) is captured in multiple snapshots, the object will be removed from the file system only after the last snapshot protecting it is deleted.
