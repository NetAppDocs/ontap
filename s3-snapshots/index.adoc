...
S3 snapshots only capture the current versions of the objects in S3 buckets. The non-current versions of versioned buckets are not captured in the S3 snapshots. Also, the point-in-time object tags are not captured in the snapshots if the object tags are modified after the snapshots are taken.
...
You should be aware of the following feature exclusions and scenarios:
...
// 2026-Aug-19, ONTAPDOC-4265
// 2025-Dec-17, ONTAPDOC-3567
