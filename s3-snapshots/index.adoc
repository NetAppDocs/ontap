...
Using the S3 snapshots feature, you can manually create snapshots or automatically generate them through snapshot policies. S3 snapshots are presented as S3 buckets in S3 clients. They are not real S3 object buckets, but point-in-time images represented as buckets. You can browse and restore the content from the snapshots through S3 clients.
...
* S3 snapshots are not supported on buckets where object-locking is enabled.
* The S3 snapshots are unsupported on NetApp BlueXP.
* The S3 snapshots are unsupported in ONTAP MetroCluster configurations.
