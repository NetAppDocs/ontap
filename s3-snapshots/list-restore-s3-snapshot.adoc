...
Beginning with ONTAP 9.18.1, support is added for ONTAP snapshot restore. S3 clients can perform a restore by browsing a specific snapshot and manually updating each object to match the snapshot.

* You can restore an entire bucket using an ONTAP S3 snapshot. 
...
== Restore content from S3 snapshots (ONTAP 9.17.1 and earlier)

For versions of ONTAP earlier than ONTAP 9.18.1, you can use this procedure to perform a restore operation on an S3 client to recover a single object, a set of objects, or an entire bucket by copying content from a snapshot bucket to the original or a different bucket. You can browse snapshots to determine which snapshot content you should copy. 
