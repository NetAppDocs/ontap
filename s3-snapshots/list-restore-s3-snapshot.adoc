...
Beginning with ONTAP 9.18.1, support is added for ONTAP snapshot restore, providing ONTAP administrators with point-in-time restore functionality. S3 clients can perform a restore by browsing a specific snapshot and manually updating each object to match the snapshot. Only the current bucket version is captured in the snapshot. Version history is not captured, and it will not be restored by the S3 snapshot restore operation.
