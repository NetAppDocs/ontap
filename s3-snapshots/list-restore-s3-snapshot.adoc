...
* All nodes in the cluster must be running ONTAP 9.18.1 or later before you can perform the bucket snapshot restore operation. 
...
== Restore a bucket using ONTAP S3 snapshots
Beginning with ONTAP 9.18.1, you can restore an entire bucket using an ONTAP S3 snapshot. You can restore only the version of the bucket that existed at the time the selected snapshot was created.
...
== Restore content from S3 bucket snapshots

You can perform a restore operation using a S3 bucket snapshot. A bucket snapshot is an object version captured in that snapshot. This restore operation is S3 client-driven. You perform a restore on an S3 client to recover a single object, a set of objects, or an entire bucket by copying content from a snapshot bucket to the original or a different bucket. You can browse snapshots to determine which snapshot content you should copy. 
