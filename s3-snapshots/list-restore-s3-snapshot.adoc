...
S3 snapshots are presented as snapshot buckets in the S3 clients. The snapshot bucket name is formatted as `<base_bucket_name>-s3snap-<snapshot_name>`. You can see all the snapshot buckets in a storage VM using the S3 `ListBuckets` API endpoint.
