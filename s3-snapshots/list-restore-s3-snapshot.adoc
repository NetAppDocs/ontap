...
Beginning with ONTAP 9.18.1, you can restore an entire bucket using an ONTAP S3 snapshot. All nodes in the cluster must be running ONTAP 9.18.1 or later before you can perform the bucket snapshot restore operation.

.Steps

. Identify the snapshot from which you want to restore the bucket:
+
----
vserver object-store-server bucket snapshot show
----
