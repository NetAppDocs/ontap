* Beginning with ONTAP 9.19.1 and ONTAP Mediator 1.12, you have the option to install ONTAP Mediator with iSCSI and HTTPS support, or support for HTTPS only. If you choose to install ONTAP Mediator with support for HTTPS only, the SCST package is not installed. 

* If you upgrade from ONTAP Mediator 1.11 or earlier with SCST installed, and you choose the HTTPS only option during the upgrade, the SCST package is uninstalled and the new ONTAP Mediator version does not contain the SCST package. 

* If you plan to use ONTAP Mediator to monitor multiple MetroCluster IP configurations, the support depends on your Mediator type, ONTAP version, and OS. Refer to link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#mediator-multiple-mccip[Use ONTAP Mediator to monitor multiple MetroCluster IP configurations^] for more information.

* Installing ONTAP Mediator with support for HTTPS only is supported for the following use cases:

** You plan to only use ONTAP Mediator to monitor your SnapMirror active sync clusters.
** You plan to configure ONTAP Mediator with HTTPS to monitor one or more MetroCluster IP configurations.

* ONTAP Mediator must be installed with support for iSCSI and HTTPS in the following scenario:

** You plan to configure ONTAP Mediator with iSCSI to monitor one or more MetroCluster IP configurations.

// 2026 Sep 4, GH 2118