...
. [[epsilon-true]]If that node that you are removing holds epsilon:
...
----
+
The node is marked unhealthy until eligibility is restored. When the master node become ineligible, one of the remaining nodes is elected by the cluster quorum as the new master.
+
[NOTE]
====
...
** After removing the node and before it can rejoin a cluster, you must use boot menu option *(4) Clean configuration and initialize all disks* or option *(9) Configure Advanced Drive Partitioning* to erase the node's configuration and initialize all disks.
+
