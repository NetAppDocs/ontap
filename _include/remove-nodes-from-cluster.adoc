...
----
+
The primary node is the node that holds processes such as `mgmt`, `vldb`, `vifmgr`, `bcomd`, and `crs`.

. If the node you are removing is the current primary node, enable another node in the cluster to be elected as the primary node:

.. Make the current primary node ineligible to participate in the cluster:
+
[source,cli]
----
...
----
+
The node is marked unhealthy until eligibility is restored. When the primary node becomes ineligible, one of the remaining nodes is elected by the cluster quorum as the new primary.
+
[NOTE]
====
...
+
If the node is the quorum primary, the cluster will briefly lose and then return to quorum. This quorum loss is temporary and does not affect any data operations.
