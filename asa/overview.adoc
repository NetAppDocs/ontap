...
Beginning with ONTAP 9.12.1 an ASA is available as a software license instead of as a separate hardware platform.  The ASA software license makes all the features of the previous ASA platform available on any AFF controller that meets requirements for ASA support. Supported configurations are listed in the http://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix Tool^].

ASAs can be configured for FC or iSCSI, and use symmetric active-active for multipathing. All paths are active/optimized so in the event of a storage failover, the host does not need to wait for the ALUA transition of the failover paths to resume I/O. This reduces time to failover.
