...
Beginning with ONTAP 9.18.1, you can create NAS buckets that enable applications to access data on FlexCache volumes using the S3 protocol. All nodes in the cluster are running ONTAP 9.18.1 or later. Before you can access a FlexCache volume using S3 protocol, you must set the `-is-s3-enabled` option to `true` link:../flexcache/create-volume-task.html[on the FlexCache volume]. The parameter is set to `false` by default.
