...
[NOTE]
Multipart objects in S3 NAS buckets must be aligned in whole sizes not partial. For example, a part can be 4MB or 4GB or a similar size. A part cannot use partial or sub-MB sizes, such as 4.5MB or 4000.5MB. 
...
* CopyObject not supported in SnapLock volumes
...
* Beginning with ONTAP 9.16.1, support for metadata with single-part objects.
...
* Beginning with ONTAP 9.16.1, support for tags with single-part objects.
...
// 2026-Jul-15, CopyObject not supported in SnapLock volumes 
// 2026-Feb-3, GitHub issue #1906
// 2026-Jan-16, Fixed typos
