...
AWS user metadata::
* For ONTAP 9.15.1 and prior, key-values pairs received as part of S3 user-metadata are not stored on disk along with object data in the current release. 
* For ONTAP 9.15.1 and prior, request headers with the prefix "x-amz-meta" are ignored.

AWS Tags:: 
* For ONTAP 9.15.1 and prior, on PUT object and Multipart Initiate requests, headers with the prefix "x-amz-tagging" are ignored. 
* For ONTAP 9.15.1 and prior, requests to update tags on an existing file (Put, Get, and Delete requests with the ?tagging query-string) are rejected with an error.
