...
* Beginning with ONTAP 9.17.1, IdP-provided group information can be mapped to ONTAP roles. This allows you to assign roles to users based on the groups that are defined in the IdP. For more information, see link:../authentication/authentication-groups.html[Working with groups].
...
* Shibboleth
* Entra beginning with ONTAP 9.17.1
...
. Enter the URL of the IdP URI (including "https://"). If you are using Entra, skip this step for now.
...
. Go to the IdP system you specified and copy the metadata from your clipboard to update the system metadata. If you are using Entra, copy the IdP URI into ONTAP after you have configured Entra with the system metadata.
...
.CLI
--
. If you are using Entra, generate the metadata file for ONTAP by running the following command:
+
[source,cli]
----
...
----
+
Configure Entra with the provided metadata. You must configure Entra before creating an ONTAP SAML configuration. Entra will not provide the IdP URI until it is confiugred with the ONTAP metadata. This step is not required if you are using a different IdP.
...
. From the IdP host, configure the IdP with the ONTAP host metadata. If you are using Entra, you have already completed this step.
+
