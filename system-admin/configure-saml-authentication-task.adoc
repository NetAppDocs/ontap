...
* The IdP that you plan to use for remote authentication must be <<Configure third-party IdP, configured>>. You must have the URI of the IdP.

* Ports 443 and 80 must be open between the ONTAP cluster and the IdP.
...
.Shibboleth
--
Prior to configuring the Shibboleth IdP, you must have a LDAP server configured.

When enabling SAML on ONTAP, save the provided host metadata XML. On the host where Sibboleth is installed, replace the contents of `metadata/sp-metadata.xml` with the host metadata XML within the Shibboleth IdP home directory.

For more information, refer to the ink:https://shibboleth.atlassian.net/wiki/spaces/IDP4/overview[Shibboleth 4 documentation^].
--
