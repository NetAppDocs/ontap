...
* Beginning with ONTAP 9.17.1, IdP-provided group information can be mapped to ONTAP roles. This allows you to assign roles to users based on the groups that are defined in the IdP. For more information, see link:../authentication/authentication-groups.html[Working with OAuth 2.0 or SAML IdP groups in ONTAP].
...
. From the IdP host, configure the IdP with the ONTAP host metadata. If you are using Entra ID, you have already completed this step.
+
