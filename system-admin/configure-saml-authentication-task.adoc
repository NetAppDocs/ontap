...
In order to authenticate with ONTAP, you may need to change the settings for your IdP. The following sections provide configuration information for supported IdPs.
...
.Entra ID
--
When configuring Entra ID, create a new application and configure SAML sign-on with the metadata provided by ONTAP. Once the application is created, edit the "Attributes & Claims" section of the application SAML settings to match the following:
