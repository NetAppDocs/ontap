---
sidebar: sidebar
permalink: setup-autosupport-task.html
...
[.lead]
You can optionally configure an ONTAP cluster to deliver AutoSupport messages to NetApp technical support and send email copies to your internal support organization. As part of this, you can test the configuration before using it.

.About this task

With ONTAP 9.5 and later releases, you enable and configure AutoSupport on all nodes of a cluster simultaneously. When a new node joins the cluster, the node automatically inherits the same AutoSupport configuration. To support this, the scope of the CLI command `system node autosupport modify` is cluster-level. The `-node` command option is retained for backward compatibility but is ignored with ONTAP 9.15 and later.
...
The recommended configuration for delivering AutoSupport messages to NetApp is HTTPS (HTTP with TLS). This option provides the most robust features and best security.
...
.. Display the values available for the new command parameter:
+
----
cluster1::> system node autosupport modify -smtp-encryption ?
----
.. Enable TLS for the SMTP message delivery:
+
----
