...
Certain command types can be used across SP and BMC command sets. The differences between them will be shown when entering the command line.

.Examples
* The `help` command within SP: `SP> help`
* The `help` command within BMC: `BMC> help`

Learn more about these options at link:https://docs.netapp.com/us-en/ontap/system-admin/online-help-at-sp-bmc-cli-task.html[View the help information in the ONTAP SP/BMC CLI^].
