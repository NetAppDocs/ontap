...
Learn more about the `help` command options available for SP and BMC in link:online-help-at-sp-bmc-cli-task.html[View the help information in the ONTAP SP/BMC CLI^].

Learn more about corresponding ONTAP BMC CLI in link:bmc-cli-commands-reference.html[Supported CLI commands for the ONTAP Baseboard Management Controller^].
