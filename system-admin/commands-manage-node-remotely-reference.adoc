...
a|
`sp status` [`-v` \| `-d`]The `-v` option displays SP statistics in verbose form. The `-d` option adds the SP debug log to the display.

a|
`bmc status` [`-v` \| `-d`]The `-v` option displays SP statistics in verbose form. The `-d` option adds the SP debug log to the display.
...
a|
`system core` [`-f`]The `-f` option forces the creation of a core dump and the reset of the node.

a|

a|
`system node coredump trigger` (advanced privilege level)
...
a|
`system battery auto_update` [`status` \| `enable` \| `disable`] (advanced privilege level)
...
a|
`system battery verify` [`image_URL`] (advanced privilege level)
...
a|
`system battery flash` `image_URL` (advanced privilege level)
