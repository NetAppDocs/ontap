 ...
 ** GET requests are not logged by default, but you can control whether GET requests sent from the ONTAP CLI (`-cliget`), from the ONTAP APIs (`-ontapiget`), or from the REST API (`-httpget`) are logged in the file.
