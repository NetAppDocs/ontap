...
a|
Addresses specified in `-to` and `-noteto`. (Only critical, service-affecting events are sent.)Addresses specified in `-partner-address`
...
a|
Addresses specified in ``-partner-address``Technical support, if `-support` is set to `enable`
...
a|
If a URI is specified using the `-uri` parameter in the `system node autosupport invoke` command, the message is sent to that URI. If `-uri` is omitted, the message is sent to the addresses specified in `-to` and `-partner-address`. The message is also sent to technical support if `-support` is set to `enable`.
a|
...
xref:autosupport-delivery-technical-support-concept.adoc[How AutoSupport OnDemand obtains delivery instructions from technical support]
