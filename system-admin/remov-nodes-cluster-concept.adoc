...
// add note here: this will cause alerts
...
. If the node you want to remove is in an SFO relationship, disable SFO:
+
[source,cli]
----
storage failover modify -node <node_name> -enabled false
----
//do we need to disable it on the partner as well?
+
