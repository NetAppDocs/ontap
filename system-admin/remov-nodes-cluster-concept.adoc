...
//above code snippets not rendering
...
----
+
This will cause the node to be temporarily marked as unhealthy. When the master node become ineligible, one of the remaining nodes is elected by the cluster quorum as the new master.
...
. If storage failover is enabled on the node, then disable it:
+
[source,cli]
----
storage failover modify -node <node_name> -enabled false
----
// this is actually called out under the "remove the node..." step
// do we need to disable it on the partner as well? or any additional warning here?
// verify this command is correct for all ONTAP versions?
+
