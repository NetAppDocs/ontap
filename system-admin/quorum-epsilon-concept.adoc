...
Each node in the cluster participates in a voting protocol that elects one node _primary_; each remaining node is a _secondary_. The primary node is responsible for synchronizing information across the cluster. When quorum is formed, it is maintained by continual voting. If the primary node goes offline and the cluster is still in quorum, a new primary is elected by the nodes that remain online.
