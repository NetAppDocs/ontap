...
summary: "The client and server-ca types specify that the certificate is used for mutually authenticating the cluster and an external key manager, such as a KMIP server."
---
...
. Use the `security certificate install` command with the `-type server-ca` parameter to install a certificate for the KMIP server.
...
// 2026 Jun 3, ONTAPDOC-3891, remove kmp-cert subtype (deprecated) 
