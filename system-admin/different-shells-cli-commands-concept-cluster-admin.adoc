...
Nodeshell options that are supported in the clustershell can be accessed by using the `vserver options clustershell` command. To see these options, you can query the clustershell CLI with `vserver options -vserver _nodename_or_clustername_ -option-name ?`
