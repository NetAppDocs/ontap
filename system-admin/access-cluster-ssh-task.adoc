...
* Beginning with ONTAP 9.19.1, administrators can use the link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=security+ssh+publickey+show[security ssh publickey show -vserver <vsname> -host-key-algorithm <algorithm> command^] to view the ONTAP host key fingerprint. This allows administrators to verify that this fingerprint matches the one stored in ONTAP.
