...
NOTE: If you installed ONTAP Mediator 1.12 or later with support for HTTPS only, the SCST package is not installed and the host OS kernel can be upgraded without affecting ONTAP Mediator. Refer to your host OS documentation for upgrade instructions.
...
a| ONTAP Mediator 1.12.1   a| scst-3.10.tar.gz
a| ONTAP Mediator 1.12     a| scst-3.10.tar.gz
...
* In some scenarios, the default installation of SCST on the upgraded OS can fail because the upgraded OS requires a patch. 
+
Beginning with ONTAP Mediator 1.12.1, if the installation fails, you can use the SCST repair tool located in `/usr/local/bin/mediator_repair_scst` to repair the installation.
+
If the upgraded OS requires a patch, the tool contains it for the version of SCST shipped within the ONTAP Mediator package. The `mediator_repair_scst` tool is a standalone tool that is separate from the main installer and is packaged inside the ONTAP Mediator `tgz` file. It does not affect the ONTAP Mediator installation or upgrade path.
