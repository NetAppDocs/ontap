...
NOTE: The ONTAP Mediator service is not compatible with Red Hat Enterprise Linux FIPS mode and will prevent it from installing successfully. You can check if FIPS mode is enabled using the `fips-mode-setup --check` command. You can disable FIPS mode using the `fips-mode-setup --disable` command. Reboot after disabling FIPS mode to successfully install the ONTAP Mediator service.
...
The best practices for using the SCST kernel are listed below.
[NOTE] 
====
* The kernel version must match the operating system version.
* Upgrading to a kernel beyond the supported OS release for the specific ONTAP Mediator release is not supported. (This likely indicates that the tested SCST module won't compile).
...
* Upgrade the SCST by stopping the service, performing the upgrade and restarting the service (OS support for SCST versions might vary). If a failure occurs, a newer version of ONTAP Mediator might need to be used.
...
* Any previous versions of ONTAP Mediator remain backwards compatible with all ONTAP versions but will include security patches for all third party elements.
