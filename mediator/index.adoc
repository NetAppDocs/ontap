...
* Beginning with ONTAP Mediator 1.4, Secure Boot is enabled on UEFI systems configured with RHEL 8.5. When Secure Boot is enabled, you must take additional steps to register the security key after installation. The README files and key location are also referenced in the system output.
