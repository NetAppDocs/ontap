...
NOTE: The ONTAP Mediator service is not compatible with Red Hat Enterprise Linux FIPS mode and will prevent it from installing successfully. You can check if FIPS is enabled using the `fips-mode-setup --check` command. You can disable FIPS using the `fips-mode-setup --disable` command. Reboot after disabling FIPS to successfully install the ONTAP Mediator service.
