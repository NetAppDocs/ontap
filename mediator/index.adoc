 ...
 ** 31784: Configure this port for inbound requests because it's an HTTPS port and only accepts incoming traffic. Use the ONTAP cluster management interfaces as the network source to connect to the ONTAP cluster.
 ** 3260: This port is bidirectional because the iSCSI protocol requires a two-way connection. Use the ONTAP cluster (data LIF or data management LIF) as the network source to connect to the ONTAP cluster.
