...
When you install or upgrade to ONTAP Mediator 1.12, you have the option to install ONTAP Mediator with both iSCSI and HTTPS support, or support for HTTPS only. If you choose to install ONTAP Mediator with support for HTTPS only, the SCST package is not installed. 

If you are upgrading from a previous version of ONTAP Mediator with SCST installed, and you choose to upgrade to ONTAP Mediator 1.12 with support for HTTPS only, the SCST package is uninstalled and the new version will not contain the SCST package. 
