...
. To check if UEFI Secure Boot is enabled on your system, run the following command:
+
`mokutil --sb-state`
+
The results indicate whether UEFI Secure Boot is enabled on this system.
+
NOTE: ONTAP Mediator 1.2.0 and earlier versions do not support this mode.

. To add the public key to the MOK list, run the following command:
+
`mokutil --import /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/scst_module_key.der`
+
NOTE: You may leave the private key in its default location or move it to a secure location. However, the public key must be maintained in its existing location for use by the Boot Manager. For further information, see the following README.module-signing file:
+
