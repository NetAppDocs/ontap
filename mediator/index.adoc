...
`mokutil --sb-state`
+
The results indicate whether UEFI Secure Boot is enabled on this system. If UEFI secure boot is enabled, go to step 2. If UEFI Secure boot is disabled, skip to link:mediator\upgrade-host-os-mediator-task.adoc[Upgrade the host operating system and then the ONTAP Mediator].
...
. Reboot the host and use your device's UEFI Boot Manager to approve the new MOK. You'll need the passphrase provided for the `mokutil` in step 1. 
