...
NOTE: The ONTAP Mediator service is not compatible with Red Hat Enterprise Linux FIPS mode. Enablng Red Hat Enterprise Linux FIPS mode will prevent the ONTAP Mediator service from installing successfully. If you encounter this issue, you can use the `fips-mode-setup --disable` command and reboot to successfully install the ONTAP Mediator service.
