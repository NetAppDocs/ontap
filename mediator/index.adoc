...
You can choose to disable UEFI Secure Boot before installing ONTAP Mediator if it isn't required or if you are troubleshooting ONTAP Mediator installation issues. Disable the UEFI Secure Boot option from your machine settings. 

[NOTE] 
====
Consult system-specific instructions for more detailed information on how to disable UEFI Secure Boot.
====
...
`mokutil --sb-state`
+
The results indicate whether UEFI Secure Boot is enabled on the system. 
+
[cols="40,60"]
|===
...
a| Step 2
a| link:upgrade-host-os-mediator-task.html[Upgrade the host operating system and the ONTAP Mediator]
...
. If the `mokutil` utility is not installed, run the following command: 
+
`yum install mokutil`
...
. Reboot the host and use your device's UEFI Boot Manager to approve the new MOK. You'll need the passphrase provided for the `mokutil` utility in Step 1. 
