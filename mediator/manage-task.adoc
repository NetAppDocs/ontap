...
+
The PKI authority verifies the request and signs the `.csr`, generating the certificate `intermediate.crt`. Additionally, you need to obtain the `root_ca.crt` certificate that signed the `intermediate.crt` certificate from the PKI authority.
+
NOTE: For SnapMirror Business Continuity (SM-BC) clusters and MetroCluster IP configurations using ONTAP Mediator configured with HTTPS, you must add the `intermediate.crt` and `root_ca.crt` certificates to an ONTAP cluster. See link:../snapmirror-active-sync/mediator-install-task.html[Configure ONTAP Mediator and clusters for SnapMirror active sync] and link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html[Configure the ONTAP Mediator for a MetroCluster IP configuration^].
...
. Update the `/opt/netapp/lib/ontap_mediator/uvicorn/config.json` file.
+
Update the values of `ssl_keyfile`, `ssl_certfile`, and `ssl_ca_certs`:
+
...
+
* The `ssl_keyfile` value is the key path in the `ontap_mediator_server.crt` file, which is `ontap_mediator_server.key`.
+
* The `ssl_certfile` value is the path of the `ontap_mediator_server_chain.crt` file.
+
* The `ssl_ca_certs` value is the path of the `root_ca.crt` file.
...
* The `ssl_ca_certs` value is the path of the `root_ca.crt` file.
+
NOTE: For SnapMirror Business Continuity (SM-BC) clusters, you must add the `intermediate.crt` and `root_ca.crt` certificates to an ONTAP cluster. See link:../snapmirror-active-sync/mediator-install-task.html[Configure ONTAP Mediator and clusters for SnapMirror active sync].
...
// 2025-Dec-3, ONTAPDOC-3520
// ONTAPDOC-3431, 2025 OCT 24
