...
[CAUTION]
====
* Third-party certificates are only supported with ONTAP Mediator on certain ONTAP versions. See link:../JiraNgage/CONTAP-243278[NetApp Bugs Online Bug ID CONTAP-243278^].
...
+
NOTE: For SnapMirror Business Continuity (SM-BC) clusters, you must add the `ca.crt` and `root_ca.crt` certificates to an ONTAP cluster. See link:../smbc/smbc_install_confirm_ontap_cluster.html#ontap-mediator[Configure the ONTAP Mediator and clusters for SM-BC].
...
* The `ca_certificate` value is the path of the `root_ca.crt` file.
+
NOTE: For SnapMirror Business Continuity (SM-BC) clusters, you must add the `intermediate.crt` and `root_ca.crt` certificates to an ONTAP cluster. See link:../smbc/smbc_install_confirm_ontap_cluster.html#ontap-mediator[Configure the ONTAP Mediator and clusters for SM-BC].
