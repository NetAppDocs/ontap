...
[NOTE] 
====
* If you're performing a first-time installation and want to pre-install SCST before installing ONTAP Mediator, you can skip this step. The ONTAP Mediator installer will apply any relevant patches to the SCST componenents during installation. 
* Instead, create the required directory by running the following command: `mkdir -p /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/`. 
==== 
