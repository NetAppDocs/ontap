...
.. `depmod`
+
NOTE: If you're performing a first-time installation and want to pre-install ONTAP Mediator, run the following command before continuing with the next step: `mkdir -p /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys`.
