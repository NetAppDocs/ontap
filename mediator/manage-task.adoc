...
CAUTION: Third-party certificates are *only* supported with ONTAP Mediator on systems running ONTAP 9.12.1P15, ONTAP 9.13.1P12, ONTAP 9.14.1P8, or ONTAP 9.15.1P2. 
...
+
The PKI authority verifies the request and signs the `.csr`, generating the certificate `ca.crt`. You also need to obtain the `root_ca.crt` certificate from the PKI authority, which signs the `ca.crt` certificate.
+
...
+
For example, if you obtained `intermediate.crt` from the PKI authority and you store its private key `intermediate.key` at the location `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`, the `ontap_mediator.user_config.yaml` file should look like the following example:
+
NOTE: If `intermediate.crt` is used to sign the `ontap_mediator_server.crt`, the  `intermediate.srl` file is generated. Refer to the note under Step 2 in <<step_2_intermediate_info, Step 2: Generate a server certificate by signing with a third-party CA certification>> for more information.
...
+
NOTE: You should have obtained both top-level certificates from the PKI authority earlier in the procedure.
+
