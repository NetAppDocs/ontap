...
If the VM running ONTAP Mediator needs to be upgraded with a newer kernel, this can cause compatibility issues with the SCST kernel modules used by ONTAP Mediator. In such cases, we recommend manually uninstalling and then reinstalling SCST.
...
NOTE: Perform this step before installing the ONTAP Mediator. If the SCST version you are using is newer than the version bundled with the ONTAP Mediator installer, the installer will skip this step.
...
+
NOTE: If you're performing a first-time installation and want to pre-install SCST before installing ONTAP Mediator, you can skip this step. The ONTAP Mediator installer will apply any relevant patches to the SCST componenents during installation.
