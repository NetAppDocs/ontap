...
The following shows configuration for Web Server setup and ONTAP Mediator Server setup.  
...
+
The PKI authority verifies the request and signs the `.csr`, generating the certificate `ca.crt`.
+
NOTE: For SnapMirror Business Continuity (SM-BC) clusters, you must add the certificate `ca.crt` to an ONTAP cluster. See link:https://docs.netapp.com/us-en/ontap/smbc/smbc_install_confirm_ontap_cluster.html#ontap-mediator[Configure the ONTAP Mediator and clusters for SM-BC].
...
To generate a server certificate, run the following commands from the folder `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`:
...
`-CAcreateserial` option is used to generate the file `ca.srl`.
...
. Replace the `ca.*` files with the third-party certificates.
...
.. If you are using a certificate structure where the `root_ca.crt` provides an `intermediate.crt` that signs the `ontap_mediator_server.crt`, connect the two top-level certificates to create a single certificate:
+
CAUTION: The following substeps are *only* supported in ONTAP 9.12.1P15, ONTAP 9.14.1P8, and ONTAP 9.15.1P2.
+
...
* The `mediator_key` value is the key path in the `ontap_mediator_server.crt`, which is `ontap_mediator_server.key`.
* The `ca_certificate` value is the path of the `root_ca.crt` file.
+
NOTE: On ONTAP clusters, the `intermediate.crt` is also required. 
