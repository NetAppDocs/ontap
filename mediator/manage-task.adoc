...
.. `depmod`
+
NOTE: If you're performing a first-time installation and want to pre-install SCST before installing ONTAP Mediator, you can skip this step. Instead, before you continue with this process, run the following command to create the required directory: `mkdir -p /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/`.  
