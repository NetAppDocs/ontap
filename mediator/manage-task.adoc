...
+
The PKI authority verifies the request and signs the `.csr`, generating the certificate `ca.crt`. You also need to obtain the `root_ca.crt` certificate that signs `ca.crt` from the PKI.
+
