...
CAUTION: Third-party certificates are *only* supported with ONTAP Mediator on systems running ONTAP 9.12.1Px, ONTAP 9.13.1Px, ONTAP 9.14.1Px, or ONTAP 9.15.1Px. 
...
+
image:mediator-cert-purposes.png[]

* The following image shows configuration for Web Server setup and ONTAP Mediator Server setup.  
+
image:mediator-certs-index.png[]
...
+
The PKI authority verifies the request and signs the `.csr`, generating the certificate `ca.crt`. You also need to obtain the `root_ca.crt` certificate from the PKI. The `root_ca.crt` is used to sign the `ca.crt` certificate.
+
...
. To generate a server certificate signing request (CSR), run the following command from the folder `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`:
+
...
. [[step_2_intermediate_info]]To generate a server certificate from the CSR, run the following command from the folder `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`: 
+
NOTE: The `ca.crt` and `ca.key` files were obtained from the PKI. If you are using a different certificate name, for example, `intermediate.crt` and `intermediate.key`, replace `ca.crt` and `ca.key` with `intermediate.crt` and `intermediate.key` respectively.
+
...
+
* The `-CAcreateserial` option is used to generate the `ca.srl` or `intermediate.srl` files depending on the certificate name you are using. 
...
. Configure the file `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config/ontap_mediator.user_config.yaml` to override the default variable values in the `ontap_mediator.config.yaml` file. 
+
For example, if you obtain `intermediate.crt` from the PKI authority and store its private key `intermediate.key`at the location `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`, then the `user_config` file should look like the following:
+
NOTE: If `intermediate.crt` is used to sign the `ontap_mediator_server.crt`, the  `intermediate.srl` file is generated. Refer to the note in <<step_2_intermediate_info, Step 2: Generate a server certificate by signing with a third-party CA certification>> for more information.
...
.. If you are using a certificate structure where the `root_ca.crt` certificate provides an `intermediate.crt` certificate that signs the `ontap_mediator_server.crt` certificate, connect the two top-level certificates to create a single certificate:
+
NOTE: You should have obtained both top-level certificates from the PKI earlier in the procedure.
+
