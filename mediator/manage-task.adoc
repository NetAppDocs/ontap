...
[NOTE]
====
* For ONTAP Mediator 1.8 and previous versions, the CA files used are: `ca.key`, `ca.csr`, `ca.srl`, and `ca.crt`. Since ONTAP Mediator 1.9, the required files are `intermediate.key`, `intermediate.csr`, `intermediate.srl`, and `intermediate.crt`. 
