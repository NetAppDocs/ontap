...
You can replace self-signed certificates with trusted third-party certificates.

.About this task

* Third-party certificates are *only* supported with ONTAP Mediator on systems running ONTAP 9.12.1P15, ONTAP 9.14.1P8, and ONTAP 9.15.1P2.
* You perform this task on the Linux host on which the ONTAP Mediator service is installed.
...
+
The PKI authority verifies the request and signs the `.csr`, generating the certificate `ca.crt`. You also need to obtain the `root_ca.crt` certificate that signs the `ca.crt` certificate from the PKI.
+
...
. To generate a server certificate from the CSR, run the following command from the folder `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`: 
+
...
+
For example, if you obtain `intermediate.crt` from the PKI authority and store its private key `intermediate.key` and certificate signing request `intermediate.csr` at a location `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`, then the user_config file should look like the following:
+
-----
...
+
NOTE: On ONTAP clusters, you also need to add the `intermediate.crt` and `root_ca.crt` to the cluster. 

.. Make sure that the following attributes of the newly generated certificates are set correctly:
+
