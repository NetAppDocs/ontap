...
.. If any of the certificate names changes, update the `/opt/netapp/lib/ontap_mediator/uwsgi/ontap_mediator.ini` file. The three variables that need to be updated are as follows:
+
...
`set-placeholder = ca_certificate = /opt/netapp/lib/ontap_mediator/ontap_mediator/server_config/intermediate.crt`
+
* `mediator_cert` has the same value as `cert_path` in the `ontap_mediator.user_config.yaml` file
* `mediator_key` has the same value as key_path in the `ontap_mediator.user_config.yaml` file
* `ca_certificate` has the same value as `ca_cert_path` in the `ontap_mediator.user_config.yaml` file
+
