...
[NOTE]
====
* From June 15th, 2025, you can't install or upgrade to ONTAP Mediator 1.9 and 1.8 because the code signature verification certificates have expired. Instead, install or upgrade to ONTAP Mediator 1.10 or later.
...
a| `ontap-mediator-1.12.1`               a| The product installation executable for version 1.12.1
a| `ontap-mediator-1.12.1.sig`           a| The SHA-256 hashed, then RSA-signed using the csc-prod key, signature for the installer
a| `ontap-mediator-1.12.1.sig.tsr`       a| The revocation request for use by OCSCP for the installer’s signature
a|`ontap-mediator-1.12.1.tsr`            a| The timestamp signing request file
...
. Verify the `ontap-mediator-1.12.1.sig.tsr` and `ontap-mediator-1.12.1.tsr` files using the associated certificates: 
+
