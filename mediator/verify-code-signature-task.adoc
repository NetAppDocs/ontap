...
a| `ontap-mediator-1.8.0`               a| The product installation executable for version 1.8.0
a| `ontap-mediator-1.8.0.sig`           a| The SHA-256 hashed, then RSA-signed using the csc-prod key, signature for the installer
a| `ontap-mediator-1.8.0.sig.tsr`       a| The revocation request for use by OCSCP for the installer’s signature
a|`ontap-mediator-1.8.0.tsr`            a| The timestamp signing request file
...
. Verify the `ontap-mediator-1.8.0.sig.tsr` and `ontap-mediator-1.8.0.tsr` files using the associated certificates: 
+
`openssl ts -verify`
