...
a| `ontap-mediator-1.11`               a| The product installation executable for version 1.11
a| `ontap-mediator-1.11.sig`           a| The SHA-256 hashed, then RSA-signed using the csc-prod key, signature for the installer
a| `ontap-mediator-1.11.sig.tsr`       a| The revocation request for use by OCSCP for the installer’s signature
a|`ontap-mediator-1.11.tsr`            a| The timestamp signing request file
...
. Verify the `ontap-mediator-1.11.sig.tsr` and `ontap-mediator-1.11.tsr` files using the associated certificates: 
+
...
.OpenSSL 3.x
--
root@sdot-r730-0003a-d6:~/ontap-mediator-1.11.0# openssl ts -verify -data ontap-mediator-1.11.0.sig -in ontap-mediator-1.11.0.sig.tsr -CAfile tsa-prod-chain-ONTAP-Mediator.pem -untrusted tsa-prod-ONTAP-Mediator.pem
Using configuration from /etc/pki/tls/openssl.cnf
Warning: certificate from 'tsa-prod-ONTAP-Mediator.pem' with subject '/C=GB/ST=West Yorkshire/O=Sectigo Limited/CN=Sectigo Public Time Stamping Signer R36' is not a CA cert
Verification: OK
root@sdot-r730-0003a-d6:~/ontap-mediator-1.11.0# openssl ts -verify -data ontap-mediator-1.11.0 -in ontap-mediator-1.11.0.tsr -CAfile tsa-prod-chain-ONTAP-Mediator.pem -untrusted tsa-prod-ONTAP-Mediator.pem
Using configuration from /etc/pki/tls/openssl.cnf
Warning: certificate from 'tsa-prod-ONTAP-Mediator.pem' with subject '/C=GB/ST=West Yorkshire/O=Sectigo Limited/CN=Sectigo Public Time Stamping Signer R36' is not a CA cert
Verification: OK
root@sdot-r730-0003a-d6:~/ontap-mediator-1.11.0#
--
.OpenSSL 1.x
--
[root@scs000791806 ontap-mediator-1.11.0]# openssl ts -verify -data ontap-mediator-1.11.0.sig -in ontap-mediator-1.11.0.sig.tsr -CAfile tsa-prod-chain-ONTAP-Mediator.pem -partial_chain
Using configuration from /etc/pki/tls/openssl.cnf
Verification: OK
[root@scs000791806 ontap-mediator-1.11.0]# openssl ts -verify -data ontap-mediator-1.11.0 -in ontap-mediator-1.11.0.tsr -CAfile tsa-prod-chain-ONTAP-Mediator.pem -partial_chain
Using configuration from /etc/pki/tls/openssl.cnf
Verification: OK
[root@scs000791806 ontap-mediator-1.11.0]#
--
====
+
...
+
----
openssl dgst -sha256 -verify ONTAP-Mediator-production.pub -signature ontap-mediator-1.11.sig ontap-mediator-1.11
----
