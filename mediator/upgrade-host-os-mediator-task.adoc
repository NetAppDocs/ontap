...
Back up log files before upgrading the OS and restore them after reinstalling ONTAP Mediator. Reconnect ONTAP clusters to ONTAP Mediator after installation.

NOTE: Perform the steps in order. After reinstalling ONTAP Mediator, stop ontap_mediator, replace log files, and restart it to prevent log loss.
...
+
....
[rootmediator-host ~]# ontap-mediator-1.11/ontap-mediator-1.11
...
For SnapMirror active sync, you do not need to reinstall TLS certificates stored outside /opt/netapp. Back up and restore certificates stored in /opt/netapp.
...
////
The following table shows the upgrade steps if you are upgrading from RHEL/CentOS 7.6 to a newer RHEL/CentOS release and also upgrading the ONTAP Mediator version.
...
.. Upgrade the operating system from RHEL/CentOS version.
.. Reboot the host to apply the kernel module changes.
If you do not reboot the host, an error message appears and prompts you to reboot.
.. Upgrade the Mediator from the immediately prior version to the current version.
a|
