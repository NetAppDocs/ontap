...
* To install ONTAP Mediator 1.12 or later with support for HTTPS and iSCSI, respond `y` to the following prompt: 
+
...
+
To install ONTAP Mediator 1.12 or later with support for HTTPS only , respond `n` to the prompt. Alternatively, you can use the `--https-only` option in the `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y` command.
