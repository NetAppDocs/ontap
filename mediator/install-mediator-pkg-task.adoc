...
[CAUTION] 
====
* In MetroCluster IP configurations, when ONTAP Mediator is configured with the HTTPS protocol, a single ONTAP Mediator instance can only monitor one MetroCluster IP configuration. Monitoring multiple MetroCluster IP configurations simultaneously is not supported. 
...
** Respond `No` if you plan to only use ONTAP Mediator for SnapMirror active sync, or if you plan to configure ONTAP Mediator with HTTPS to monitor a single MetroCluster IP configuration per ONTAP Mediator instance.
+
