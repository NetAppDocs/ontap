...
** Respond `Yes` to install ONTAP Mediator with support for HTTPS and iSCSI if you plan to use ONTAP Mediator configured with iSCSI to monitor one or more MetroCluster IP configuration. 

** Respond `No` to install ONTAP Mediator with support for HTTPS only if you plan to only use ONTAP Mediator for SnapMirror active sync, or if you plan to use an ONTAP Mediator configured with HTTPS to monitor a single MetroCluster IP configuration.
+
