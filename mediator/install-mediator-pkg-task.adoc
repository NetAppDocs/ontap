...
* When you install or upgrade to ONTAP Mediator 1.12, you have the option to install ONTAP Mediator with iSCSI and HTTPS support, or HTTPS support only. If you choose to install ONTAP Mediator with HTTPS support only, the SCST package is not installed. 
...
* You can install ONTAP Mediator with HTTPS support only if the following applies to your configuration:

** You are planning to use ONTAP Mediator with HTTPS to monitor your MetroCluster IP configuration, and you only want to use a single ONTAP Mediator to monitor one MetroCluster configuration.
+
CAUTION: When installed with HTTPS support only, ONTAP Mediator can only be used to monitor one MetroCluster IP configuration. Monitoring multiple configurations is not supported. You must carefully evaluate the requirements for your configuration before you choose to install ONTAP Mediator with HTTPS support only. 
...
* If you choose to install ONTAP Mediator with HTTPS support only, you can respond `Yes` to the prompts or use the `-https-only` option in the command. 
====
+
...
.Example ONTAP Mediator installation with HTTPS support only (console output)
[%collapsible]
...
.Example of ONTAP Mediator upgrade with HTTPS support only (console output)
[%collapsible]
...
NOTE: If you installed ONTAP Mediator 1.12 or later with support for HTTPS only, the SCST package is not installed so you can skip this task.
