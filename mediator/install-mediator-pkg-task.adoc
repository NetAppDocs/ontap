...
* When you install or upgrade to ONTAP Mediator 1.12, you have the option to install ONTAP Mediator with iSCSI and HTTPS support, or support for HTTPS only. If you choose to install ONTAP Mediator with support for HTTPS only, the SCST package is not installed. 
...
* If the following applies to your configuration, you can install ONTAP Mediator with support for HTTPS and not iSCSI:
...
* If you choose to install ONTAP Mediator with support for HTTPS only, you can respond `Yes` to the prompts or use the `-https-only` option in the command. 
====
+
