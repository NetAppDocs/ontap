...
a| 
* Support for RHEL:
** Compatible: 10.1 and 9.7.
** Recommended: 10.2, 10.0, 9.8, 9.6, 9.4, and 8.10.
* Support for Rocky Linux 10.2, 9.8, and 8.10.
* Support for Oracle Linux 10.1 and 9.7 with both rhck and uek kernels.
* Beginning with ONTAP Mediator 1.12.1 and ONTAP 9.19.1, a single ONTAP Mediator instance configured with HTTPS can monitor the following configurations simultaneously:

** A maximum of ten four-node and five eight-node MetroCluster IP configurations if installed on a host with 8-core CPU and at least 16GB of RAM. See link:https://review.docs.netapp.com/us-en/ontap-metrocluster_aoife-mcc-9191ga/install-ip/concept_mediator_requirements.html#mediator-multiple-mccip[Use ONTAP Mediator to monitor multiple MetroCluster IP configurations^].
** One four-node MetroCluster IP configuration and one SnapMirror active sync peer relationship if installed on a host OS with 2-core CPU. See link:../snapmirror-active-sync/mediator-install-task.html#ontap-mediator[Configure the ONTAP Mediator for SnapMirror active sync].
* Includes a new tool to to repair a broken SCST installation. After ONTAP Mediator is installed, the tool can be found at `/usr/local/bin/mediator_repair_scst`. The tool can be used after an OS upgrade if the upgraded OS requires an SCST patch that is not available in the default SCST package.
...
* Support for Rocky Linux 10.2, 9.8, and 8.10.
* Support for Oracle Linux 10.1 and 9.7 with both rhck and uek kernels.
* Support for configuring an ONTAP Mediator that uses HTTPS for communication in MetroCluster IP configurations. HTTPS provides increased security by encrypting entire sessions using SSL/TLS. You can choose to install ONTAP Mediator that only uses the HTTPS protocol. The SCST package is not installed if this installation method is used. link:index.html#https-iscsi-installation[Learn more about HTTPS and iSCSI installation options for ONTAP Mediator].

a| 1.11 
a| 
* Support for RHEL 10.1, 10.0, 9.7, 9.6, 9.4, and 8.10.
* Support for Rocky Linux 10.1, 9.7, and 8.10.
* Support for Oracle Linux 10.0 and 9.6 with both rhck and uek kernels.
* Adds support for IPv6 for MetroCluster IP configurations.
* Adds support for fapolicyd.

a| 1.10 
a| 
* Support for RHEL 10.0, 9.6, 9.4, and 8.10.
...
h| OS for ONTAP Mediator h| 1.12.1 h| 1.12 h| 1.11 h| 1.10 h| 1.9.1 h| 1.9 h| 1.8 h| 1.7 h| 1.6 h| 1.5 h| 1.4 h| 1.3 h| 1.2 h| 1.1 h| 1.0
