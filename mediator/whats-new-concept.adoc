...
a| 6995122
a| Implemented changes to remediate hard-stop of ONTAP Mediator installation when a kernel mismatch is detected. These changes ensure that the installation process continues without any disruption and a warning message is issued when a kernel mismatch is detected. 


a| 6623764
a| Implemented changes to remediate hard-stop of the ONTAP Mediator installer on OpenSSL verification failures. The `scst` version inside the README file also gets updated.


a| 6912810
a| Added support for ONTAP Mediator health check events and ONTAP support operations.
