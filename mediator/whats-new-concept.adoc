...
a| 6995122
a| Implemented changes to remediate the disruption of the ONTAP Mediator installation when a kernel mismatch is detected. These changes ensure that the installation process continues without any disruption and a warning message is issued when a kernel mismatch is detected. 


a| 6623764
a| Implemented changes to remediate the disruption of the ONTAP Mediator installer on the OpenSSL verification failures. The `scst` version inside the README file also gets updated.
