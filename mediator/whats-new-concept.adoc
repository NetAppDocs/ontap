...
* Fixed gitleaks secrets scan findings in test scripts

* Implemented a fix to remediate the installation of ONTAP Mediator 1.6 failing issue in reboot_needed() because of rpm command failure 

* Implemented a fix to remediate the ONTAP Mediator 1.8 negative certificate validation hard-stop issue
...
* Added support for ONTAP Mediator health check events and ONTAP support operations 
* Upgraded the `scst` package to 3.8.0 
* Introduced a new script to validate certificates used by the ONTAP Mediator 1.8 
