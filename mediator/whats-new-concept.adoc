...
* Support for configuring an ONTAP Mediator that uses HTTPS for communication in MetroCluster IP configurations. HTTPS provides increased security by encrypting entire sessions using SSL/TLS.
* Support for installing an ONTAP Mediator that only uses the HTTPS protocol to monitor a MetroCluster IP configuration. The SCST package is not installed and only one MetroCluster IP configuration can be monitored if this installation method is used. For more information. link:index.html#https-iscsi-installation[Learn more about HTTPS and iSCSI installation options].
...
h| OS for ONTAP Mediator h| 1.12 h| 1.11 h| 1.10 h| 1.9.1 h| 1.9 h| 1.8 h| 1.7 h| 1.6 h| 1.5 h| 1.4 h| 1.3 h| 1.2 h| 1.1 h| 1.0
...
| ONTAP Mediator 1.12 | scst-3.9.tar.gz
