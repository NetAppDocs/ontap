...
* Support for monitoring multiple MetroCluster IP configurations using an ONTAP Mediator configured with HTTPS that is installed on a host OS running at least 8-core CPU and with at least 16GB RAM. Refer to
link:https://review.docs.netapp.com/us-en/ontap-metrocluster_aoife-mcc-9191ga/install-ip/concept_mediator_requirements.html#mediator-multiple-mccip[Use ONTAP Mediator to monitor multiple MetroCluster IP configurations^] for more information.
* Support for monitoring one four-node MetroCluster IP configuration and one SnapMirror active sync peer relationship using an ONTAP configured with HTTPS that is installed on a host OS running 2-core CPU. See link:../snapmirror-active-sync/mediator-install-task.html#ontap-mediator[Configure the ONTAP Mediator for SnapMirror active sync] for more information.
