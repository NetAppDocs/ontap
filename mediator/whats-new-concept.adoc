...
* Includes a new tool to repair a broken SCST installation. After ONTAP Mediator is installed, the tool can be found at `/usr/local/bin/mediator_repair_scst`. The tool can be used after an OS upgrade if the upgraded OS requires an SCST patch that is not available in the default SCST package.
