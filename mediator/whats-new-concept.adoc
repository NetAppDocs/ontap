...
* Support for Oracle Linux 10.1 and 9.8 with both rhck and uek kernels.
...
** A maximum of ten four-node or five eight-node MetroCluster IP configurations if installed on a host with an 8-core CPU and at least 16GB of RAM. See link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#mediator-multiple-mccip[Use ONTAP Mediator to monitor multiple MetroCluster IP configurations^].
** One four-node MetroCluster IP configuration and one SnapMirror active sync peer relationship if installed on a host with a 2-core CPU. See link:../snapmirror-active-sync/mediator-install-task.html#ontap-mediator[Configure the ONTAP Mediator for SnapMirror active sync].
* Includes a new tool to repair a broken SCST installation. After installing ONTAP Mediator, you can find the tool at `/usr/local/bin/mediator_repair_scst`. You can use the tool after an OS upgrade, if the upgraded OS requires an SCST patch that is not available in the default SCST package.
