...
* Support for RHEL FIPS.
* Support for Bulk GET/PATCH for mailboxes and Bulk LONGPOLL API on the mailbox (a variation of Bulk-GET API).
* The default self-signed certificate structure has been changed from a 2-tier structure to 3-tier structure. The CA certificate "ca.crt" is now replaced by "intermediate.crt" to be added to ONTAP clusters.
* Added performance enhancements for larger scalability.
* Improved filenames to simplify setting up PKI-signed certificates.
...
* "Compatible" means that RHEL no longer supports this version but ONTAP Mediator 1.9 can still be installed.
* "Recommended" OS versions are confirmed as supported by ONTAP Mediator 1.9.
