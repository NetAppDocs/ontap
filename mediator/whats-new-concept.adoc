...
* Support for installing an ONTAP Mediator that can only use the HTTPS protocol to monitor a MetroCluster IP configuration. The SCST package is not installed and only one MetroCluster IP configuration can be monitored if this installation method is used. link:index.html#https-iscsi-installation[Learn more about HTTPS and iSCSI installation options for MetroCluster IP].
