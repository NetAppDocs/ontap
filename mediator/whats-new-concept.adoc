...
* Support for monitoring multiple MetroCluster IP configurations using an ONTAP Mediator configured with HTTPS that is installed on a host OS running 8-core CPU and with at least 16GB RAM. Refer to
...
* Includes a new tool to to repair a broken SCST installation. After ONTAP Mediator is installed, the tool can be found at /usr/local/bin/mediator_repair_scst. The tool can be used after an OS upgrade if the upgraded OS requires an SCST patch upgrade that might not be available in the default SCST package.
