...
[NOTE]
The ONTAP Mediator might have dependencies on some python modules present in the "extras" and "optional" repositories, therefore, it might be required to access the `rhel-X-for-x86_64-extras-rpms` and `rhel-X-for-x86_64-optional-rpms` files.
