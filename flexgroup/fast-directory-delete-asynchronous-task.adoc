...
Beginning with ONTAP 9.17.1, you can use the the `volume async-delete show` command to check the status of in progress asynchronous delete jobs, including asynchronous delete jobs issued from clients.
...
....
. Check the status of the in-progress async delete jobs:
+
`volume async-delete show`
