...
* Converting FlexVol volumes to FlexGroup volumes is a non-disruptive operation. Conversion normally takes a few seconds, without taking the volume offline or remounting shares and exports, so clients can continue accessing data.
...
** With releases earlier than ONTAP 9.10.1, the SVM where the FlexVol volume to be converted is located is currently using SVM-DR. Beginning with ONTAP 9.10.1, you can link:convert-flexvol-svm-dr-relationship-task.html[convert a FlexVol volume to a FlexGroup volume on an SVM-DR source].
...
** Autonomous Ransomware Protection has been disabled on the volume. You should not enable it again until the conversion is complete.
...
// 2026-Sept-14, ONTAPDOC-4324
// 2026-July-27, ONTAPDOC-4066
// 2025-10-23, ONTAPDOC-3376
