...
[WARNING]
If you have clients with SMB Multichannel enabled, you must disable it before enabling advanced capacity balancing. Using SMB Multichannel with advanced capacity rebalancing can cause high latency and might result in data loss. For more information, see link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-Issues/CONTAP-400433[CONTAP-400433: High read/write latency when using FlexGroup Rebalancing/GDD over SMB Multichannel enabled clients].
