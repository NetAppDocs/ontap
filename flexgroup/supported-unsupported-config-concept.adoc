...
* Snapshot copy tagging: Support for creating, modifying and deleting Snapshot copy tags (SnapMirror label and comment) for Snapshot copies on FlexGroup volumes using the `volume snapshot` command.
