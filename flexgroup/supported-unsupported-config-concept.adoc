...
* Snapshot tagging: Support for creating, modifying and deleting snapshot tags (SnapMirror labels and comments) for Snapshot copies on FlexGroup volumes using the `volume snapshot` command.
