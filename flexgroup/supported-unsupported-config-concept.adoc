...
* xref:../performance-admin/adaptive-qos-policy-groups-task.html[Nested QoS policies] are supported for the following object pairs:

** SVMs and FlexGroup volumes contained by the SVM 
** FlexGroup volumes and qtrees within the volumes 
...
* xref:../anti-ransomware/index.html[Autonomous Ransomware Protection] for FlexGroup volumes, including the following supported functionality:
...
+
For more information, see the link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_To_Convert_a_Transitioned_FlexVol_to_FlexGroup[NetApp Knowledge Base: How To Convert a Transitioned FlexVol to FlexGroup^].
