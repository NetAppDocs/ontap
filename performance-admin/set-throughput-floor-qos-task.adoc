...
[.lead]
A QoS minimum or throughput floor guarantees that throughput for a workload does not fall below a minimum number of IOPS or MBps, or IOPS and MBps regardless of competing workloads. Set a throughput floor to ensure critical workloads are prioritized and meet minimum performance requirements. You can apply the policy group when you create or modify the storage object. 
