...
* Beginning in ONTAP 9.13.1, you can enforce throughput floors at the SVM level using an xref:adaptive-policy-group-template-task.html[adaptive policy group template]. You cannot set an adaptive policy group template on an SVM with a QoS policy group. 
