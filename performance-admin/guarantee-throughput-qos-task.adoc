...
Once an adaptive policy group template is set on the SVM, all volumes created or migrated to the SVM will inherit that policy unless a separate policy is specified on the individual volume.
