...
|===
 ...
 In some cases you cannot assign a storage object to a policy group if its containing object or its child objects belong to a policy group. Beginning with ONTAP 9.18.1, you can assign a QoS policy to an SVM and to volumes contained by the SVM at the same time. The following table lists the restrictions.
...
Any storage objects contained by the SVM to a policy group. If you are running ONTAP 9.18.1 and later, volumes contained by SVMs can be assigned.
a|
Volume to a policy group
a|
The volume's containing SVM or any child LUNs to a policy group. If you are running ONTAP 9.18.1 and later, the volume's containing SVM can be assigned.
a|
