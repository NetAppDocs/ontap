...
In some cases you cannot assign a storage object to a policy group if its containing object or its child objects belong to a policy group. Beginning with ONTAP 9.18.1, you can assign a QoS policy to an SVM and to volumes contained by the SVM at the same time. 
...
Any storage objects contained by the SVM. If you are running ONTAP 9.18.1, volumes contained by SVMs can be assigned.
a|
Volume to a policy group
a|
The SVM containing the volume or any child LUNs. If you are running ONTAP 9.18.1 and later, the SVM containing the volume can be assigned.
a|
