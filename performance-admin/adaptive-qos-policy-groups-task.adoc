...
== Storage object policy group assignment restrictions

In some cases you cannot assign a storage object to a policy group if its containing object or its child objects belong to a policy group. 

Beginning with ONTAP 9.18.1, you can use nested QoS policies, which allow assigning policy groups to both the containing object, such as an SVM, and its child object, such as a volume. In a multi-tenant environment, nested QoS policies enable administrators to subdivide the QoS limits on SVMs among the volumes and qtrees within the SVM, and provide the ability to balance storage resources across computing environments while also enabling prioritization of mission critical workloads.
