...
In some cases you cannot assign a storage object to a policy group if its containing object or its child objects belong to a policy group. However, beginning with ONTAP 9.18.1, you can assign policy groups to both the containing object and its child object. Nested QoS policies are supported for the following object pairs: 
