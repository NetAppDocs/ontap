...
* When you invoke the snapshot creation, you can set an optional timeout value of between 5 and 120 seconds. If no timeout value is provided, the operation times out at the default of 7 seconds. In the ONTAP REST API, set the timeout value with the `action_timeout` parameter. In the ONTAP CLI, use the `-timeout` flag. 

.Steps

You can complete a two-phase snapshot with the ONTAP REST API or, beginning with ONTAP 9.14.1, the ONTAP CLI. This operation is not supported in System Manager. 

[NOTE]
If you invoke the snapshot creation with the ONTAP REST API, you must commit the snapshot with the ONTAP REST API. If you invoke the snapshot creation with the ONTAP CLI, you must commit the snapshot with the ONTAP CLI. Mixing methods is not supported. 
