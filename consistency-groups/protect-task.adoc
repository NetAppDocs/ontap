...
+
`consistency-group snapshot start -vserver _svm_name_ -consistency-group _consistency_group_name_ -snapshot supersnap [-timeout _time_in_seconds_ -write-fence {true|false}]`
. Commit the Snapshot: 
+
`consistency-group snapshot commit _svm_name_ -consistency-group _consistency_group_name_ -snapshot supersnap`
--
====
