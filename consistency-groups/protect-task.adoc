...
== Set a remote protection policy

If you are utilizing remote protection, to ensure Snapshot copies of consistency groups created on your consistency group are copied to the destination, the policy labels in the source and destination cluster must match.

Remote protection policies can be configured using:
* xref:../data-protection/snapmirror-disaster-recovery-concept.html[Asynchronous SnapMirror] (beginning ONTAP 9.13.1)
* xref:../smbc/index.html[Snap Mirror Business Continuity] (beginning ONTAP 9.10.1)

.Considerations
* Asynchronous SnapMirror protection is only available for single consistency groups. It is not supported for hierarchical consistency groups. 
