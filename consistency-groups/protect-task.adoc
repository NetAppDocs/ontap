...
.CLI
--
CLI commands for consistency groups are available beginning in ONTAP 9.14.1.

[IMPORTANT]
In ONTAP 9.14.1, you cannot complete both phases of CG snapshot creation with the CLI.
+
When you initiate a two-phase CG snapshot in the ONTAP CLI, the CLI does not provide the UUID of the snapshot, which is required to complete the second step. You can initiate the snapshot commit from the CLI, but you must use the REST API to complete it. Alternately, you can initiate phase one from the REST API and use the CLI to complete the snapshot creation. 
