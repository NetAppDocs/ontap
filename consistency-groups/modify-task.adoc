...
* Beginning with ONTAP 9.13.1, you can non-disruptively add volumes to a consistency group with an active SnapMirror active sync or asynchronous SnapMirror protection policy.
...
* In ONTAP 9.12.1 and earlier, you _cannot_ add volumes to a consistency group in an SnapMirror active sync relationship. You must first break the SnapMirror active sync relationship, modify the consistency group, then restore protection with SnapMirror active sync.
...
.About this task
* You cannot remove volumes from a consistency group in a SnapMirror active sync or SVM disaster recovery relationship. You must first break the SnapMirror active sync relationship to modify the consistency group and then reestablish the relationship.
