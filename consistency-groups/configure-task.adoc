...
+
`consistency-group create -vserver _SVM_name_ -consistency-group _consistency-group-name_ -volume-prefix _prefix_for_new_volume_names_ -volume-count _number_ -size _size_ -export-policy _policy_name_`

.Create a consistency group with SAN volumes
. Create the consistency group:
+
`consistency-group create -vserver _SVM_name_ -consistency-group _consistency-group-name_ -lun _lun_name_ -size _size_ -lun-count _number_ -lun-os-type _LUN_operating_system_format_ -igroup _igroup_name_`
