...
--
.System Manager 9.17.1 and later
--
.Steps
...
.. For **New LUNs**: Select the host operating system and LUN format. Enter the host initiator information. Beginning with ONTAP 9.17.1, if you have a SnapMirror active sync configuration, you can select NVMe as the host type and configure host proximity using the SVM source, destination, or both. 
.. For **New NAS volumes**: choose the appropriate export option (NFS or SMB/CIFS) based on the NAS configuration of your SVM. 
.. For **New NVMe namespaces**: Select the host operating system and NVMe subsystem.
. To configure protection policies, add a child consistency group, or access permissions, select *More options*.
. Select *Save*.
