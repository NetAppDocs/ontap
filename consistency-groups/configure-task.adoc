...
.Considerations
* In ONTAP 9.10.1 through 9.12.1, you can create a consistency group using new LUNs or existing volumes. 
* Beginning in ONTAP 9.13.1, System Manager allows you to create a consistency group with new LUNs, new NVMe namespaces, and new NAS volumes in addition to existing volumes. (This is also supported in the ONTAP REST API beginning with ONTAP 9.12.1.)
* In ONTAP 9.10.1 through 9.11.1, modifying the member volumes of a consistency group is not supported.
+
