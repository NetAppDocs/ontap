...
Monitoring data is refreshed every five minutes is maintained for up to one year. You can track metrics for:
...
* link:../data-protection/snapmirror-svm-replication-concept.html[Asynchronous SVM data replication] (beginning 9.14.1)
...
[NOTE]
Snapshots created with the ONTAPI commands `cg-start` and `cg-commit` are recognized as consistency group Snapshots and thus cannot be managed through System Manager's consistency group interface or the consistency group endpoints in the ONTAP REST API. Beginning with ONTAP 9.14.1, these Snapshots can be mirrored to the destination volume if you are using an Asynchronous SnapMirror policy. For more information, see xref:protect-task.html#configure-asynchronous-snapmirror-protection[Configure asynchronous SnapMirror protection].
...
| Asynchronous SVM data replication | ✓ | | | |
| CLI support | ✓ | | | | |
|===
...
// 2023 17 august, ONTAPDOC-1152, ONTAPDOC-1280
