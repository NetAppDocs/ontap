...
Beginning with ONTAP 9.11.1,  you can provision consistency groups with new volumes on the local site (site A) in a MetroCluster configuration. These volumes must be placed on mirrored aggregates.
Consistency groups provisioned with existing volumes on site A can have all their volumes located on the mirrored aggregates, unmirrored aggregates, or both.
...
GET and DELETE operations on consistency groups which have some or all their associated volumes located on unmirrored aggregates that are not currently accessible, behave similarly to when local volumes associated with a consistency group or their hosting aggregates are offline.
