...
=== Co-existence with MetroCluster configurations

Beginning with ONTAP 9.11.1, consistency groups can be provisioned with new volumes on the local site (site A) in a MetroCluster configuration. These volumes must be placed on mirrored aggregates.

Consistency groups provisioned with existing volumes on site A can have all their volumes placed on the mirrored aggregates, unmirrored aggregates or both.

Volumes associated with consistency groups can be moved between mirrored and unmirrored aggregates, after they are provisioned.

Mirrored aggregates containing volumes associated with consistency groups can be modified to become unmirrored.  Similarly, unmirrored aggregates containing volumes associated with consistency groups can be modified to enable mirroring.

Volumes associated with consistency groups placed on mirrored aggregates and their snapshots, including any consistency group snapshots, are replicated to the remote site (site B). The contents of the volumes on site B are consistency group semantics-compliant.  Replicated consistency group Snapshots can be accessed using consistency group Snapshot REST APIs on clusters running ONTAP 9.11.1 or later.

Operations such as GET and DELETE on consistency groups with some or all their associated volumes are on unmirrored aggregates that are not currently accessible behave similar to when local volumes associated with a consistency group or their hosting aggregates are offline.
...
=== Different ONTAP versions running on site A and site B
If site B is running ONTAP 9.10.1 or earlier, none of the consistency groups on site A are replicated to site B. After site B is upgraded to ONTAP 9.11.1, data for consistency groups on site A, that have all their associated volumes placed on mirrored aggregates, are replicated to site B.
