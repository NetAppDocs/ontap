...
Volumes and snapshots associated with consistency groups placed on mirrored aggregates are replicated to the remote site (site B). The contents of the volumes on site B provide a write-order guarantee for the consistency group, allowing you to recover from site B in the event of a disaster. You can access consistency group snapshots using consistency group with the ONTAP REST API and System Manager on clusters running ONTAP 9.11.1 or later. Beginning with ONTAP 9.14.1, you can also access snapshots with the ONTAP CLI. 
...
Consistency group snapshots created in the ONTAP REST API can be managed through System Manager's Consistency Group interface and through consistency group ONTAP REST API endpoints. Beginning with ONTAP 9.14.1, consistency group snapshots can also be managed with the ONTAP CLI. 
