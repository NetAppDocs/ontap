...
Volumes associated with consistency groups placed on mirrored aggregates and their snapshots, including any consistency group snapshots, are replicated to remote site (site B). The contents of the volumes on site B are consistency group semantics-compliant.  Replicated consistency group snapshots can be accessed using consistency group Snapshot REST APIs on clusters running ONTAP 9.11.1 or later.
