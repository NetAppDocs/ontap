...
In the following example, an application is hosted on SVM1. The administrator has created a parent consistency group, `SVM1_app`, which includes two child consistency group: `SVM1appDataCG` containing the data, and `SVM`app_logCG` containing the logs. Each child consistency group has its own snapshopt policy. Snapshots of the data are taken every 15 minutes, while snapshots of the logs are taken hourly. The parent consistency group`SVM1_app` includes an SM-BC policy which replicates the data to a secondary cluster, which the consistency group will failover to in the event of a disruption. 
