...
Consistency groups offer application-consistent protection, ensuring consistency of your data across multiple volumes or LIFs. When creating a Snapshot copy of a consistency group, a "fence" is established on the consistency group. This fence briefly halts I/O until after the Snapshot operation completes, ensuring point-in-time consistency of data across all entities in the consistency group. The fencing operation can cause a transient spike in latency. 
