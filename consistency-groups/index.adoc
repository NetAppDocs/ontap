...
After they are provisioned, volumes associated with consistency groups can be moved between mirrored and unmirrored aggregates. Therefore, they can be located on mirrored aggregates, unmirrored aggregates, or both.
...
Volumes associated with consistency groups placed on mirrored aggregates and their snapshots, including any consistency group snapshots, are replicated to the remote site (site B). The contents of the volumes on site B are consistency group semantics-compliant. Replicated consistency group Snapshots can be accessed using consistency group Snapshot REST APIs and System Manager on clusters running ONTAP 9.11.1 or later.
...
>>>>>>> 948c2703f0a377cf32d4663b1ab9bd9f5772a220
