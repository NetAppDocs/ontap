...
GET and DELETE operations on consistency groups which some or all their associated volumes located on unmirrored aggregates that are not currently accessible, behave similar to when local volumes associated with a consistency group or their hosting aggregates are offline.
