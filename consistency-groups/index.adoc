...
[.lead]
A consistency group is a collection of volumes that are a managed as a single unit. In ONTAP, consistency groups provide easy management and a protection guarantee for an application workload spanning multiple volumes.
...
Beginning in ONTAP 9.13.1, you can monitor IOPs for individual consistency groups. 
...
* Local protection through Snapshot policies (beginning 9.10.1)
* Remote protection with:
** xref:../smbc/index.html[SnapMirror Business Continuity (SM-BC)] (beginning 9.10.1)
** <<mcc>> (beginning 9.11.1)
** TBD (beginning 9.13.1)
...
| Remote protection with SM-BC | X | X | X | X |
