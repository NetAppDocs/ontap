...
[.lead]
ONTAP provides several approaches a customer might use to achieve file-level ABAC, including NFS v4.2 security labels and extended attributed (xattrs) using NFS and SMB/CIFS.

== NFS v4.2 security labels

Beginning with ONTAP 9.9.1, the NFSv4.2 feature called Labeled NFS is supported. 

NFS v4.2 security labels are a way to manage granular file and folder access by using SELinux labels and Mandatory Access Control (MAC). These MAC labels are stored with files and folders, and they work in conjunction with UNIX permissions and NFSv4.x ACLs. 
...
=== Example NFS v4.2 security labels configurations

.Example configuration of NFS v4.2 security labels
...
.Example of NFS v4.2 security labels with MCS
