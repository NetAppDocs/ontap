...
summary: "ONTAP provides varied approaches a customer might use to achieve file-level ABAC, including NFS 4.2 security labels and xattrs using NFS and SMB/CIFS."
---
...
[.lead]
ONTAP provides several approaches a customer might use to achieve file-level ABAC, including NFS 4.2 security labels and extended attributed (xattrs) using NFS and SMB/CIFS.

== NFS 4.2 security labels

Beginning with ONTAP 9.9.1, the NFSv4.2 feature called Labeled NFS is supported. 

NFS 4.2 security labels are a way to manage granular file and folder access by using SELinux labels and Mandatory Access Control (MAC). These MAC labels are stored with files and folders, and they work in conjunction with UNIX permissions and NFSv4.x ACLs. 

Support for NFS 4.2 security labels means that ONTAP now recognizes and understands the NFS client's SELinux label settings. NFS 4.2 security labels are covered in RFC-7204.

Use Cases for NFS 4.2 security labels include the following:
...
.Enable NFS 4.2 security labels

You can enable or disable NFS 4.2 security labels with the following command (advanced privilege required):
...
.Enforcement modes for NFS 4.2 security labels
...
=== Example NFS 4.2 security labels configurations

.Example configuration of NFS 4.2 security labels
...
.Example of NFS 4.2 security labels with MCS
...
NOTE: In parallel to this content, NetApp has a reference implementation using GreyBox. One assumption for this content is that the customer's identity, authentication, and access services include at minimum a Policy Enforcement Point (PEP) and a Policy Decision Point (PDP) that act as intermediaries for access to the file system.

//todo: is the above relevant to a general audience?
//todo: find examples of a PEP and PDP tool/service? there was an example around a python script for PEP and SELinux for PDP, but I deleted it because it was missing a lot of context and was out of place (referring to 'the provided Python script' when no Python script is provided).
...
[NOTE]
NFS 4.2 security labels have the caveats defined in <<NFS 4.2 security labels>>.
...
NOTE: These examples are for illustrative purposes, and it is the customer's responsibility to determine the metadata associated with NFS 4.2 security labels and xattrs. Details on updating and label retention are omitted for simplicity.
...
* NFS 4.2 security labels: Utilized for making security decisions 
