...
[NOTE]
====
You can use the `security key-manager key query -key-type NSE-AK` command to view key IDs. Learn more about `security key-manager key query` in the link:https://docs.netapp.com/us-en/ontap-cli/security-key-manager-key-query.html[ONTAP command reference^].
====
+
----
