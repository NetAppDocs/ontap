...
Volumes
a|
Data volumes and existing root volumes. You cannot encrypt data on MetroCluster metadata volumes. In versions of ONTAP earlier than 9.14.1, you can encrypt data on the SVM root volume. Beginning with ONTAP 9.14.1, ONTAP supports xref:configure-nve-svm-root-task.html[NVE on SVM root volumes]. 
a|
Aggregate-level encryption
a|
