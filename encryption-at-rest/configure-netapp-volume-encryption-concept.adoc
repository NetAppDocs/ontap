...
---
= Learn about ONTAP NetApp Volume Encryption
:icons: font
:imagesdir: ../media/
...
FlexGroup volumes
a|
Beginning with ONTAP 9.2, FlexGroup volumes are supported. Destination aggregates must be of the same type as source aggregates, either volume-level or aggregate-level. Beginning with ONTAP 9.5, in-place rekey of FlexGroup volumes is supported.
a|
7-Mode transition
a|
