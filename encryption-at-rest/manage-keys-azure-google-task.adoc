...
. Before you begin, you must obtain the appropriate authentication credentials from your Azure account, either a client secret or certificate. 
...
Beginning with ONTAP 9.19.1, use:
`security key-manager external azure create-config -config-name _config_name_ -client-id _client_id_ -tenant-id _tenant_id_ -name URL -key-id _key_id_ -authentication-method {certificate|client-secret} -vserver _vserver_name_`
followed by:
`security key-manager keystore enable -vserver _vserver_name_ -keystore-type AKV -config-name _config_name_`
For ONTAP 9.18.1 and earlier, use:
`security key-manager external azure enable -client-id _client_id_ -tenant-id _tenant_id_ -name _name_ -key-id _key_id_ -authentication-method {certificate|client-secret} -vserver _vserver_name_`
