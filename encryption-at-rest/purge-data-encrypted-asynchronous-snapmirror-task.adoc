...
.. Create a snapshot on the source volume in the SnapMirror asynchronous relationship:
+
`volume snapshot create -snapshot snapshot_name -vserver _SVM_name_ -volume _volume_name_`
.. Update the SnapMirror relationship to move the base snapshot forward to the destination:
+
