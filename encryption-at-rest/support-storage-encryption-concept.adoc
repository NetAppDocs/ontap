...
* Starting with ONTAP 9.6, self-encrypting NVMe drives that have not undergone FIPS testing are supported on AFF A800 and A320 systems. These drives, called _SEDs,_ offer the same encryption capabilities as FIPS drives, but can be mixed with non-SEDs on the same node or HA pair.
