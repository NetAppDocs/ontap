...
* Delete the external key manager database if you use NSE with an external KMIP server. For details, see link:delete-key-management-database-task.html[Transition from external key management to ONTAP onboard key management].
...
+
The following ONTAP 9.6 command synchronizes the keys in the onboard key hierarchy:
+
----
...
+
Learn more about `security key-manager setup` in the link:https://docs.netapp.com/us-en/ontap-cli-9161/security-key-manager-setup.html[ONTAP command reference^].
