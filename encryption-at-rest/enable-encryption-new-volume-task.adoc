...
* Beginning with ONTAP 9.7, newly created volumes are encrypted by default when you have the link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/install-license-task.html[VE license] and onboard or external key management. By default, new volumes created in an NAE aggregate will be of type NAE rather than NVE. 
...
|===
+
For complete command syntax, refer to the command reference page for link:https://docs.netapp.com/us-en/ontap-cli/volume-create.html[`volume create`^].
