...
.Before you begin
* Barbican KMS and Keystone must be configured. The SVM you are using with Barbican must have network access to the Barbican and Keystone servers.
* If you are using a custom Certificate Authority (CA) for the Barbican and Keystone servers, you must install the CA certificate with `security certificate install -type server-ca -vserver <admin_svm>`.
