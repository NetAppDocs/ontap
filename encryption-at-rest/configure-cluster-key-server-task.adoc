...
`security key-manager external modify-server -vserver <svm_name> -key-servers <primary_key_server> -secondary-key-servers <list_of_key_servers>`
+
* Do not include a port number for secondary key servers for the `-secondary-key-servers` parameter. It uses the same port number as the primary key servers.
+
