...
[.lead]
Beginning with ONTAP 9.11.1, you can configure connectivity to clustered external key management servers on an SVM. With clustered key servers, you can designate primary and secondary key servers on an SVM. When registering keys, ONTAP will first attempt to access the primary key server before sequentially attempting to access secondary servers until the operation completes successfully, preventing duplication of keys. 

External key servers can be used for NetApp Storage Encryption (NSE), NetApp Volume Encryption, and NetApp Aggregate Encryption (NAE) keys. An SVM can support up to four primary external KMIP servers. Each primary server can support up to three secondary key servers. 
...
. Confirm that no key management has been enabled for the SVM:
+
...
+
* You cannot provide a port number for the `-secondary-key-servers` parameter. It uses the same port number as the primary key server.
+
NOTE: If you are running this command for the admin SVM in a MetroCluster configuration, you must run the command on both clusters. If you are running the command for an individual data SVM, you don't need to run the command on both clusters. NetApp strongly recommends that you use the same key servers on both clusters.
...
`security key-manager external modify-server -vserver <svm_name> -key-servers <primary_key_server> -secondary-key-servers <list_of_key_servers>`
+
* You cannot provide a port number for the `-secondary-key-servers` parameter. It uses the same port number as the primary key servers.
+
NOTE: If you are running this command for the admin SVM in a MetroCluster configuration, you must run the command on both clusters. If you are running the command for an individual data SVM, you don't need to run the command on both clusters. NetApp strongly recommends that you use the same key servers on both clusters.
...
You can modify external key servers clusters by changing the designation (primary or secondary) of particular key servers, add and removing secondary key servers, or by changing the access order of secondary key servers. If you modify clustered key servers in a MetroCluster configuration, NetApp strongly recommends that you use the same key servers on both clusters.
