...
You can modify clustered external key servers by changing the designation (primary or secondary) of particular key servers, adding and removing secondary key servers, or by changing the access order of secondary key servers. If you modify clustered external key servers in a MetroCluster configuration, NetApp strongly recommends that you use the same key servers on both clusters.
