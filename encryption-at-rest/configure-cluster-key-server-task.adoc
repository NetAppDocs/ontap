...
If the vserver already has the maximum of four primary key servers enabled, you must remove one of the existing primary key servers before adding a new one.  
...
For additional information, refer to the `security key-manager external` command reference. 
