...
. Perform the <<Create a clustered key server>> procedure to add a secondary key server to an existing primary key server.
...
. Perform the <<Create a clustered key server>> procedure and follow the steps to add a primary key server.
