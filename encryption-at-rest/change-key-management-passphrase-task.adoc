...
= Change the ONTAP Onboard Key Manager passphrase

:icons: font
:imagesdir: ../media/

[.lead]
You should change the Onboard Key Manager passphrase regularly. You must store the new passphrase in a secure location outside the storage system.
...
. Change the Onboard Key Manager passphrase. The command you use depends on the ONTAP version you are running.
+
[role="tabbed-block"]
====
...
. If you are in a MetroCluster configuration, perform the following steps to synchronize the passphrase on the partner cluster:
..  Run the following command on the partner cluster:
+
The command you use depends on the ONTAP version you are running.
+
[role="tabbed-block"]
====
...
.After you finish
Copy the Onboard Key Manager passphrase to a secure location outside the storage system for future use.

Back up key management information manually whenever you change the Onboard Key Manager passphrase.
