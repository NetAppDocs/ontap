...
The Onboard Key Manager stores and manages keys in a secure database that is internal to the cluster. Its scope is the cluster. An external key manager stores and manages keys outside the cluster. Its scope can be the cluster or the storage VM. 

.About this task
You must use the ONTAP command line interface (CLI) to configure external key managers for MetroCluster configurations. Learn more about link:configure-external-key-management-concept.html[configuring ONTAP external key management using the CLI].


One or more external key managers can be used. The following conditions apply:
...
// 2026 Mar 23, Git Issue 1997
// 2026 Mar 16, Git Issue 1997
