...
.About this task
You must use the ONTAP command line interface (CLI) to configure external key managers for MetroCluster configurations. Learn about link:configure-external-key-management-concept.html[configuring ONTAP external key management using the CLI].
