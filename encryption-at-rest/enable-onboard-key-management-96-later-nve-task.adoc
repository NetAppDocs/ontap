...
* After the Onboard Key Manager has been configured on both sites, you can run the `volume encryption conversion start` command to convert plain text volumes to encrypted volumes.
...
All key management information, except the passphrase and backup data, is automatically backed up to the replicated database (RDB) for the cluster.
