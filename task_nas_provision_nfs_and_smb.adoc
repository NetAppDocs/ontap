...
You can create FlexVol volumes, or for large file systems with high performance requirements, you can create FlexGroup volumes.  See  link:task_nas_provision_flexgroup.html[Provision NAS storage for large file systems using FlexGroup volumes].
...
| System Manager Classic (ONTAP 9.7 and earlier) | https://docs.netapp.com/us-en/ontap-sm-classic/nas-multiprotocol-config/index.html[SMB and NFS multiprotocol configuration overview^]
a| The ONTAP command line interface a| * link:smb-config/index.html[SMB configuration overview with the CLI]
* link:nfs-config/index.html[NFS configuration overview with the CLI]
* link:nfs-admin/security-styles-their-effects-concept.html[What the security styles and their effects are]
