...
. Add a new volume in a storage VM enabled for both NFS and SMB/CIFS.
...
. [[step2,Step 2 in the workflow]] On a Linux client, verify that the export is accessible.
...
NOTE: After you save the volume, return to <<step2>> to complete multiprotocol provisioning for Windows and Linux servers.
