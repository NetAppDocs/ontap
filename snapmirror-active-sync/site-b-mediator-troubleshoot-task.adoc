...
. Unmap all LUNs belonging to the consistency group from Site B. This will fail, so first the igroup must be modified to remove the replication peer vserver and then proceed to delete the lun map

C1_sti233-vsim-sr092a_cluster::*> lun mapping show
...
C1_sti233-vsim-sr092a_cluster::*> lun mapping delete -path /vol/cg1_lun/lun_5 -igroup igroup1
Error: command failed: The peer cluster is unreachable and a SnapMirror
       ...
       supported.
C1_sti233-vsim-sr092a_cluster::*> igroup modify -igroup igroup1 -replication-peer -

C1_sti233-vsim-sr092a_cluster::*> lun mapping delete -path /vol/cg1_lun/lun_1 -igroup igroup1
