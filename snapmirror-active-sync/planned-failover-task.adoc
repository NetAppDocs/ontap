...
You might want to perform a planned failover to test the health of your disaster recovery configuration or to perform maintenance on the primary cluster. 
...
. To monitor the status of the failover, use the `snapmirror failover show` command in the ONTAP CLI. 
--
...
+
Example failing over a SAN configuration:
+
destination::>snapmirror failover start -destination-path   vs1_dst:/cg/cg_dst
+
Example failing over a NAS configuration:
+
destination::>snapmirror failover start -destination-path  vs_b:


. From the destination, monitor the progress of the failover:
+
