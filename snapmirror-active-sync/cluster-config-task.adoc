...
. At least one SAN LIF (either FC or iSCSI as applicable) for your SAN configuration is created on each node in both clusters, or at least one NAS LIF (either NFS or SMB as applicable) is created on each node in the primary cluster for replication to the secondary cluster.
+
...
+
link:../nfs-config/index.html[Learn about NFS configuration]
+
link:../smb-config/index.html[Learn about SMB configuration]

. SAN only: The necessary LUNs are created and mapped to an igroup, which is used to map LUNs to the initiator on the application host.
+
link:../san-admin/provision-storage.html[Create LUNs and map igroups]

. The application host is re-scanned to discover any new LUNs or volumes.
