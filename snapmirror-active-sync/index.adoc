...
Available beginning with ONTAP 9.9.1, SnapMirror active sync is supported on AFF clusters, All-Flash SAN Array (ASA) clusters, C-Series (AFF or ASA). Primary and secondary clusters must be of the same type: either ASA or AFF. SnapMirror active sync protects applications with iSCSI or FCP LUNs.
