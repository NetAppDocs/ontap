...
Beginning with ONTAP 9.15.1, SnapMirror active sync supports a xref:archicture-concept.html#symmetric-active-active[symmetric active/active capability], enabling read and write I/O operations from both copies of a protected LUN with bidirectional synchronous replication, enabling both LUN copies to serve I/O operations locally.  
