...
Available beginning with ONTAP 9.9.1, SnapMirror active sync is supported on AFF and ASA clusters. The primary and secondary clusters must be of the same type: either AFF, ASA, or ASA r2. SnapMirror active sync protects applications with iSCSI or FCP LUNs or NVMe namespaces.
...
Beginning with ONTAP 9.19.1, SnapMirror active sync supports NAS workloads at the SVM level using NFS (v3 and v4), and SMB (2.x or later), on AFF clusters (2-node only) or AFX clusters (4-node only) and read-write access only on the primary. The secondary SVM and the volumes it contains are not accessible using NAS protocols, but you can clone the secondary volumes into a different SVM to provide access to them. 
...
.SVM and FlexVol support (SnapMirror active sync for NAS)
...
* **Non-uniform host access** means hosts from each site are connected only to the cluster at the same site. Cross-site paths and stretched paths aren't connected. 
