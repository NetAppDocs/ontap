...
[NOTE]
NVMe is not supported with asymmetric active/active configurations
...
* Asymmetric active/active
* Symmetric active/active
Symmetric active/active configurations support 2-node clusters in ONTAP 9.15.1.  4-node clusters are supported in ONTAP 9.16.1 and later with the iSCSI protocol.
...
* LUN, namespace, or storage unit identity remains the same, so the application sees them as a shared virtual device.
