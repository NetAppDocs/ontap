...
* Consistency groups (which define the failover units in a SnapMirror active sync SAN relationship); for each consistency group, the following information is provided: 
...
Primary-first and primary bias are supported in SnapMirror active sync SAN configurations beginning with ONTAP 9.15.1. Primary copies are designated in System Manager and output with the REST API and CLI. 
