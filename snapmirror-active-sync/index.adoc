...
[.lead]
SnapMirror active sync (referred to as SnapMirror Business Continuity prior to ONTAP 9.15.1) allows business services to continue functioning in the event of a complete site failure. This technology enables applications to failover seamlessly to a secondary copy without manual intervention or custom scripting.

SnapMirror active sync (SM-as) is designed to be a more granular, lower-cost, easier-to-use application-level protection with automatic failover. With SnapMirror active sync, you can synchronously replicate multiple volumes of an application between sites at geographically dispersed locations. You can automatically failover to the secondary copy in case of disruption of the primary, thus enabling business continuity for tier one applications.
...
Available beginning with ONTAP 9.9.1, SnapMirror active sync is supported on AFF and ASA clusters. The primary and secondary clusters must be of the same type: either ASA, ASA r2, or AFF. SnapMirror active sync protects applications with iSCSI or FCP LUNs or NVMe namespaces.

SnapMirror active sync for SAN workloads supports both symmetric and asymmetric configuration. ONTAP 9.9.1 and later includes support for asymmetric configuration. Support for symmetric active/active was introduced in ONTAP 9.15.1. Symmetric active/active configuration allows both copies of a protected LUN to perform read and write I/O operations with bidirectional synchronous replication, enabling each LUN copy to serve local I/O requests.

Beginning with ONTAP 9.19.1, SnapMirror active sync supports NAS workloads at the SVM level using NFS v3 and v4.x, or SMB 2.x and later, on AFF clusters (2-node only) or AFX clusters (4-node only) and read-write access only on the primary. The secondary SVM and the volumes it contains are not accessible using NAS protocols, but you can clone the secondary volumes into a different SVM to provide access to them. 
...
* LUN, NVMe namespace, NVMe subsystem, or storage unit identity for SAN configurations and volume identity for NAS configurations, remains the same, so the application sees them as a shared virtual device.
...
SnapMirror active sync enables you to protect your data LUNs or NVMe namespaces in SAN configurations and volumes in NAS configurations, which enables applications to fail over transparently for the purpose of business continuity in the event of a disaster. For more information, see link:use-cases-concept.html[Use cases].  

== Key concepts

SnapMirror active sync uses consistency groups in SAN configurations and SVMs in NAS configurations to ensure your data is replicated. SnapMirror active sync uses the ONTAP Mediator or, beginning with ONTAP 9.17.1, the Cloud Mediator for automated failover, ensuring the data is served in the event of a disaster scenario. When planning your SnapMirror active sync deployment, it is important to understand the essential concepts in SnapMirror active sync and its architecture. 

.Asymmetry and symmetry 

In symmetric active/active SAN cluster configurations, both sites can access local storage for active I/O.  Symmetric active/active is optimized for clustered applications including VMware vMSC, Windows Failover Cluster with SQL, and Oracle RAC. 

In asymmetric active/active SAN configurations, data on the secondary site is proxied to a LUN, namespace, or storage unit. 

For more information, see xref:architecture-concept.html[SnapMirror active sync architecture].

In NAS configurations there is no proxy and the secondary SVM is dormant, but you can clone the volumes to another SVM to provide access to them.

.Consistency group (SAN-only)
...
The purpose of a consistency group is to take simultaneous coordinated snapshot images of a collection of volumes or storage units, thus ensuring crash-consistent copies of the collection at a point in time. A consistency group ensures all volumes of a dataset are quiesced and then snapped at precisely the same point in time. This provides a data-consistent restore point across volumes or storage units supporting the dataset. A consistency group thereby maintains dependent write-order consistency. If you decide to protect applications for business continuity, the group of volumes or storage units corresponding to this application must be added to a consistency group so a data protection relationship is established between a source and a destination consistency group. The source and destination consistency must contain the same number and type of volumes.  

In ONTAP 9.19.1, SnapMirror active sync for NAS supports protection of SVMs that contain FlexVol volumes. Consistency groups or FlexGroup volumes within an SVM are not supported by SnapMirror active sync in NAS configurations. 

.Constituent

In a SAN configuration, a constituent is an individual volume, LUN, or NVMe namespace (beginning with ONTAP 9.17.1) that is part of the consistency group protected in the SnapMirror active sync relationship. 

In a NAS configuration, a constituent is a volume, and the containing SVM is the entity that is protected in the SnapMirror active sync relationship.
...
* Failover units (which correspond to consistency groups in a SnapMirror active sync for SAN relationship and to SVMs in a SnapMirror active sync for NAS relationship); for each failover unit, the following information is provided: 
...
Primary-first and primary bias are supported in SnapMirror active sync SAN configurations beginning with ONTAP 9.11.1. Primary copies are designated in System Manager and output with the REST API and CLI. 
...
* **Uniform host access** means that hosts from both sites are connected to all paths to storage clusters on both sites. Cross-site paths are stretched across distances. Uniform host access applies to SAN configurations only. 
...
The type of clusters, protocols, and configurations supported by SnapMirror active sync varies depending on your version of ONTAP. The table indicates the ONTAP version in which updates were added and the configurations that support those updates.

[cols=2*]
|===
h| ONTAP version h| Adds support for 
...
NAS protocols are supported on these cluster types:
 ...
 * NVMe for VMware host workloads

| 9.16.1 and later	
a|

* ASA r2, 2-node clusters


* Symmetric active/active 4-node to 4-node AFF or ASA clusters
+
Symmetric active/active configurations support 4-node to 4-node clusters in ONTAP 9.16.1 and later, in addition to 2-node to 2-node clusters. 
+
For ASA r2, only 2-node to 2-node clusters are supported.

| 9.15.1 and later 	
a| The following configuration type:

* Symmetric active/active
+
Symmetric active/active configurations support 2-node clusters in ONTAP 9.15.1.	
...
* link:prerequisites-reference.html[SnapMirror active sync prerequisites]
* link:limits-reference.html[SnapMirror active sync object limits]


**THIS IS THE OLD TABLE. TO BE REMOVED PRIOR TO PUBLICATION**
...
[NOTE]
Asymmetric active/passive does not support ASA r2 and NVMe.
...
* Asymmetric active/passive
* Symmetric active/active
Symmetric active/active configurations support 4-node to 4-node clusters in ONTAP 9.16.1 and later. For ASA r2, only 2-node clusters are supported.
...
* Symmetric active/active
Symmetric active/active configurations support 2-node clusters in ONTAP 9.15.1.  2-node to 2-node clusters and 4-node to 4-node clusters are supported in ONTAP 9.16.1 and later.
...
a| Asymmetric active/passive

|===



// 2026-May-21, ONTAPDOC-3544
// 2026-Apr-20, GitHub issue #2026
// 2026-Mar-11, ONTAPDOC-3621
// 2026-Feb-18, ONTAPDOC-3725
