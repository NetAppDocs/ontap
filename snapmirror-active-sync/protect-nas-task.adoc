...
| FabricPool
| Partially
| On the secondary SVM, File System Analytics is set to `Off`, and you must  manually set it to `On` after a failover.
...
You might want to exclude some volumes from a SnapMirror active sync NAS configuration if the volume type is not supported; for example, a FlexGroup or DP-type volume, or if you want to delete the volume, such as a test clone.

[NOTE] 
It is recommended that you rehost unprotected volumes to another SVM for easier management. 
Marking a currently protected volume as unprotected initiates a shrink operation on the SnapMirror active sync relationship. The shrink operation deletes SnapMirror entries for the volume on both the source and destination. Marking a volume as unprotected does not automatically delete the volume on the primary or secondary relationship. If you want to delete the volume, you can manually delete it on both the primary and secondary SVM. 
