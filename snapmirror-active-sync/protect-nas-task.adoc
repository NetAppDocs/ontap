...
You might want to create volumes that are not protected by SnapMirror active sync in a NAS configuration if the volume type is not supported; for example, a FlexGroup or DP-type volume. You can exclude new volumes from SnapMirror active sync if protection is not required; for example while creating a FlexClone volume for dev/test.
