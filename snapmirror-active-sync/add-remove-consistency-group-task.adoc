...
In most instances, this is a disruptive process requiring you to break the SnapMirror relationship, modify the consistency group, then resume protection. Beginning with ONTAP 9.13.1, adding volumes to a consistency group with an active SnapMirror relationship is a non-disruptive operation.
...
. Find the SnapMirror configuration you want to add volumes to. Select image:icon_kabob.gif[kebab icon] then **Expand**. 
