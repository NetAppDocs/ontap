...
summary: If you want to change the composition of the consistency group by adding or removing a volume in a SAN configuration, you must first delete the original relationship and then create the consistency group again with the new composition.
---
...
[.lead]
As your application workload requirements change, you may need to add or remove volumes from a consistency group to ensure business continuity in your SAN configuration. The process of adding and removing volumes in an an active SnapMirror active sync relationship depends on the version of ONTAP you are using.
