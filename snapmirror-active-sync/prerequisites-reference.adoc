...
* AFF and ASA (legacy) support 4-node clusters. ASAr2 only supports HA clusters.
...
** As far as I know its not only AFF-A to AFF-C but also ASA-A to ASA-C (legacy ASA, no ASAr2 as there are no ASA-C in r2) what is supported. Maybe we can be more specific here and also include something in regards of ASAr2 also as there were also questions about whether ASA(legacy) to ASAr2 is supported.
** We support only SMAS A/A in ASAr2
** ASA(legacy) to ASAr2 is not supported
