...
* NetApp AFF and NetApp All San Array (ASA) legacy support 4-node clusters, while ASAr2 only supports HA clusters.
* Clusters must be either AFF (A-Series and C-Series) or ASA (A-Series and C-Series). Mixing between AFF and ASA is not supported.
* ASAr2 is excluded as there are no ASA-C clusters in r2. 
* Replication is supported between AFF A-Series and C-Series. ASA (legacy) to ASAr2 replication is not supported. ASAr2 only supports SMAS A/A.
...
h| Cluster type h| Supported models h| Nodes supported
...
h| Source h| Destination h| Replication supported?
