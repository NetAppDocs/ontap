...
Beginning with ONTAP 9.16.1, SnapMirror active sync supports four-node clusters in symmetric active/active configurations. In addition to the extended protection offered by the 4-node cluster, you can also use it for a tech refresh. 

.Before you begin
* Review the link:protect-task.html#configure-symmetric-activeactive-protection[requirements for 4-node clusters].
...
* These instructions apply to an existing 4-node configuration with 50 or fewer consistency groups and 400 or fewer volume endpoints. 

.Steps
. link:link:../volumes/move-volume-task.html[Move all the SnapMirror active sync volumes onto a _single_ high-availability (HA) pair].
