...
=== Configure multi-node symmetric active/active protection 

Beginning with ONTAP 9.16.1, you can configure symmetric active/active protection in a four-node cluster. All clusters must be running ONTAP 9.16.1 or later to configure multi-node symmetric active/active protection.
...
.System Manager
--
. If you haven't created a cluster peer relationship between the clusters you want to protect, link:../peering/create-cluster-relationship-93-later-task.adoc[create a cluster peer relationship^].
. On the primary site, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes[create a consistency group using new LUNs.^]
.. When creating the consistency group, specify host initiators to create igroups.
.. Select the checkbox to **Enable SnapMirror** then choose the `AutomatedFailoverDuplex` policy.
.. Ensure that all the host initiators are part of the same iGroup and assign each initiator's proximity to source vs destination based on your configuration.  
.. In the dialog box that appears, select  the **Replicate initiator groups** checkbox to replicate igroups. In **Edit proximity settings**, set proximal SVMs for your hosts.

<Configure iGroups and Map the LUNS>
. Navigate to your host to discover the paths on the hosts and verify the hosts have an active/optimized path to the storage LUN from the preferred cluster.
