...
* Beginning with ONTAP 9.16.1, SnapMirror active sync symmetric active/active configurations are supported on multi-node configurations, meaning you can deploy symmetric/active active protection on up to four nodes. You must be running ONTAP 9.16.1 on all nodes for this configuration. 
    * Before deploying a multi-node configuration, you must link:../peering/create-cluster-relationship-93-later-task.adoc[create a cluster peer relationship^] for a clusters you intend to protect.
    * Review the link:limits-reference.adoc[limits] for multi-node clusters. 
    * If you configure SnapMirror active sync on a more-than-two-node cluster and then choose to revert to a two-node cluster, you must remove the SnapMirror active sync relationships from the cluster before reverting.
