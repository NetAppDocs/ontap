...
* In ONTAP 9.15.1, symmetric active/active is only supported on two-node clusters. 
* Beginning with ONTAP 9.16.1, SnapMirror active sync supports symmetric active/active configurations on 4-node clusters. 
** To use SnapMirror active sync on a 4-node cluster, you must be running ONTAP 9.16.1 or later.
** Before deploying a 4-node configuration, you must link:../peering/create-cluster-relationship-93-later-task.adoc[create a cluster peer relationship^].
** Review the link:limits-reference.adoc[limits] for 4-node clusters. 
...
** With the 4-node configuration, you can replace old platform nodes with new nodes. This process is non-disruptive and expands the cluster while moving volumes into the new nodes. For more information, see link:upgrade-revert-task.html#tech-refresh[tech refresh].
