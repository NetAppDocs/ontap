...
[role="tabbed-block"]
====
.System Manager SCSI configuration
--
...
. Establish the NVMe subsystem configuration. Set the preferred paths for initiators on the local cluster. Specify the option to replicate the configuration to the peer cluster for inverse affinity. 
+
...
.Steps
Beginning with ONTAP 9.17.1, you can use System Manager or the ONTAP CLI to create a consistency group and configure symmetric active/active protection using NVMe protocol host mappings. For more information about configuring consistency groups, see link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes[create a consistency group.^]


[role="tabbed-block"]
====
.System Manager NVMe configuration (ONTAP 9.17.1 and later)
--
.Steps for a uniform configuration
. On the primary site, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes[create a consistency group.^]
...
. On the primary site, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes[create a consistency group.^]
. Select *+Add* and choose *Using new NVMe namespaces*.
. Select *More* then enter the consistency group name, 
.. In the *Protection* area, select the checkbox to *Enable SnapMirror* then choose the `AutomatedFailoverDuplex` policy.
.. Select *Save* to create the consistency group, NVMe namespace, SnapMirror relationship, and host subsystem mapping. 
. On the secondary site, create an NVMe subsystem. 
.. Navigate to *Hosts > NVMe subsystem*. 
.. Select *+Add* to create a new NVMe subsystem.
.. Provide a *Name*, select the *Host Operating System*, choose the *Host NQN*, and set *In proximity to*.
.. Select *Save* to initialize the relationship. 
--

.CLI NVMe configuration (ONTAP 9.17.1 and later)
--
.Steps for a uniform configuration 
. Create a new SnapMirror relationship grouping all the volumes in the application. Ensure you designate the `AutomatedFailOverDuplex` policy to establish bidirectional sync replication.
+
...
`snapmirror show -destination-path <destination_path>`
. On your host, configure host connectivity with access to each cluster according to your needs. 
. Establish the NVMe namespace configuration. Specify the option to replicate the configuration to the peer cluster for inverse affinity. 
+
`SiteA::> vserver nvme namespace create -vserver <svm_name> -path <path> -size <size_of_namespace> -ostype <OS_type>`

+
`SiteA::> vserver nvme subsystem create -vserver <svm_name> -subsystem <name_of_subsystem> -ostype <OS_type>`
...
`snapmirror show -destination-path <destination_path>`
. On your host, configure host connectivity with access to each cluster according to your needs. 
. Establish the NVMe namespace configurations on both the source and destination clusters.
+
`# primary site
SiteA::> vserver nvme subsystem host add-proximal-vserver -subsystem <subsystem_1_name> -host-nqn <host_1_nqn> -proximal-vservers <host_1_name>`
+
`# secondary site
SiteB::> vserver nvme subsystem host add-proximal-vserver -subsystem <subsystem_2_name> -host-nqn <host_1_nqn> -proximal-vservers  <host_2_name>`
+
. From the host, discover the paths and verify the hosts have an active/optimized path to the storage from the preferred cluster. 
. Deploy the application and distribute the VM workloads across clusters to achieve the required load balancing. 
. Establish the NVMe host subsystem configurations on both the source and destination clusters.
