...
- Beginning with ONTAP 9.16.1, SnapMirror active sync symmetric active/active configurations on up to four-node clusters, an increase from the previous two-node limit. 
    - To run SnapMirror active sync on a four-node cluster, all clusters must be running ONTAP 9.16.1 or later.
    - Before deploying a four-node configuration, you must link:../peering/create-cluster-relationship-93-later-task.adoc[create a cluster peer relationship^] for a clusters you intend to protect.
    - Review the link:limits-reference.adoc[limits] for four-node clusters. 
    - If choose to revert to a two-node cluster, you must remove the SnapMirror active sync relationships from the cluster before reverting.
