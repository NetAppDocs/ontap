...
--
.Steps for a uniform configuration
. On the primary site, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes[create a consistency group using new volumes or LUNs.^]
...
.Steps for a non-uniform configuration
. On the primary site, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes[create a consistency group using new volumes or LUNs.^]
.. To configure the consistency group using NVMe, select *+Add* and choose *Using new NVMe namespaces*.
.. Enter the consistency group name.
.. Select *More*.
.. In the *Protection* area, select *Enable SnapMirror* then choose the `AutomatedFailoverDuplex` policy.
.. Select *Save*. 
...
`snapmirror show -destination-path <destination_path>`
. On your host, configure host connectivity with access to each cluster according to your needs. 
. Establish the NVMe host proximity. Specify the option to replicate the configuration to the peer cluster for inverse affinity. 
+
...
`snapmirror show -destination-path <destination_path>`
. On your host, configure host connectivity with access to each cluster according to your needs. 
. Establish the NVMe host proximity on both the source and destination clusters.
+
`# primary site
SiteA::> vserver nvme subsystem host add -subsystem ss1 -host-nqn <H1_NQN> -proximal-vservers <VS_A>`
+
