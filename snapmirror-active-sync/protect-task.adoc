...
* Beginning with ONTAP 9.16.1, SnapMirror active sync supports symmetric active/active configurations on four-node clusters. 
** To use SnapMirror active sync on a four-node cluster, you must be running ONTAP 9.16.1 or later.
** Before deploying a four-node configuration, you must link:../peering/create-cluster-relationship-93-later-task.adoc[create a cluster peer relationship^].
