...
For additional supportability information for NAS configurations, see link:protect-nas-task.html[Protect with ONTAP SnapMirror active sync for NAS].
...
* AIX 
** Beginning with ONTAP 9.19.1 Transparent Application Failover (TAF) is supported with Symmetric active/active configurations. No PVR is required.
** Beginning with ONTAP 9.11.1 through ONTAP 9.18.1, AIX is supported only by PVR for automated failover with symmetric active/active configurations.
...
Beginning with ONTAP 9.19.1, Transparent Application Failover for AIX is supported with SnapMirror active sync in symmetric active/active configurations. You can use System Manager or the ONTAP CLI to create an AIX LUN. To ensure transparent failover, when you use the ONTAP CLI, you must use the option access-mode local with the lun create or lun modify command. By default, the access mode is set to global. When you use System Manager, choose AIX as the host and local as the access mode. For more information about creating a LUN, see link:../san-admin/provision-storage.html[Provision SAN storage]. For additional information about support for AIX, see the NetApp Knowledge Base article: link:https://kb.netapp.com/on-prem/ontap/DP/SnapMirror/SnapMirror-KBs/SnapMirror_active_sync_support_for_AIX%3A_behavior_by_ONTAP_release_and_migration_guidance[SnapMirror active sync support for AIX: behavior by ONTAP release and migration guidance^]. 

If you are using ONTAP 9.11.1 through ONTAP 9.18.1, AIX is supported with SnapMirror active sync via standard engineering Feature Policy Variance Request (FPVR) with the agreement that the following stipulations are understood: 
...
To ensure the Solaris client applications are non-disruptive when an unplanned site failover switchover occurs in an SnapMirror active sync environment, modify the default Solaris OS settings. To configure Solaris with the recommended settings, see the link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/Solaris_Host_support_recommended_settings_in_SnapMirror_Business_Continuity_(SM-BC)_configuration[NetApp Knowledge Base: Solaris Host support recommended settings in SnapMirror active sync^].
...
Beginning with ONTAP 9.19.1, ARP/AI supports primary volumes that participate in SnapMirror active sync SAN relationships. For more information, see link:../anti-ransomware/support-arpai.html#arp-ai-and-virtual-machines[Autonomous Ransomware Protection use cases and considerations].
...
In xref:../data-protection/supported-deployment-config-concept.html[fan-out configurations], your source volume can be mirrored to a SnapMirror active sync destination endpoint and to one SnapMirror asynchronous relationship.

image:fanout-diagram.png[Diagram of a fan-out configuration]

SnapMirror active sync for SAN supports xref:../data-protection/supported-deployment-config-concept.html[fan-out configurations] with the `MirrorAllSnapshots` policy and, beginning with ONTAP 9.11.1, the `MirrorAndVault` policy. Fan-out configurations are not supported in SnapMirror active sync with the `XDPDefault` policy. 

Beginning with ONTAP 9.15.1, SnapMirror active sync supports automatic reconfiguration in the fan-out leg after a failover event. If the failover from the primary to the secondary site has succeeded, the reconfiguration of the fan-out relationship is initiated during the next scheduled update of the asynchronous fan-out leg. It is not triggered immediately following the SnapMirror active sync failover. The async fan-out leg can be a consistency group relationship in a SAN configuration or an independent volume relationship. The reconfiguration will work for either of the cases. Reconfiguration is triggered by either a planned or unplanned failover. Reconfiguration also occurs upon failback to the primary site. 
...
Beginning in ONTAP 9.12.1, link:../data-protection/restore-part-file-snapshot-task.html[partial LUN restore] is supported for SnapMirror active sync volumes. You can restore data from application-created snapshots that have been replicated between the SnapMirror active sync source (volume) and the destination (snapshot) volumes. Partial LUN or file restore may be necessary if you need to restore a database on a host that stores multiple databases on the same LUN. Using this functionality requires you to know the starting byte offset of the data and byte count.

=== Tamperproof snapshots

Tamperproof snapshots are not supported with SnapMirror active sync.
...
// 2026-July-17, ONTAPDOC-4068
// 2026-June-26, ONTAPDOC-3817
// 2026-May-25, ONTAPDOC-3544
// 2026-June-2, ONTAPDOC-3918
// 2026-Apr-14, ONTAPDOC-3650, IDR# 596
// 2025-Mar-3, ONTAPDOC-3767
