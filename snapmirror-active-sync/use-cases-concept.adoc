...
SnapMirror Business Continuity (SM-BC) provides the following use cases: 
...
.Disaster scenario
Synchronously replicate multiple volumes for an application between sites at geographically dispersed locations. You can automatically failover to the secondary copy in case of disruption of the primary, thus enabling business continuity for tier one applications.
...
.Disaster scenario 
When the site hosting the primary cluster experiences a disaster, the host multipathing software marks all paths through the cluster as down and uses paths from the secondary cluster. The result is a non-disruptive failover enabled by ONTAP Mediator, to the mirror copy for LUN L1 with L1S now converted from a mirror copy to an active copy with the hosts preferred path through VS2:N3. 