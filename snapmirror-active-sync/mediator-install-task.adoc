...
+
image:Configure_Mediator_System_Manager.png[system manager mediator add]
+
When the certificate addition is complete, the ONTAP Mediator will be added to the ONTAP cluster. 
+

The following image demonstrates a successful ONTAP Mediator configuration:
+
image:Successful_Mediator_Installation.png[mediator add successful].
...
.Steps
. The ONTAP Mediator cannot be initialized in ONTAP without a valid CA certificate. Therefore, you need to add a valid certificate authority to the certificate store for peered clusters. Find the ONTAP Mediator CA certificate from the ONTAP Mediator Linux VM/host software installation location `cd /opt/netapp/lib/ontap_mediator/ontap_mediator/server_config`.
...
. Initialize the ONTAP Mediator on one of the clusters. The ONTAP Mediator will be automatically added for the other cluster:
+
...
You might want to re-initialize the ONTAP Mediator service. There might be situations that warrant the re-initialization of the ONTAP Mediator service such as change in the ONTAP Mediator IP address, certificate expiration, and more.
...
+
[NOTE]
This step will not remove the self-signed server-ca from the ONTAP cluster. NetApp recommends navigating to the *Certificate* tab and removing it manually before performing the next step below to add a third-party certificate:
+
image:Remove_Mediator.png[system manager mediator remove]
...
. Refer to the steps described in link:../mediator/manage-task.html#Replace-self-signed-certificates-with-trusted-third-party-certificates[Replace self-signed certificates with trusted third-party certificates] on how to obtain certificates from subordinate CA, referred to as `ca.crt`.
+
[NOTE]
`ca.crt` has certain properties that it derives from the request that needs to be sent to the PKI authority, defined in the file `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config/openssl_ca.cnf`.


. Find the new third-party ONTAP Mediator CA certificate `ca.crt` from the ONTAP Mediator Linux VM/host software installation location:
+
*Example*
+
----
...
. Add the ONTAP Mediator again now that you have the third-party certificate in ONTAP. Remove the previously configured ONTAP Mediator from the SM-BC cluster:
+
*Example*
+
-----
