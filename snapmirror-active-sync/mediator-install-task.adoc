...
+
[NOTE]
The `ca.crt` has certain properties that it derives from the request that needs to be sent to the PKI authority, defined in the file `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config/openssl_ca.cnf`.
