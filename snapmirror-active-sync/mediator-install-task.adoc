...
Each cluster peer relationship can only be associated with a single ONTAP Mediator instance. HA Mediator instances are not supported. When a cluster is in several peer relationships with other clusters, the following ONTAP Mediator options are available:
...
For example, if cluster B has a peer relationship with cluster A, cluster C, and cluster D, and SnapMirror active sync is configured on each relationship, all three cluster peer relationships can have a unique associated ONTAP Mediator instance. Alternatively, cluster B can use the same ONTAP Mediator instance for all three peer relationships. In this scenario, the same instance of ONTAP Mediator is listed three times for the cluster.
