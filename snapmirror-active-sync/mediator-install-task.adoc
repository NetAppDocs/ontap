...
Once you have installed the ONTAP Mediator and configured your clusters, <<initialize-the-ontap-mediator, initialize the ONTAP Mediator for SnapMirror active sync using self-signed certificates>>. You must then xref:protect-task.html[create, initialize, and map the consistency group for SnapMirror active sync].
...
* The ONTAP Mediator includes its own set of prerequisites. You must meet these prerequisites before installing the ONTAP Mediator.
+
...
Perform each of the following steps to install the ONTAP Mediator and verify the cluster configuration. For each step, you should confirm that the specific configuration has been performed. Each step includes a link to the specific procedure that you need to follow.

.Steps

. Install the ONTAP Mediator service before verifying that your source and destination clusters are configured correctly.
+
...
. Confirm that the SVMs are created on each cluster.
+
xref:../smb-config/create-svms-data-access-task.html[Creating an SVM]

. Confirm that a peer relationship exists between the SVMs on each cluster.
+
...
Job ID: 884
              ...
              Owning Vserver: cluster1
                 ...
                 Description: Creating a mediator entry
                  ...
                  Queue Time: 04/06 07:46:45

                  Start Time: 04/06 07:46:45

                    End Time: 04/06 07:46:46
           ...
           Completion String: Mediator CA certificate is not installed for mediator IP address "192.168.0.64" on cluster "cluster2". Use the command "security certificate show -common-name ONTAPMediatorCA" to verify. If the certificate is installed, it might be installed for a different mediator IP address. Use the command "security certificate install -type server-ca -vserver <cserver_name>" to install the CA certificate.

                    Job Type: Snapmirror mediator

                Job Category: Snapmirror mediator add
...
You might need to re-initialize the ONTAP Mediator service. There might be situations that require the re-initialization of the ONTAP Mediator service such as a change in the ONTAP Mediator IP address, certificate expiration, and so on.
