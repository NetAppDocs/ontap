...
[NOTE]
In ONTAP Mediator 1.8 and earlier, the CA certificate is `ca.crt`. Starting with ONTAP Mediator 1.9, the required certificate required is `intermediate.crt`.
...
. Refer to the steps described in link:../mediator/manage-task.html[Replace self-signed certificates with trusted third-party certificates] on how to obtain certificates from subordinate CA, referred to as `intermediate.crt`.
...
. Add the `intermediate.crt` file to the peered cluster. Repeat this step for all the peer clusters:
+
*Example*
+
-----
