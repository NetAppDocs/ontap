...
The ONTAP Mediator provides a persistent and fenced store for high availability (HA) metadata used by the ONTAP clusters in a SnapMirror active sync relationship. It provides a synchronous node health query functionality to aid in quorum determination as well as serves as a ping proxy for controller liveliness detection.
