...
[.lead]
SnapMirror active sync utilizes peered clusters to protect your data in the event of a failover scenario. The ONTAP Mediator is a key resource enabling business continuity by monitoring the health of each cluster. To configure SnapMirror active sync, you must first install the ONTAP Mediator and verify that your primary and secondary clusters are configured properly. 
...
* By default, the ONTAP Mediator provides service through TCP port 31784. You should make sure that port 31784 is open and available between the ONTAP clusters and Mediator.

== Install the ONTAP Mediator and confirm cluster configuration 

Proceed through each of the following steps. For each step, you should confirm that the specific configuration has been performed. Follow the documentation linked after each step to get more information as needed.
