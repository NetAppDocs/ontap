...
[options="header"]
|===
| S. No |Platform 4+| Endpoints per HA pair for SnapMirror active sync 3+| Overall sync and SnapMirror active sync endpoints per HA pair
|
|
...
|Unique LUNs per cluster
a| * 6144 (4-node cluster, available in ONTAP 9.16.1 and later)
* 4096 (2-node cluster, ONTAP 9.9.1 and later)
