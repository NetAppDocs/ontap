...
---
= Convert an existing ONTAP SnapMirror SAN relationship to SnapMirror active sync for SAN relationship
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
If you've configured SnapMirror protection, you can convert a SAN relationship to SnapMirror active sync for SAN. Beginning with ONTAP 9.15.1, you can convert the relationship to use symmetric active/active protection.

For information about converting an SVM-DR relationship to a SnapMirror active sync for NAS relationship, see link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship[Convert an SVM DR relationship to a SnapMirror active sync relationship].
...
* link:mediator-install-task.html[ONTAP Mediator] must be configured for SnapMirror active sync.
