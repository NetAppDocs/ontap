...
== SnapMirror active sync symmetry

In _asymmetric configurations_, the primary storage copy exposes an active-optimized path and actively serves client I/O. The secondary site uses a remote path paths for I/O. The storage paths for the secondary site are considered active-non-optimized and access to the write LUN is proxied from the secondary site. 
