...
* Primary ONTAP cluster hosting the SnapMirror active sync primary SVM
* Secondary ONTAP cluster hosting the mirror SVM
* ONTAP Mediator 
...
See the link:mediator-install-task.html[ONTAP Mediator] documentation for more information.
...
The following figure illustrates the design of SnapMirror active sync in SAN configuration at a high level.
...
SnapMirror active sync offers asymmetric and symmetric solutions for SAN configurations. 
...
In _symmetric active/active configurations_, active-optimized paths are exposed on both sites, are host specific, and are configurable, meaning hosts on either side are able to access local storage for active I/O. Beginning with ONTAP 9.16.1, symmetric active/active is supported on 2-node clusters and on 4-node clusters. SnapMirror active sync supports 2-node cluster to 2-node cluster configurations and 4-node cluster to 4-node cluster configurations. It does not support either 4-node cluster to 2-node cluster configurations or 2-node cluster to 4-node cluster configurations. Beginning with ONTAP 9.17.1, symmetric active/active configurations support NVMe protocol on two node clusters.
...
Beginning with ONTAP 9.19.1, SnapMirror active sync provides protection for NAS workloads at the SVM level. In a NAS configuration, the destination (secondary) SVM remains in a dormant state until a failover event occurs. To enable non-disruptive failover, the network identity and protocol configurations, including IP addresses, must be identical on both the primary and secondary SVMs. These configurations are automatically replicated to the secondary cluster, ensuring consistent IP addressing and protocol settings across both clusters. The following figure illustrates the design of SnapMirror active sync in a NAS configuration at a high level.

image:snapmirror-active-sync-nas-config.png[SnapMIrror active sync NAS configuration]

In ONTAP 9.19.1, only FlexVol volumes are supported. Consistency groups are not supported. With FlexVol only support, the secondary cluster does not guarantee write-order-consistency across volumes. In the figure, data and configuration information is replicated from SVM A1 on the source cluster to the dormant SVM, SVM A1', on the destination cluster. 



// 2026-May-20, ONTAPDOC-3544
