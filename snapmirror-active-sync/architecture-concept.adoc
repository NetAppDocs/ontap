...
[NOTE]
When ONTAP Mediator is not available, you cannot perform planned or automated failovers. The application data continues to synchronously replicate without any interruption to for zero data loss.

== Operations 

The following figure illustrates the design of SnapMirror active sync at a high level.

image:workflow_san_snapmirror_business_continuity.png[Diagram of SnapMirror active sync architecture]
...
In the event of a disaster scenario at the primary data center, the ONTAP Mediator directs the secondary data center to act as the primary, serving all I/O operations. Only the volumes that are mirrored in the consistency group are served. Any operations pertaining to the other two volumes on the SVM is be affected by the disaster event. 
...
image:snapmirror-active-sync-asymmetric.png[Diagram of an asymmetric active configuration]
...
image:snapmirror-active-sync-symmetric.png[Diagram of an symmetric active configuration]

Symmetric active/active relationships are targeted for, but not limited to, VMware Metro Storage Cluster (vMSC) load-balancing deployments, enabling migration of VMs between the primary and secondary ESX Clusters for better performance and resource utilization.