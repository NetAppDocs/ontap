...
Beginning with ONTAP 9.19.1, SnapMirror active sync supports NAS configurations at the SVM level. In SnapMirror active sync NAS configurations the secondary SVM is dormant until failover because the IP address and protocol configuration on the primary and secondary SVM must be the same to enable non-disruptive failover. The following figure illustrates the design of SnapMirror active sync in a NAS configuration at a high level.

image:snapmirror-active-sync-nas-config.png[SnapMIrror active sync NAS configuration]

In ONTAP 9.19.1, only FlexVol volumes are supported. Consistency groups are not supported. With FlexVol only support, the secondary does not guarantee write-order-consistency across volumes. In the figure, data and configuration information is replicated from SVM A1 on the source cluster to the dormant SVM, SVM A1', on the destination cluster. 
