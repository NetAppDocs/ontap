...
[.lead]
You create a Flash Pool local tier (aggregate) by enabling the feature on an existing local tier composed of HDD RAID groups, and then adding one or more SSD RAID groups to that local tier. This results in two sets of RAID groups for that local tir: SSD RAID groups (the SSD cache) and HDD RAID groups.
...
--
*Use System Manager to create a Flash Pool local tier using physical SSDs*
...
--
*Use the CLI to create a Flash Pool aggregate using physical SSDs*
