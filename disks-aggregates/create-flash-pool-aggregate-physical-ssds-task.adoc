...
summary: "You create a Flash Pool local tier by enabling the feature on an existing local tier composed of HDD RAID groups, and then adding one or more SSD RAID groups to that local tier. This results in two sets of RAID groups for that local tier, which are the SSD RAID groups, or the SSD cache, and HDD RAID groups."
---
...
.Related information
* link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+aggregate+add[storage aggregate add^]
* link:https://docs.netapp.com/us-en/ontap-cli/storage-aggregate-modify.html[storage aggregate modify^]


// 2025 Aug 01, ONTAPDOC-2960
