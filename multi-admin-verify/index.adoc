...
You can create rules to protect the following commands. You can protect each command beginning with the ONTAP version in which protection capability for the command first became available.
...
* `volume delete`
* `volume flexcache delete`
* `volume recovery-query modify` ^2^
* `volume recovery-query purge` ^2^
* `volume recovery-query purge-all` ^2^
