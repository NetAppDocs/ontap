...
When a new request is created, older requests are checked for deletion eligibility. Requests that are expired or executed are deleted if they are over one day past their approval expiration time. If there are more than 1000 requests, a warning is issued but no requests are deleted automatically.
