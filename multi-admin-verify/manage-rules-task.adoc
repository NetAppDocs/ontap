...
When you create a rule, you can optionally specify the `-query` option to limit the request to a subset of the command functionality. 

For example, in the default set command, `-query` is set to `-privilege diag`, meaning that a request is generated for the set command only when `-privilege diag` is specified.
...
The `-query` option can also be used to limit configuration elements, such as the SVM, volume, and Snapshot names. 
...
NOTE: Any excluded configuration elements would not be protected by MAV, and any administrator could delete (or rename and delete) them.
