...
Beginning with ONTAP 9.15.1, FPolicy supports the NFSv4.1 protocol. FPolicy monitoring of NFSv4.2 file access events is not yet supported. 

Beginning with ONTAP 9.19.1, administrators can control NFSv4.2 availability at the SVM level, enabling or disabling the protocol for individual storage VMs using either System Manager, the CLI, or REST API. If you configure FPolicy monitoring, you need to disable NFSv4.2 on the SVM using the steps provided in the link:nfs-admin/enable-disable-nfs-task.html[Enable or disable NFS for ONTAP SVMs] topic. 
