...
[NOTE]
Beginning with ONTAP 9.19.1, administrators can control NFSv4.2 availability at the SVM level, enabling or disabling the protocol for individual storage VMs using either System Manager, the CLI, or REST API. FPolicy monitoring of NFSv4.2 file access events is not yet supported.

.Reference information
* link:task_nas_enable_linux_nfs.html[Enable NAS storage for Linux servers using NFS with ONTAP System Manager]
* link:nfs-admin/enable-disable-nfsv42-task.html[Enable or disable NFSv4.2 for ONTAP SVMs]
