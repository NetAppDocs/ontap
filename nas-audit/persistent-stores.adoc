...
Have the snapshot policy set to `none` for that volume instead of `default`. This ensures that there is no accidental restore of the snapshot leading to loss of current events and to prevent possible duplicate event processing.
