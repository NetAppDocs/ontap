...
Beginning with ONTAP 9.15.1, use the `fpolicy persistent-store-create` command to create the FPolicy persistent store with inline volume creation and configuration. ONTAP will automatically block the volume from external user protocol access (CIFS/NFS).  

.Before you begin 

* The SVM where you want to create the persistent store must have at least one aggregate.  

* You should have access to the aggregates available for the SVM and sufficient permissions to create volume 
