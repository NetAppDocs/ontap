...
summary: "Issues can arise when there is insufficient space on either the staging volumes or on the volume containing the audit event logs. If there is insufficient space, new audit records cannot be created. When audit guarantee is enabled, this can prevent clients from accessing data and cause access requests to fail. You should know how to troubleshoot and resolve these volume space issues."
---
...
[NOTE]
====
This behavior applies when audit guarantee is enabled. For more information, see link:../nas-audit/auditing-process-concept.html[Guaranteed auditing].
====

== Troubleshoot space issues related to the event log volumes

If volumes containing event log files run out of space, auditing cannot convert log records into log files. When audit guarantee is enabled, this results in client access failures. You must know how to troubleshoot space issues related to event log volumes.
