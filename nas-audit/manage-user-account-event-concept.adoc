...
The user-account events with event-ids 4720, 4722, 4724, 4725, 4726, 4738, and 4781 are generated when a local CIFS or NFS user is created or deleted from the system, local user account is enabled, disabled or modified, and local CIFS user password is reset or changed. The user-account events are generated when a user account is modified using `vserver cifs users-and-groups <local user>` and `vserver services name-service <unix user>` commands.

The following example displays a user account event with the ID 4720 generated, when a local CIFS user is created:
...
The following example displays a user account event with the ID 4781 generated, when the local CIFS user created in the preceding example is renamed:
