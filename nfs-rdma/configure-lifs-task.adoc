...
NOTE: Each RDMA port can have a maximum of 127 LIFs. When more LIFs are added or when a LIF fails over to a port that exceeds that limit, the port is unable to serve RDMA traffic.
