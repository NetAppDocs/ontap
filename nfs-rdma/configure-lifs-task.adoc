...
NOTE: Each RDMA port can have a maximum of 127 LIFs. When you add more LIFs or when a LIF fails over to a port that exceeds that limit, the newly added LIFs or the failed over LIF are unable to serve RDMA traffic.
