...
Beginning with ONTAP 9.10.1, NFS over RDMA configurations are supported for the NFSv4.0 protocol. Refer to the chart in requirements to understand NFS version support in subsequent ONTAP releases.
//TODO how can we refer to "chart in requirements?" Make sure no key info is being removed here from the previous.


.Requirements
* Storage systems must be running ONTAP 9.10.1 or later.
* For information about target RNIC support, refer to the link:https://hwu.netapp.com/[NetApp Hardware Universe^]. 
//* For information about adapter support, refer to the link:https://hwu.netapp.com/[NetApp Hardware Universe^]. NFS over RDMA supported adapters display "RoCE" in the description field. 
//TODO: make sure "target RNIC support" makes sense. Or use the bullet from below (commented bullet above me). Not sure if we use IMT for that now, double check.
...
* Refer IMT to find supported Client Operating Systems with NFS over RDMA. For RoCE v2 supported RNICs, refer to respective RNIC vendor documentation. 
//TODO how to link/refer to IMT? Make sure the below bullet doesn't contain key info that is getting lost. Do we need to call out Mellanox here? And do we need to link to HWU and include the note on RoCE?
//* Clients must be using Mellanox RDMA-capable NIC cards and Mellanox OFED (MOFED) network software. For information about adapter support, refer to the link:https://hwu.netapp.com/[NetApp Hardware Universe^]. NFS over RDMA supported adapters display "RoCE" in the description field. 
