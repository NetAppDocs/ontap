...
Beginning with ONTAP 9.13.1, you can specify the maximum capacity that can be allocated for all volumes in a storage VM. You can enable the maximum capacity when you create a storage VM or when you edit an existing SVM.

To set a maximum capacity limit on an SVM with the CLI, see xref:./volumes/manage-svm-capacity.html[Manage capacity limits for an SVM].
...
.. To add a storage VM, select image:icon_add_blue_bg.gif[add icon].
.. To edit a storage VM, select image:icon_kabob.gif[kebab icon] next to the name of the storage VM, and then click *Edit*. 
