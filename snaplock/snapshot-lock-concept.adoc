...
Beginning with ONTAP 9.14.1, Snapshot copy locking supports long-term retention Snapshot copies on SnapLock vault destinations and on non-SnapLock SnapMirror destination volumes. This enables Snapshot copy locking using the retention period associated with SnapMirror policy rules. If there is no retention period associated with the SnapMirror label, however, the default retention period of the volume is used.
