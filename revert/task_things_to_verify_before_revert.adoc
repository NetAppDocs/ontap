...
If you are reverting from ONTAP 9.12.1 or later to ONTAP 9.12.0 or earlier, you must link:..nvme/disable-secure-authentication-nvme-task.html[disable in-band authentication] before you revert.  If in-band authentication using DH-HMAC-CHAP is not disabled, revert will fail. 
