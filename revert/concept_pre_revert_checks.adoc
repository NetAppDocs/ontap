...
* link:arp-warning-clear-revert.html[Respond to Autonomous Ransomware Protection warnings of abnormal activity].

| ONTAP 9.19.1
a|
* If you have any SVMs with active-active multipathing configurations, you must link:delete-active-active-svms.html[delete the SVMs].
* If you have modified the default settings on any QoS policy group created in ONTAP 9.19.1 or later, you must link:modify-qos-policy-groups.html[reset them]. 
* If you upgraded volumes to ONTAP 9.19.1, and added a second S3 access key to any user, you must link:second-s3-access-key.html[remove these keys].

| ONTAP 9.18.1
a|
* If automatic enablement has been set for ARP/AI as part of an ONTAP 9.18.1 upgrade, you'll need to link:anti-ransomware-disable-auto-enable.html[disable it].

| ONTAP 9.17.1
a|
* If you have enabled the ONTAP ARP/AI feature for SAN, link:anti-ransomware-disable-san.html[disable it].
