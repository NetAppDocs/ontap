...
You must have completed the downgrade link:task_things_to_verify_before_downgrade_revert.html[verifications] and link:task_quiesce_snapmirror_relationships_before_downgrade.html[pre-checks].

. Downgrade the first node in an HA pair
+
You downgrade the first node in an HA pair by initiating a takeover by the node's partner. The partner serves the node's data while the first node is upgraded.
...
. Complete link:task_verify_health.html[post-downgrade verifications].

=== Downgrading the first node in an HA pair
...
=== Downgrading the partner node in an HA pair
