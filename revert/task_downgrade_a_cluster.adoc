...
You should perform a downgrade for the following reasons:

* You want to move from ONTAP 9.1 to ONTAP 9.0.
* You want to move to an earlier version of ONTAP within the same major release, for example, ONTAP 9.2.3 to ONTAP 9.2.2.

If you are not moving from ONTAP 9.1 to 9.0 or to an earlier version of ONTAP within the same major release, you must follow the link:task_reverting_an_ontap_cluster.html[revert process].

NOTE: This procedure only applies to downgrades from ONTAP 9.1 to 9.0 versions. To move from any version of ONTAP other than 9.1 to any version other than 9.0, you must perform a link:task_reverting_an_ontap_cluster.html[revert].  All downgrades from ONTAP 9.1 to ONTAP 9.0 are nondisruptive except downgrades of 2-node MetroCluster configurations, which are disruptive.
...
You must have completed the downgrade link:task_things_to_verify_before_downgrade_revert[verifications] and link:concept_pre_downgrade_revert_checks.html[pre-checks].
