...
= Downgrade a cluster using the CLI
:icons: font
:imagesdir: ../media/

[.lead]
You can perform a downgrade from ONTAP 9.1 to ONTAP 9.0.
...
A nondisruptive downgrade from ONTAP 9.1 to ONTAP 9.0 is peformed using the rolling update method.  This method has several steps: initiating a failover operation on each node in an HA pair, updating the "`failed`" node, initiating giveback, and then repeating the process for each HA pair in the cluster.
