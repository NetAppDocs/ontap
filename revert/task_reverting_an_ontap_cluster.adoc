...
. After both nodes have been reverted, reenable HA for the cluster: 
+
[source, cli]
----
...
----
+
NOTE: Re-enabling cluster HA might cause the `storage failover auto-giveback` setting to revert to true.
...
. Verify the `storage failover auto-giveback` setting on both nodes: 
+
[source, cli]
----
...
----
+
If a different setting is required for your environment, use the `storage failover modify -node <nodename> -auto-giveback <true|false>` command to modify it.
...
* link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_auto-giveback_enabled_after_re-enabling_storage_failover_setting[NetApp Knowledge Base: Why auto-giveback enabled after re-enabling storage failover setting^]


// 2026 Aug 18, Git Issue 2088
// 2025 Dec 16, ONTAPDOC-3361
