...
[cols="1,1,1,1", options="header"]
|===
| Storage system | ONTAP version |SAN protocol | LIF failover support
...
| iSCSI
| Automatic LIF failover with active-active or active-active-scsi-only multipathing configurations
...
NOTE: If automatic LIF failover occurs on a system running the NVMe/TCP protocol, and the maximum number of NVMe controllers are in use on the surviving node, no new NVMe controllers can be created on the LIFs that failed over. 
...
+
Both clusters in a SnapMirror active sync relationship must be running the same version of ONTAP 9.19.1 or later and the multipath configuration on the source SVM and destination SVM must be the same. Configuring different multipath configurations for the source SVM and destination SVM is not supported.

* *MetroCluster configurations*
+
The storage system type in the primary and secondary sites must be the same and both clusters must be running ONTAP 9.19.1 or later when active-active SVMs are configured on either cluster.  If one cluster is running ONTAP 9.18.1 or earlier, creating iSCSI or NVMe/TCP LIFs with the sfo-partner-only failover policy in active-active or active-active-scsi-only configurations can cause LIF replication to fail on the destination cluster. 
