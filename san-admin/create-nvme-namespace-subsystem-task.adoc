...
[.lead]
When you provision storage, you enable your NVMe hosts to read from and write data to ONTAP storage systems. For systems using the NVMe protocol, you must create one or more NVMe namespaces and subsystems. Each namespace can then be mapped to an NVMe subsystem to allow data access from your host.

If the storage virtual machine (SVM) containing the NVMe subsystem has both NVMe/FC and NVMe/TCP network interfaces, the namespaces that are mapped to the NVMe subsystem can be accessed by both NVMe/FC and NVMe/TCP transport protocols. NVMe support for FC is available beginning with ONTAP 9.4 and later. NVMe support for TCP is available with ONTAP 9.10.1 and later.  If you require protocol-specific access to your namespaces, you should link:configure-svm-nvme-task.html[configure a SVM] for each protocol.
