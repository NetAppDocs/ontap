...
---
= Create a port sets and bind to an igroups
:icons: font
:imagesdir: ../media/

[.lead]
In addition to using link:selective-lun-map-concept.html[Selective LUN Map (SLM)], you can create a port set and bind the port set to an igroup to further limit which LIFs can be used by an initiator to access a LUN.
...
--
Beginning with ONTAP 9.10.1, you can use System Manager to create port sets and bind them to igroups.

If you need to create a port set and bind it to an igroup in an ONTAP release earlier than 9.10.1 you must use the ONTAP CLI procedure.
