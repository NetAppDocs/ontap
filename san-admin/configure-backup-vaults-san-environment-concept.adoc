...
It is important to ensure that LUNs being backed up are in a consistent state before the Snapshot copies are created and copied to the SnapVault secondary volume. Automating the Snapshot copy creation with SnapCenter ensures that backed up LUNs are complete and usable by the original application.
