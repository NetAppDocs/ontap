...
Suppose, for example, that you need to rebalance your storage workloads to improve performance distribution. This requires you to migrate ten virtual machines (VMs) containing 45 storage units with an average size of 500 GB each.  This means you need to copy around 22.5 TB of data. Instead of using its own CPU for data migration, the host can use NVMe copy offload to avoid reducing its CPU resources for application workloads while the data is being copied.
...
* Data can be migrated only between ONTAP cluster nodes in the same HA pair.
