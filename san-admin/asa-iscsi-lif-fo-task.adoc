...
* Does this still work here? The manual movement of an iSCSI LIF is a LIF migrate/revert, which is initiated by the cluster admin using the ONTAP CLI or System Manager.
+
<<Migrate and revert an iSCSI LIF>>
+
You manually migrate/revert an iSCSI LIF under the following circumstances:
