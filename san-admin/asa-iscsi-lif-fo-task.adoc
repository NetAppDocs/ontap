...
* The manual movement of an iSCSI LIF is a LIF migrate/revert, which is initiated by the cluster admin using the ONTAP CLI or System Manager.
** You manually migrate and revert an iSCSI LIF when scheduled maintenance or replacement is needed, or when you have a pre-existing iSCSI LIF, meaning that the iSCSI LIF was created before you upgraded your HA pair to ONTAP 9.11.1 or later, and you have not enabled the iSCSI LIF failover feature on the LIF.
...
[.Lead]
Beginning with ONTAP 9.11.1, on All SAN Array (ASA) platforms, for LIFs with iSCSI LIF failover enabled (either automatically or manually), the following applies.
...
[.Lead]
Beginning with ONTAP 9.11.1, on All SAN Array (ASA) platforms, you can use System Manager or the ONTAP CLI to manually migrate an iSCSI LIF to a different port on the same node or to a different port on the HA partner, and then revert the LIF back to its home node and home port.

=== Migrate and revert an iSCSI network interface (LIF) using System Manager
...
.Before you begin
Your ASA platform HA pair must be running ONTAP 9.11.1 or later.

==== Migrate a network interface

.Steps (MUST use Sundari's text with correct formating and image/s!!!!!!!!!)
. In System Manager, click Network>Overview>Network Interfaces
. Select the LIF you want to migrate, then click xxx , and then click Migrate.
...
==== Revert a network interface

.Steps (MUST use Sundari's text with correct formating and image/s!!!!!!!!!)
. In System Manager, click Network>Overview>Network Interfaces.
. Select the LIF you want to revert, then click   and then click Revert Network Interface.
...
.About this task??
You manually migrate and revert an iSCSI LIF when scheduled maintenance or replacement is needed, or if you have a pre-existing iSCSI LIF not enabled with the iSCSI LIF failover feature, meaning that the iSCSI LIF was created before you upgraded your HA pair to ONTAP 9.11.1 or later, and that after upgrading, you did not enable the iSCSI LIF failover feature (set the failover policy to “sfo-partner-only”).
