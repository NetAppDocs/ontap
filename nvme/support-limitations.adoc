...
+
SCSI, NAS and NVMe protocols can exist on the same storage virtual machine (SVM).  If protocol co-existence is not supported by the host operating system, you should configure separate SVMs for each protocol supported by the host.
+ 
