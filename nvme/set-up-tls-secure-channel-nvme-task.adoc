...
----
. Verify that the NVMe subsystem host controller is configured for TLS secure channel. You can optionally use the `tls-key-type`, `tls-identity`, and `tls-cipher` arguments to only display the controllers that have those TLS attributes:
+
[source,cli]
----
