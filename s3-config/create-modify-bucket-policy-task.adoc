...
[NOTE]
In the *Resources* and *Conditions* fields, you can specify variables in a policy. These are policy variables are placeholders that are replaced with the contextual values when the policy is evaluated. If `${aws:username}` is specified as a variable for a policy, then policy variable is replaced with request context username, and the policy action is performed as configured for that user. This variable can also be used in as a condition.
...
----
The following example creates an object store server bucket policy statement for the storage VM svm1 and bucket1, and specifies `${aws:username}` as a variable for a policy resource. When the policy is evaluated, the policy variable is replaced with request context username, and the policy action is performed as configured for that user. For example, when the following policy statement is evaluated, `${aws:username}` is replaced with the user performing the S3 operation, say a user `user1`. Therefore, user1 will have access to bucket1 as `bucket1/user1/*`.
