...
When an object store server is created, a root user with UID 0 is created. No access key or secret key is generated for this root user. The ONTAP administrator must run the `object-store-server user regenerate-keys` command to set the access key and secret key for this user.
...
// 2026-July-17, GitHub issue #2071
