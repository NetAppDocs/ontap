...
An storage VM containing an S3 server must already exist.
...
For more information about storage service definitions, see link:storage-service-definitions-reference.html[Storage service definitions]
...
For more information about performance management, see link:../performance-admin/index.html[Performance management]
...
Beginning with 9.14.1, object locking is supported on S3 buckets. If you want to lock the objects in your bucket from getting deleted or overwritten, select *Enable object locking*. Locking can be enabled on all or specific versions of objects, and only when the SnapLock compliance clock is initialized for the cluster nodes. Follow these steps:
...
. Select *Compliance* if you want to assign stricter rules of deletion and update on the objects. The objects can be expired only on the completion of the specified retention period. Unless a retention period is specified, the objects remain locked indefinitely.
