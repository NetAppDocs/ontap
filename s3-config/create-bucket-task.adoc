...
* Beginning with ONTAP 9.18.1, you can create NAS buckets that enable applications to access data on FlexCache volumes using the S3 protocol. All nodes in the cluster are running ONTAP 9.18.1 or later. Before you can access a FlexCache volume using S3 protocol, you must link:../flexcache/create-volume-task.html[set the `-is-s3-enabled` option to `true` on the FlexCache volume]. The parameter is set to `false` by default.
...
. Create a bucket:
+
`vserver object-store-server bucket create -vserver <svm_name> -bucket <bucket_name> -type [s3|nas] [-size integer{KB|MB|GB|TB|PB}] [-comment text] [additional_options]`
+
