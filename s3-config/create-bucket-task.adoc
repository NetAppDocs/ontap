...
. If the SnapLock compliance clock is not initialized on any node of the cluster, the *Initialize SnapLock Compliance Clock* button appears. Click *Initialize SnapLock Compliance Clock* to initialize SnapLock compliance clock on the cluster nodes.
. Select *Governance* to activate a time-based lock that allows _Write once, read multiple times (WORM)_ permissions on the objects. The objects can, however, be deleted by administrator users with specific permissions.
...
For information, see https://docs.netapp.com/us-en/ontap/s3-snapmirror/create-remote-mirror-new-bucket-task.html#system-manager-procedure[Create mirror for new bucket].

=== Verify user access to the bucket
