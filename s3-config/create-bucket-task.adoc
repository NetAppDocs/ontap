...
* ##Click *Save* to create a bucket with these defaults.##
...
. If you want to lock the objects in your bucket from getting deleted or overwritten, select *Enable object locking*. Locking can be enabled on all or specific versions of objects, and only when SnapLock compliance clock is initialized for the cluster.
.. If the SnapLock compliance clock is not initialized on any node of the cluster, the *Initialize SnapLock Compliance Clock* button appears. Click *Initialize SnapLock Compliance Clock* to initialize SnapLock compliance clock on the cluster.
.. Select *Governance* to enable a time-based lock on the objects, that is enable a _Write once, read multiple times (WORM)_ permissions. The objects can, however, be deleted by administrator users with specific permissions.
.. Select *Compliance* if you want to assign stricter rules of deletion and update on the objects. The objects can be expired only on the completion of the specified retention period.
.. Specify the retention tenure for the lock in days or years if you want the locking to be effective for a certain period. The locked objects never expire if no retention period is specified.
[NOTE]
Locking is enabled on versioned and on-versioned buckets. Object locking is not enabled for NAS objects.
+
. Optionally, you can configure protection and permission settings, and performance service level for the bucket. For information, see https://docs.netapp.com/us-en/ontap/s3-snapmirror/create-remote-mirror-new-bucket-task.html#system-manager-procedure[Create mirror for new bucket].
