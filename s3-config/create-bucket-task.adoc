...
Beginning with ONTAP 9.14.1, you can provision a bucket on a mirrored or unmirrored aggregate in MetroCluster environment. For more information, refer to link:create-bucket-mcc-task.html[Create a bucket on a mirrored or unmirrored aggregate in a MetroCluster configuration]. 
...
On S3 client applications (whether ONTAP S3 or an external 3^rd^-party application), you can verify your access to the newly created bucket by entering the following:
