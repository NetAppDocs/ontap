...
+
The `-dns-name <text>` parameter is often required by clients to specify the Subject Alternate Name extension for providing a list of DNS names.
+
The `-ipaddr <IP address>` parameter is often required by client to specify the Subject Alternate Name extension for providing a list of IP addresses.
+
