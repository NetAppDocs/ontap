...
. In the *Settings* tab, select image:icon_pencil.gif[Edit icon] in the *S3* tile.
. In the *Users* tab, verify that there is no access key, or the key has expired for the user.
. If you need to regenerate the key, select image:icon_kabob.gif[Menu options icon] next to the user, then select *Regenerate Key*.
. By default, generated keys have a validity value of 0 and never expire. Beginning with ONTAP 9.14.1, you can modify their retention period, after which the keys automatically expire. Enter the retention period in days, hours, minutes, or seconds.
. Beginning with ONTAP 9.19.1, users can have a second access key. If the user has a second access key, select the access key to regenerate. You can select both access keys for regeneration.
. Select *Storage > Storage VMs* and then select the storage VM.
...
. By default, generated keys are valid indefinitely. Beginning with ONTAP 9.14.1, you can modify their retention period, after which the keys automatically expire. You can add the retention period in this format: `P[<integer>D]T[<integer>H][<integer>M][<integer>S] | P<integer>W`
...
. Beginning with ONTAP 9.19.1, if the user has more than one access key, you can choose which key to regenerate. You specify the key in this format: `[-key-id {1..2}]`
+
...
. In the *Settings* tab, select image:icon_pencil.gif[Edit icon] in the *S3* tile.
. In the *Users* tab, select image:icon_kabob.gif[Menu options icon] next to the user, then select *Add another key*.
. By default, generated keys have a validity value of 0 and never expire. Beginning with ONTAP 9.14.1, you can modify their retention period, after which the keys automatically expire. Enter the retention period in days, hours, minutes, or seconds.
. Select *Storage > Storage VMs* and then select the storage VM.
...
. By default, generated keys are valid indefinitely. Beginning with ONTAP 9.14.1, you can modify their retention period, after which the keys automatically expire. You can add the retention period in this format: `P[<integer>D]T[<integer>H][<integer>M][<integer>S] | P<integer>W`
For example, if you want to enter a retention period of one day, two hours, three minutes, and four seconds, enter the value as `P1DT2H3M4S`.
. Beginning with ONTAP 9.19.1, you can generate a second access key in this format: `[-key-id 2]`
+
For example:
+
----
...
. Select *Storage > Storage VMs* and then select the storage VM.
. In the *Settings* tab, select image:icon_pencil.gif[Edit icon] in the *S3* tile.
. In the *Users* tab, verify that there are no second access keys for the S3 users.
--
...
. In the *Settings* tab, select image:icon_pencil.gif[Edit icon] in the *S3* tile.
. In the *Users* tab, select image:icon_kabob.gif[Menu options icon] next to the user, then select *Add another key*.
. Select *Remove key 1* or *Remove key 2*.
. Select *Save*. The selected key is removed. 
. Download or save the access key and secret key. They will be required for access from S3 clients.
--
