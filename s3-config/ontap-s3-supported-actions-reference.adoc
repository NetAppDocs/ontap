...
ONTAP S3 supports this additional header:
...
Beginning with ONTAP 9.19.1, ONTAP S3 supports enforcement of conditional writes for object uploads and conditional deletes for object deletes. Conditional writes and deletes provide a way to prevent overwrites or unintended deletes of objects. Conditional operations work only if the request is made using AWS_V4 signing method over a TLS connection. The following conditional headers and object operations are supported: 

[options="header"]
|===
| Object operation | Conditional header
...
ONTAP S3 supports this additional URI query parameter:
`uploadId`
a|ONTAP 9.8
...
ONTAP S3 supports these additional headers:
...
ONTAP S3 supports these additional URI query parameters:
...
ONTAP S3 supports these additional URI parameters:
...
* ONTAP S3 adds support for chunked uploads signing requests using `x-amz-content-sha256: 
...
// 2026-Sept-14, ONTAPDOC-4295
// 2026-Sep-09, GH 2116
// 2026-Jun-18, corrected typo
// 2025-Apr-16, ONTAPDOC-3666
