...
To maintain access S3 resources if the original access key expires, beginning with ONTAP 9.19.1, you can link:regenerate-access-keys.html#generate-a-second-s3-access-key[generate a second access key] for each S3 user.
...
.System Manager
--
. Select *Storage > Storage VMs*. Select the storage VM to which you need to add a user, select *Settings* and then select image:icon_pencil.gif[Edit icon] under S3.
. To add a user, select *Users > Add*.
...
. Select *Save*. The user is created, and an access key and a secret key are generated for the user. 
