...
+
Beginning in ONTAP 9.14.1, on volumes with xref:enable-temperature-sensitive-efficiency-concept.html[temperature-sensitive storage efficiency (TSSE)] enabled, the volume show-space -physical used command reports the amount of space used on the volume including the space savings realized as a result of TSSE.
