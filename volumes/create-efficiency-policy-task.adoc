...
[NOTE]
====
You can run deduplication or data compression operations at a scheduled time, or by creating a schedule with a specific duration, or by specifying a threshold percentage, which waits for the new data to exceed the threshold and then triggers the deduplication or data compression operation. This threshold value is the percentage of the total number of blocks used in the volume. For example, on a volume prefilled with 1GB of data and with a threshold set to 20%, deduplication or data compression triggers when you reach 204MB (20% of 1GB) of data on the volume. Setting the threshold to 20% after filling 204MB of data automatically triggers deduplication or data compression. If required, you can obtain the total number of blocks used from the `df` command output.
====
