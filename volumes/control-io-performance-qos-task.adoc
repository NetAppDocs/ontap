...
* Beginning with ONTAP 9.18.1, you can assign a QoS policy to an SVM and to volumes under the SVM at the same time. In releases earlier than ONTAP 9.18.1, if you assign a volume to a policy group, then you cannot assign the volume's containing SVM or any child LUNs or files to a policy group.
