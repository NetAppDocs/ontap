...
Starting with ONTAP 9.9, VIP BGP provides default route automation using BGP peer grouping to simplify configuration.
...
Starting with ONTAP 9.9, these fields have been added:
...
[NOTE]
While ONTAP supports the above BGP attributes, different routers support different attributes. NetApp highly recommends you confirm which attributes are supported by your router and configure ONTAP accordingly. For details, refer to the BGP documentation provided by your router.
...
+
*Sample 1*: Create a static route:
+
....
...
....
+
*Sample 2*: No static route needed:
+
....
