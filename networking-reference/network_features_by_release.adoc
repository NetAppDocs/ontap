...
permalink: networking-reference/network_features_by_release.html
...
** In switched cluster configurations, the system avoids ports that experience partial packet loss (link flapping).
** In switchless cluster configurations, the system avoids ports that experience total packet loss (connectivity loss).
** The system avoids elevated packet loss due to NIC issues (hardware errors).
* Automatic node failover:
** If a node cannot serve data across its cluster network, that node should not own any disks. Instead its HA pair should take over, if the partner is healthy.
* Diagnostic commands to analyze connectivity issues:
** Run the following command to discover cluster path connectivity information:
`cluster network-connectivity start-scan`
Use the results of this command to discover which paths are experiencing issues or are failing.
** Run the following command to discover cluster path performance information:
`cluster network-performance start-scan`
Use the results of this command to discover degraded performance or elevated loss on each cluster path.
** Run the following command to discover cluster path connectivity information:
...
a|VIP BGP provides default route automation using BGP peer grouping to simplify configuration. ONTAP has a simple way to learn default routes using the BGP peers next-hop routers.

The `asn_integer` parameter specifies the ASN.
...
* Starting in ONTAP 9.9, ASN for BGP supports a 4-byte non-negative integer (65536 - 4294967295). The default ASN is 65501. ASN 23456 is reserved for ONTAP session establishment with peers that do not announce 4-byte ASN capability.
|ONTAP 9.8
|Auto port placement
