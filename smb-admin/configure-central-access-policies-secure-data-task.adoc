...
summary: "There are several steps that you must take to secure access to data on the CIFS server using central access policies, including enabling Dynamic Access Control on the CIFS server, configuring central access policies in Active Directory, applying the central access policies to Active Directory containers with GPOs, and enabling GPOs on the CIFS server."
---
