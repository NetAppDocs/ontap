...
summary: "There are several reasons for creating local users and local groups on your storage virtual machine. For example, you can access an SMB server by using a local user account if the domain controllers are unavailable, you might want to use local groups to assign privileges, or your SMB server is in a workgroup."
---
