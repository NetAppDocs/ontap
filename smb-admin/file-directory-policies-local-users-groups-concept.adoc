...
summary: "There are certain guidelines that you must keep in mind before applying file-directory policies on the storage virtual machine disaster recovery destination in an ID discard configuration if your file-directory policy configuration uses local users or groups in either the security descriptor or the DACL or SACL entries."
---
