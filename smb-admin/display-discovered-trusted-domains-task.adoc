...
Beginning with ONTAP 9.13.1, ONTAP also performs site discovery for trusted domains and uses the discovered site when locating domain controllers for those domains. This helps ensure that DNS SRV lookups use the correct site information for trusted-domain domain controllers. If site discovery for a trusted domain fails, ONTAP falls back to the SMB server's default site.
