...
summary: "You can enable or disable local user authentication for SMB access on storage virtual machines. The default is to allow local user authentication, which is useful when the SVM cannot contact a domain controller or if you choose not to use domain-level access controls."
---
