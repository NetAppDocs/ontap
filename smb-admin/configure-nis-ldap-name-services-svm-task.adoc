...
====
 ...
 .. Verify that the NIS name service is properly configured and active: `vserver services name-service nis-domain show vserver vserver_name`
+
...
----
. If UNIX user, group, and netgroup information or name mapping is managed by LDAP name services, configure LDAP name services by using the information located in the _NFS Reference_.
