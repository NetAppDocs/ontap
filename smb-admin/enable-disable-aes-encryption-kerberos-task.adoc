...
summary: "To take advantage of the strongest security with Kerberos-based communication, you can enable AES-256 and AES-128 encryption on the SMB server. If you do not want the SMB server to select the AES encryption types for Kerberos-based communication with the Active Directory KDC, you can disable AES encryption. By default, AES encryption is disabled."
---
...
When you modify the `-advertised-enc-types` parameter, the specified encryption types replace any previously configured types. All previous encryption type settings are removed.

To enhance security, the storage virtual machine (SVM) changes its machine account password in the AD each time the AES security option is modified. Changing the password might require administrative AD credentials for the organizational unit (OU) that contains the machine account. A credential prompt appears only when ONTAP must reset the machine account password in Active Directory and your AD configuration requires administrative credentials for the organizational unit containing the machine account. If your AD is synchronized and auto-reset is enabled, no prompt appears.
