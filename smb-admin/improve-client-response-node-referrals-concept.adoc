...
summary: "Auto Location uses SMB automatic node referrals to increase SMB client performance on storage virtual machines. Automatic node referrals automatically redirect the requesting client to a LIF on the node SVM that is hosting the volume in which the data resides, which can lead to improved client response times."
---
