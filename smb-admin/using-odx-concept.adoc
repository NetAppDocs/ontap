...
+
ODX copy offload pass-through supports copy operations initiated from a Hyper-V guest operating system. The guest operating system must support ODX and use SCSI virtual disks backed by ODX-capable SMB or SAN storage. IDE virtual disks do not support ODX pass-through. The source and destination storage must be on the same cluster.
+
For more information, see link:use-cases-odx-concept.html[Use cases for ODX on ONTAP SMB servers].
