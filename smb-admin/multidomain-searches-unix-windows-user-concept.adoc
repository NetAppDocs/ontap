...
Beginning with ONTAP 9.13.1, when ONTAP searches trusted domains during multidomain name-mapping operations, it performs site discovery for those trusted domains and uses the discovered site for domain controller lookups. This behavior helps ensure that DNS SRV lookups use the correct site information for trusted-domain domain controllers. If trusted-domain site discovery fails, ONTAP uses the SMB server's default site.
