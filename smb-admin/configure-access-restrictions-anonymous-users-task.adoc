...
summary: "By default, an anonymous, unauthenticated user (also known as the null user) can access certain information on the network. You can use a CIFS server option to configure access restrictions for the anonymous user."
---
...
[.lead]
By default, an anonymous, unauthenticated user (also known as the _null user_) can access certain information on the network. You can use a CIFS server option to configure access restrictions for the anonymous user.

.About this task

The `-restrict-anonymous` CIFS server option corresponds to the `RestrictAnonymous` registry entry in Windows.
