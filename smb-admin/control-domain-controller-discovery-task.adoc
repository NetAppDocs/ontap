...
summary: "Beginning with ONTAP 9.3, you can modify the default process by which domain controllers are discovered. This enables you to limit discovery to your site or to a pool of preferred DCs, which can lead to performance improvements depending on the environment."
---
...
+
The `default-site` parameter for the SMB server can be defined to use this mode with LIFs that are not assigned to a site in sites-and-services. Beginning with ONTAP 9.13.1, the default site can also be used as a fallback if ONTAP cannot discover site information for a trusted domain.
