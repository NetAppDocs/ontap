...
The Active Directory Certificate Service must already be installed and configured for the domain to which the CIFS server belongs. You can find information about installing and configuring Active Directory Certificate Services by consulting the http://technet.microsoft.com/en-us/library/[Microsoft TechNet Library: technet.microsoft.com]

[NOTE]
====
Beginning with ONTAP 9.13.1, if `-use-start-tls-for-ad-ldap` or `-use-ldaps-for-ad-ldap` is enabled for the SMB server, ensure that the required CA certificates are installed for trusted domains as well as the home domain. ONTAP must be able to establish TLS or LDAPS connections to trusted-domain domain controllers to determine their site information.
====
.Step
...
[NOTE]
====
Beginning with ONTAP 9.13.1, when secure LDAP communication is enabled for Active Directory connections, ONTAP also uses those secure connections when contacting trusted-domain domain controllers. If the required certificates are not installed for trusted-domain domain controllers, ONTAP might be unable to determine trusted-domain site information.
====
