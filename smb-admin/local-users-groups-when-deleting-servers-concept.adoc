...
summary: "The default set of local users and groups is created when a CIFS server is created, and they are associated with the storage virtual machine hosting the CIFS server. SVM administrators can create local users and groups at any time. You need to be aware of what happens to local users and groups when you delete the CIFS server."
---
