...
. Reset the privileges on a local or domain user or group: `vserver cifs users-and-groups privilege reset-privilege -vserver vserver_name -user-or-group-name name`
. Verify that the privileges are reset on the object: `vserver cifs users-and-groups privilege show -vserver vserver_name ‑user-or-group-name name`
