...
summary: "You can se continuously available SMB 3.0 and later file shares to store Hyper-V virtual machine files or SQL Server system databases and user databases on volumes residing in SVMs, while at the same time providing nondisruptive operations for both planned and unplanned events."
---
...
* NetApp Backup and Recovery protects standalone Hyper-V virtual machines, FCI cluster instances, and System Center Virtual Machine Manager (SCVMM)-provisioned virtual machines hosted on CIFS shares.
+
For information about protecting Hyper-V workloads with NetApp Backup and Recovery, see the https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-hyperv-protect-overview.html[Protect Hyper-V workloads overview].
// ONTAPDOC-3772
