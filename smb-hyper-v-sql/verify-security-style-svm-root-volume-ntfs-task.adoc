...
summary: "To ensure that nondisruptive operations for Hyper-V and SQL Server over SMB are successful, volumes must be created with NTFS security style. Since the root volume’s security style is applied by default to volumes created on the storage virtual machine, the security style of the root volume should be set to NTFS."
---
