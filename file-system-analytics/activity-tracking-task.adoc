...
Beginning in ONTAP 9.13.1, Activity Tracking is enabled by default on all newly created volumes. 

* If you are using the ONTAP CLI to create a volume, no additional parameters are required to enable Activity Tracking by default. To disable Activity Tracking on a newly created volume, issue the `volume create` command with the `-activity-tracking-state` parameter set to `off`. 
+
Alternately, you disable activity tracking after creating a volume using the  volume activity-tracking off.
* If you are using System Manager, you can disable Activity Tracking in the *Activity Tracking* menu or in the cluster settings. 
...
. Enable Activity Tracking:
+
`volume activity-tracking on -vserver vs0 -volume [* | !_file_names_]
+
Using `\*` will enable Activity Tracking for all volumes on the specified storage VM. 
+
Using `!` followed by volume names will enable Activity Tracking for all volumes on the SVM except the named volumes.
+
