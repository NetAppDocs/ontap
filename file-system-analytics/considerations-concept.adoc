...
If you experience a notable increase in latency, refer to the link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/High_or_fluctuating_latency_after_turning_on_NetApp_ONTAP_File_System_Analytics[NetApp Knowledge Base: High or fluctuating latency after turning on NetApp ONTAP File System Analytics^].

== Scan considerations

When you enable capacity analytics, ONTAP conducts an initialization scan for capacity analytics. The scan accesses metadata for all files in volumes for which capacity analytics is enabled. No file data is read during the scan. Beginning with ONTAP 9.14.1, you can track the progress of the scan with the ONTAP REST API, in the **Explorer** tab of System Manager, or with the `volume analytics show` ONTAP CLI command. If there is a throttling event, ONTAP provides a notification.
