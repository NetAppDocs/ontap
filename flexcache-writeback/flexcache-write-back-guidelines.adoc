...
* Write-back caches automatically switch to write-around mode when the origin volume's free space drops to 20% or below. This helps prevent running out of space at the origin, which would result in leaving dirty data orphaned at a write-back-enabled cache. However, if the origin volume is overprovisioned--meaning its logical provisioned size is larger than the free space in the aggregate--the reported free space percentage may trigger this threshold earlier than you expect. The 20% threshold is evaluated against the both origin volume's reported free space and the aggregate's available physical capacity.
