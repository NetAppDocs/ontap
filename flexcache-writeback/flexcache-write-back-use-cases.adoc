...
Knowing that these calls must traverse the WAN helps you to identify workloads suited for write-back. Generally, the more writes that can be done between `OPEN` and `CLOSE` calls without one of the other calls <write-calls,above> being issued, the better the performance gain write-back provides.
...
image::point-of-return-nfs3-writeback.png[Point of Return]

IMPORTANT: Write-back should not be used for intracluster caching. Intracluster caching is when the origin and cache are in the same cluster.
