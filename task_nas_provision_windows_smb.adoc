...
After verifying access, you can restrict client access with the share ACL and set any desired security properties on the mapped drive. See xref:/smb-config/create-share-task.html[Create an SMB share] for more information.
...
*** xref:/smb-hyper-v-sql/continuously-available-share-hyper-v-concept.html[Continuously available share requirements for Hyper-V over SMB]
*** xref:/smb-hyper-v-sql/continuously-available-share-sql-concept.html[Continuously available share requirements for SQL Server over SMB]
...
To modify an exisitng share, click **Storage > Shares**, then click the image:icon_kabob.gif[three-dot veritcal kabob icon] and select **Edit**.
...
This topic describes how to provision SMB storage with System Manager (ONTAP 9.7 and later).

[cols=2,options="header"]
|===
| To perform this task with... | See this content...
...
| Command line interface | link:../smb-config/index.html[SMB configuration overview with the CLI]

|===

// 2021-10-27, IE-442
// 2021-06-16, BURT 1395879
