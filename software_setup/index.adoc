...
System Manager provides a simple and easy workflow for cluster set and configuration including assigning a node management IP address, initializing the cluster, creating a local tier, configuring protocols and provisioning initial storage. For example, suppose you have completed the installation of a new AFF C-series system.  You can use System Manager to setup a new cluster, enable iSCSI or FC for SAN access, and provision LUNs for storage. 
