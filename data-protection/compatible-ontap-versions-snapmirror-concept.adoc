...
[NOTE]
When link:flexgroup/enable-adv-capacity-flexgroup-task.html[advanced capacity balancing] is enabled on clusters running ONTAP 9.16.1 or later, SnapMirror transfers are not supported to clusters running ONTAP versions earlier than ONTAP 9.16.1.
