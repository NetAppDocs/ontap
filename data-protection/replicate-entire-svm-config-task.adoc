...
* Beginning with ONTAP 9.19.1, enabling IPsec is not supported on SVMs configured for SVM DR. If you are using IPsec, you must disable it or choose a different SVM to configure an SVM DR relationship. You will receive an error if you are using ONTAP 9.19.1 or later and you attempt to enable IPsec on an SVM configured for SVM DR.
...
. From the source cluster, create an SVM peer relationship using the `vserver peer create` command.
+
...
// 2026-Aug-5, ONTAPDOC-4017
// 2026-July-1, CONTAP-668182
