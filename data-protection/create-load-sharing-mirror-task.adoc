...
LSM is not suitable for single HA pair clusters, and it should be configured for multiple HA pair clusters, configuring the LSM between the node with the root volume and another node from a different HA pair.
