...
[.lead]
You can recover a volume to an earlier point in time by restoring from a Snapshot copy. You can use the `volume snapshot restore` command to restore the contents of a volume from a Snapshot copy.
...
.Steps
You can use System Manager or the ONTAP CLI.
...
. Under *Snapshot Copies*, click image:icon_kabob.gif[Menu Options] next to the Snapshot copy you want to restore, and select *Restore*.
--
.ClI
--
