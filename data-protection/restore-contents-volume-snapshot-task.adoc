...
[.lead]
You can recover a volume to an earlier point in time by restoring from a snapshot. You can use System Manager or the `volume snapshot restore` command to restore the contents of a volume from a snapshot.
...
. Under *Snapshots*, click image:icon_kabob.gif[Menu options icon] next to the snapshot you want to restore, and select *Restore*.
--
.CLI
--
