...
You should be aware that if a filesystem is rebooted for any reason; for example, a node reboot, takeover/giveback, or panic, then initialization will not automatically resume and must be restarted manually.
