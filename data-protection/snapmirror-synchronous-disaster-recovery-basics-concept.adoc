...
Beginning with ONTAP 9.19.1, you can use the SnapMirror policy parameter `-sync-snapshot-replication-mode` in advanced privilege mode to replicate scheduled snapshots to the SnapMirror destination during `snapshot create`, `snapshot delete`, `snapshot rename`, and `snapshot modify` operations. Replicating scheduled snapshots provides symmetry between the primary and secondary volumes in a SnapsMirror synchronous relationship.

.Prerequisites

* You must be a cluster or SVM administrator
* You must be in advanced privilege mode
* You must configure a user-defined SnapMirror policy that includes the option `sched_created`.
