...
The parameter also accepts 'none' or a comma-separated list of types: `application_created`, `sched_created`, and `sm_created`. The parameter only takes effect once the relationship status is `InSync`. Snapshots that are not replicated according to the policy parameter are tagged internally as primary-only. 
