...
h| Platform h| ONTAP 9.14.1 through ONTAP 9.11.1 h|  ONTAP 9.10.1 h| ONTAP 9.9.1  h| Releases earlier than ONTAP 9.9.1
a|
AFF
a|
400 
...
| ONTAP 9.7
| If a snapshot is tagged with the appropriate label at the time of the `snapshot create` operation, using the CLI or the ONTAP API, SnapMirror synchronous replicates the snapshots, both user created or those created with external scripts, after quiescing the applications. Beginning with ONTAP 9.14.1, the label requirement for replication of application-created snapshots no longer exists. For more information about replicating application-created snapshots, see the link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous[NetApp Knowledge Base: How to replicate application created snapshots with SnapMirror synchronous^].

| Autonomous ransomware protection
| ONTAP 9.19.1
// TODO: Verify with engineering that this description accurately reflects 9.19.1 GA behavior for ARP on SMS volumes
| If you enable ARP on volumes that are part of SnapMirror synchronous or SnapMirror active sync relationships after the upgrade, the relationship briefly moves to an out-of-sync state. This condition is temporary, and the relationship should automatically resynchronize without intervention. For more information, see link:../anti-ransomware/use-cases-restrictions-concept.html[Autonomous Ransomware Protection use cases and considerations].
...
| Replication of snapshot management operations
| ONTAP 9.19.1
| Beginning with ONTAP 9.19.1, the snapshot management operations `snapshot delete`, `snapshot rename`, and `snapshot modify` are replicated to secondary cluster.

| Scheduled snapshot replication
| ONTAP 9.19.1
| Beginning with ONTAP 9.19.1, you can enable replication of scheduled snapshots using a SnapMirror policy. In releases earlier than ONTAP 9.19.1, scheduled snapshots created using a snapshot policy are not replicated.
...
| ONTAP 9.6
a| * A SnapMirror synchronous source can also be a SVM disaster recovery source, for example, a fan-out configuration with SnapMirror synchronous as one leg and SVM disaster recovery as the other.
...
| ONTAP 9.19.1
| You can specify the expiry time only during snapshot creation. You cannot set or extend the expiry time on existing snapshots.
...
Beginning with ONTAP 9.19.1, you can use the SnapMirror policy parameter `-sync-snapshot-replication-mode` in advanced privilege mode to replicate scheduled snapshots to the SnapMirror destination. Replicating scheduled snapshots provides symmetry between the primary and secondary volumes in a SnapsMirror synchronous relationship.

.Prerequisites

* You must be a cluster or SVM administrator
* You must be in advanced privilege mode
* You must configure a user-defined SnapMirror policy that uses the parameter `-sync-snapshot-replication-mode` and includes the option `sched_created`.
** System-defined policies cannot be modified. You must create a new policy and reestablish the relationship.
** User-defined policy changes require a resync (quiesce followed by resume) to take effect on existing relationships.

The parameter accepts these options:

 * `none`- disables replication of non-SnapMirror synchronous snapshots
 * `application_created`- replicates application-created snapshots
 * `sched_created` - replicates scheduled snapshots
 * `sm_created` - replicates SnapMirror asynchronous (fanout leg) created snapshots
 * A comma-separated list of types: `application_created`, `sched_created`, and `sm_created`. 
 
The parameter only takes effect once the relationship status is `InSync`. Snapshot types not listed are stored only on the primary volume. 

Beginning with ONTAP 9.19.1, `snapmirror initialize` replicates all snapshots to the secondary cluster, while `snapmirror resync` only replicates new snapshots created after the last common snapshot. During a resync operation, non-replicated snapshots tagged as primary-only when the relationship status is `InSync` are not replicated to the secondary cluster, and those snapshots are retained at the new secondary cluster following a flip resync operation.

NOTE: Creating snapshots on SnapMirror synchronous volumes can increase user I/O latency. It is recommended that you use snapshot schedules with a frequency greater than 30 minutes.

.Related information

* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf[NetApp Technical Report 4733: SnapMirror synchronous configuration and best practices^]
...
// 2026-Jun-11, contap-719516
// 2026-Jun-2, ONTAPDOC-3918
// 2026-May-1, CONTAP-688240
// 2026-Apr-29, ONTAPDOC-3870 & CONTAP-697860
// 2025-Dec-3, GitHub issue# 1918
// 2025 Sep 08, ONTAPDOC-1127
