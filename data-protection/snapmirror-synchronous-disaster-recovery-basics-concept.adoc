...
Beginning with ONTAP 9.19.1, `snapmirror nitialize` replicates all snapshots to the secondary cluster, while `snapmirror resync` only replicates new snapshots created after the last common snapshot. During a resync operation, non-replicated snapshots tagged as primary-only when the relationship status is `InSync` are not replicated to the secondary cluster, and those snapshots are retained at the new secondary cluster following a flip resync operation.
