...
. From the original destination SVM or the original destination cluster, delete the original data protection relationship:
+
...
+
`snapmirror release -source-path SVM:volume -destination-path SVM:volume -relationship-info-only true`
+
[NOTE]
====
You must enter a colon (:) after the SVM name in the `-source-path` and `-destination-path` options. See the example below.
...
+
----
cluster_src::> snapmirror release -source-path svm1:volA -destination-path svm_backup:volA_dst -relationship-info-only true
----

. From the original source SVM or the original source cluster, create a reverse relationship using the same configuration, policy, and identity-preserve setting as the original relationship:
+
`snapmirror create -source-path SVM:volume -destination-path SVM:volume`
+
[NOTE]
====
You must enter a colon (:) after the SVM name in the `-source-path` and `-destination-path` options. See the example below.
====
+
The following example creates a relationship between the SVM from which you are serving data, `svm_backup:volA_dst`, and the original source SVM, `svm1:volA`:
+
----
cluster_src::> snapmirror create -source-path svm_backup:volA_dst -destination-path svm1:volA
----

. From the original source SVM or the original source cluster, reverse the original data protection relationship:
+
...
+
`snapmirror release -source-path _SVM:volume_ -destination-path _SVM:volume_ -relationship-info-only true`
+
[NOTE]
====
...
+
----
cluster_dst::> snapmirror release -source-path svm_backup:volA_dst -destination-path svm1:volA -relationship-info-only true
----
