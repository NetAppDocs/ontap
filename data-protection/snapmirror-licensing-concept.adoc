...
Data Protection Optimized (DPO) installations are no longer being sold, and DPO is not supported on current platforms; however, if you have a DPO license installed on a supported platform, NetApp continues to provide support until the end of availability of that platform. 

You cannot upgrade to the ONTAP One license bundle if the DPO license is installed on a system.
