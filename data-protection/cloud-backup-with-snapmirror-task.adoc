...
= Back up data to the cloud using ONTAP SnapMirror cloud
:toclevels: 1
:hardbreaks:
:nofooter:
:icons: font
...
[.lead]
_SnapMirror cloud_ provides ONTAP replication to object store destinations. This allows you to replace legacy backup-to-tape architectures with object storage for long-term data retention and archiving. You can use providers like Amazon S3, Alibaba Cloud, Azure Blob, Google Cloud Storage, IBM Cloud Object Storage, NetApp StorageGRID, or ONTAP S3 as your cloud object store.

[NOTE]
SnapMirror Cloud replication that is managed by SystemManager (including via Console) only supports StorageGrid and ONTAP S3 as the object storage destination. Other providers are not supported.

[NOTE]
Support for SnapMirror Cloud replication to other object store providers with other backup and recovery products varies. Consult your 3rd party vendor for their support policy.

[NOTE]
Use of the SnapMirror Cloud APIs is generally restricted to backup partners. Customers who wish to develop their own API-based solutions should contact their NetApp account team for information on license terms.

Beginning with ONTAP 9.19.1:

* SnapMirror cloud increases the number of S3 object store buckets supported per cluster from 6 to 100. This limit applies to the total number of object stores configured across all SnapMirror cloud object store endpoints on an ONTAP cluster. When using this feature, be aware of the following considerations:
** When you use more than 6 buckets per cluster, you should use the virtual-hosted-style URL configuration for providers that support it, including AWS S3, Alibaba Cloud, StorageGrid, Google Cloud Storage, IBM Cloud Object Storage, ONTAP S3, and other S3-compatible providers. This configuration is recommended to enable scalable object store addressing and to avoid underlying limitations.
** On-demand restore is enabled by default. 
** Latency of reads during on-demand restores can be higher depending on how many concurrent restores are active in the system.

Beginning with ONTAP 9.18.1:

* SnapMirror cloud supports backup and restore operations for FlexGroup volumes on MetroCluster configurations using the existing link:https://docs.netapp.com/us-en/ontap-automation/get-started/access_rest_api.html[ONTAP REST APIs^]. This functionality enables you to create SnapMirror cloud relationships for FlexGroup volumes on MetroCluster configurations that are maintained from the partner site after a switchover and switchback.
...
[NOTE]
You may need both a SnapMirror Cloud license key and an API key. While ONTAP One includes the SnapMirror Cloud license itself, the license key is required to enable the SnapMirror Cloud feature. The license key may need to be generated and installed separately.
...
// 2026-June-10, ONTAPDOC-3938
// 2026-Apr-28, ONTAPDOC-3790
// 2026 Jan 19, ONTAPDOC-3325
