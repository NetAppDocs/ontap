...
[NOTE]
SnapMirror Cloud replication that is managed by SystemManager, Console, or other NetApp data protection products only support StorageGrid and ONTAP S3 as the object storage destination. Other providers are not supported.
...
[NOTE]
You may need both a SnapMirror Cloud license key and an API key. While ONTAP One includes the license itself, the license file may need to be generated and installed separately.
