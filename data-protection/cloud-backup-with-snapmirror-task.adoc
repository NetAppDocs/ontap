...
[NOTE]
SnapMirror Cloud replication that is managed by System Manager only supports StorageGrid and ONTAP S3 as the object storage destination. Other providers are not supported.
...
[NOTE]
You might need both a SnapMirror Cloud license key and an API key. While ONTAP One includes the SnapMirror Cloud license itself, the license key is required to enable the SnapMirror Cloud feature. The license key may need to be generated and installed separately.
