...
[NOTE]
====
 SnapMirror Cloud replication that is managed by SystemManager, Console, or other NetApp data protection products only supports StorageGrid and ONTAP S3 as the object storage destination. Other providers are not supported.

 SnapMirror Cloud replication to other object store providers with other backup and recovery products depends on the product. Consult the 3rd party vendor for their support policy.
====
