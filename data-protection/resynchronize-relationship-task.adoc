...
* For extended data protection (XDP) relationships, you can use the optional `-quick-resync true` parameter with the `snapmirror resync` command. It's recommended that you use this parameter if the source of the resync does not have volume efficiency enabled or if reducing resync time is more important than preserving all possible storage efficiency. 
+
This parameter is not supported for relationships with non-ONTAP endpoints or for relationships with a policy of type strict-sync-mirror and sync-mirror. When you specify this parameter, resync does not preserve the storage efficiency of the new data with existing data over the wire and on the destination. for more information `snapmirror resync`, see the link:https://docs.netapp.com/us-en/ontap-cli/snapmirror-resync.html[ONTAP command reference^].
