...
[.lead]
Beginning with ONTAP 9.9.1, you can use System Manager to back up data in the cloud using BlueXP backup and recovery service (formerly Cloud Backup).
...
.	In System Manager, click *Protection > Overview*, then scroll to the *BlueXP backup and recovery* section.
...
.	Click *Connect and enable BlueXP backup and recovery*.
...
.	Click  image:icon_kabob.gif[Menu options icon], then *Edit*.
